...
public class ServerModel implements Serializable {
	...
	public static class DcsMaster {
		 ...
		 * @param listenerPort the port its listening on
		 * @param portRange the listener port range
		 * @param startTimestamp the start timestamp
		 */
		public DcsMaster(String hostName, String listenerPort, String listenerPortRange, String starTimestamp) {
		...
		public DcsServer addDcsServer(String znode, String data) { 
			...
			scn.close();
			ServerModel.DcsServer dcsServer = new ServerModel.DcsServer(hostName, instance, infoPort, startTimestamp);
	...
	public static class DcsServer {
		 ...
		 */
		public DcsServer(String hostName, String instance, String infoPort,String starTimestamp) {
			this.hostName = hostName;
		...
		public void addTrafodionServer(String znode, String data) { 
			...
			if(this.hostName.equalsIgnoreCase(hostName) && this.instance.equalsIgnoreCase(instance))
				trafServerList.add(new ServerModel.TrafodionServer(hostName,instance,trafInstance 
						,state,timestamp,dialogueId,nid,pid,processName,ipAddress,port,clientHostName
	...
	public static class TrafodionServer {
		 ...
		 * @param startTime the starttime
		 */
		public TrafodionServer(String hostName,String dcsInstance,String instance,String state
				,String timestamp,String dialogueId,String nid,String pid,String processName
				,String ipAddress,String port,String clientHostName,String clientIpAddress
				,String clientPort,String clientAppl) {
			this.hostName = hostName;
	...
	public DcsMaster addDcsMaster(String znode, String data) { 
		...
		String startTimestamp = scn.next();
		scn.close();
		dcsMaster = new ServerModel.DcsMaster(hostName, listenerPort, listenerPortRange, startTimestamp);
		return dcsMaster;
	}
	...
	public String toString() {
		...
		/*
		if (!dcsMaster.isEmpty()) {
			//sb.append(String.format("%d DcsMaster server(s), %d DcsServer server(s)\n\n",dcsMaster.size(), dcsMaster.getDcsServer().size());
			//sb.append(liveNodes.size());
			//sb.append(" live servers\n");
			for (DcsMaster aServer: dcsMaster) {
				sb.append("    ");
				sb.append(node.name);
				sb.append(' ');
