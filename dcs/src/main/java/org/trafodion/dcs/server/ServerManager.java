/**********************************************************************
 * @@@ START COPYRIGHT @@@ Licensed to the Apache Software Foundation (ASF) under one or more
...
public final class ServerManager implements Callable {
    ...
    public boolean monitor() {
      try {
        ...
        if (stat != null) { // User program znode found in
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug("[" + (Integer.parseInt(nid) + 1) + ":" + childInstance + "]." + pid
                + ".isRunning [" + isRunning + "]");
          }
        }
      } catch (Exception e) {
  ...
  class ServerRunner {
    ...
    public ServerRunner(int childInstance, String registeredPath) {
      ...
      String command = userProgramCommand.replace("-ZKHOST", "-ZKHOST " + zkc.getZkQuorum() + " ")
          ...
          .replace("-STATISTICSCACHESIZE", "-STATISTICSCACHESIZE " + statisticsCacheSize + " ")
          ...
          .replace("-TCPKEEPALIVESTATUS", "-TCPKEEPALIVESTATUS " + userProgKeepaliveStatus + " ")
    ...
    public void exec() throws Exception {
      cleanupZk();
      LOG.info("Instance : [" + childInstance + "], User program exec ["
          + scriptContext.getCommand() + "]");
      ScriptManager.getInstance().runScript(scriptContext);// This will
                                                           // block while
                                                           // user prog is
                                                           // running
      LOG.info("Instance : [" + childInstance + "], User program exit ["
    ...
    private void cleanupZk() {
      try {
        ...
        if (stat != null) zkc.delete(registeredPath, -1);
  ...
  class ServerHandler implements Callable<Integer> {
    ...
    public ServerHandler(Configuration conf, int childInstance) {
      int maxRestartAttempts =
          conf.getInt(Constants.DCS_SERVER_STARTUP_MXOSRVR_USER_PROGRAM_RESTART_HANDLER_ATTEMPTS,
      ...
      this.childInstance = childInstance;
      this.registeredPath = parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_SERVERS_REGISTERED + "/"
          + hostName + ":" + instance + ":" + childInstance;
      retryCounter =
          RetryCounterFactory.create(maxRestartAttempts, retryTimeoutMinutes, TimeUnit.MINUTES);
  ...
  public ServerManager(Configuration conf, ZkClient zkc, DcsNetworkConfiguration netConf,
    ...
    this.hostName = getLowCaseShortHostName(netConf.getHostName());
    this.instance = Integer.parseInt(instance);
    this.infoPort = infoPort;
    this.childServers = childServers;
    this.parentZnode =
        this.conf.get(Constants.ZOOKEEPER_ZNODE_PARENT, Constants.DEFAULT_ZOOKEEPER_ZNODE_PARENT);
    this.connectingTimeout = this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_CONNECTING_TIMEOUT,
      Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_CONNECTING_TIMEOUT);
    this.zkSessionTimeout =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_ZOOKEEPER_SESSION_TIMEOUT,
          Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_ZOOKEEPER_SESSION_TIMEOUT);
    this.userProgExitAfterDisconnect =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_EXIT_AFTER_DISCONNECT,
          Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_EXIT_AFTER_DISCONNECT);
    this.maxHeapPctExit = this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_MAX_HEAP_PCT_EXIT,
      Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_MAX_HEAP_PCT_EXIT);
    this.statisticsIntervalTime =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_STATISTICS_INTERVAL_TIME,
          Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_STATISTICS_INTERVAL_TIME);
    this.statisticsLimitTime =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_STATISTICS_LIMIT_TIME,
          Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_STATISTICS_LIMIT_TIME);
    this.statisticsCacheSize =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_STATISTICS_CACHE_SIZE,
          Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_STATISTICS_CACHE_SIZE);
    this.statisticsType = this.conf.get(Constants.DCS_SERVER_USER_PROGRAM_STATISTICS_TYPE,
      Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_STATISTICS_TYPE);
    this.statisticsEnable = this.conf.get(Constants.DCS_SERVER_USER_PROGRAM_STATISTICS_ENABLE,
      Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_STATISTICS_ENABLE);
    this.sqlplanEnable = this.conf.get(Constants.DCS_SERVER_USER_PROGRAM_STATISTICS_SQLPLAN_ENABLE,
      Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_STATISTICS_SQLPLAN_ENABLE);
    this.userProgPortMapToSecs =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_PORT_MAP_TIMEOUT_SECONDS,
          Constants.DEFAULT_DCS_SERVER_USER_PROGRAM_PORT_MAP_TIMEOUT_SECONDS);
    this.userProgPortBindToSecs =
        this.conf.getInt(Constants.DCS_SERVER_USER_PROGRAM_PORT_BIND_TIMEOUT_SECONDS,
    ...
    this.userProgKeepaliveStatus =
        conf.get(Constants.DEFAULT_DCS_SERVER_PROGRAM_TCP_KEEPALIVE_STATUS,
          Constants.DCS_SERVER_PROGRAM_KEEPALIVE_STATUS);
    this.userProgKeepaliveIdletime =
        conf.getInt(Constants.DEFAULT_DCS_SERVER_PROGRAM_TCP_KEEPALIVE_IDLETIME,
          Constants.DCS_SERVER_PROGRAM_KEEPALIVE_IDLETIME);
    this.userProgKeepaliveIntervaltime =
        conf.getInt(Constants.DEFAULT_DCS_SERVER_PROGRAM_TCP_KEEPALIVE_INTERVALTIME,
          Constants.DCS_SERVER_PROGRAM_KEEPALIVE_INTERVALTIME);
    this.userProgKeepaliveRetrycount =
        conf.getInt(Constants.DEFAULT_DCS_SERVER_PROGRAM_TCP_KEEPALIVE_RETRYCOUNT,
          Constants.DCS_SERVER_PROGRAM_KEEPALIVE_RETRYCOUNT);
    serverHandlers = new ServerHandler[this.childServers];
  }

  private String getLowCaseShortHostName(String hostName) {
  ...
  private boolean isTrafodionRunning(String nid) {
    ...
    scriptContext.setScriptName(Constants.SYS_SHELL_SCRIPT_NAME);
    String command;
    if (nid != null) command = ("sqcheck -n " + nid);
    else command = ("sqcheck");
    scriptContext.setCommand(command);
  ...
  public Boolean call() throws Exception {
    ...
    /*
     * ExecutorService pool = Executors.newSingleThreadExecutor(); try { getMaster();
     * registerInRunning(); featureCheck(); Callable<Boolean> serverMonitor = new ServerMonitor();
     * Callable<ScriptContext> serverRunner = new ServerRunner(); long timeoutMillis=5000;
     * while(true) { Future<Boolean> monitor = pool.submit(serverMonitor); if(false ==
     * monitor.get().booleanValue()) { //blocking call LOG.info("User program is not running");
     * Future<ScriptContext> runner = pool.submit(serverRunner); ScriptContext scriptContext =
     * runner.get();//blocking call StringBuilder sb = new StringBuilder(); sb.append("exit code ["
     * + scriptContext.getExitCode() + "]"); if(! scriptContext.getStdOut().toString().isEmpty())
     * sb.append(", stdout [" + scriptContext.getStdOut().toString() + "]"); if(!
     * scriptContext.getStdErr().toString().isEmpty()) sb.append(", stderr [" +
     * scriptContext.getStdErr().toString() + "]"); LOG.info(sb.toString());
     * switch(scriptContext.getExitCode()) { case 3: LOG.error("Trafodion is not running");
     * timeoutMillis=60000; break; case 127: LOG.error("Cannot find user program executable");
     * timeoutMillis=60000; break; default: timeoutMillis=5000; } } else { timeoutMillis=5000; } try
     * { Thread.sleep(timeoutMillis); } catch (InterruptedException e) { } } } catch (Exception e) {
     * e.printStackTrace(); LOG.error(e); pool.shutdown(); throw e; }
     */
  }

  private void featureCheck() {
    final String msg1 = "Property " + Constants.DCS_SERVER_USER_PROGRAM + " is false. "
    ...
    boolean ready = false;
    while (!ready) {
      userProgEnabled = conf.getBoolean(Constants.DCS_SERVER_USER_PROGRAM,
        Constants.DEFAULT_DCS_SERVER_USER_PROGRAM);
      userProgramHome = System.getProperty("dcs.user.program.home");
      userProgramCommand = conf.get(Constants.DCS_SERVER_USER_PROGRAM_COMMAND,
      ...
      if (userProgEnabled == false) LOG.error(msg1);
      if (userProgramHome.isEmpty()) LOG.error(msg2);
  ...
  private void getMaster() {
    ...
    while (!found) {
      LOG.info("Checking DcsMaster znode [" + parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER
          + "]");
      try {
        Stat stat = zkc.exists(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER, false);
        if (stat != null) {
          List<String> nodes =
              zkc.getChildren(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER, null);
          if (!nodes.isEmpty()) {
            ...
            found = true;
            LOG.info("DcsMaster znode [" + parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER
                + "] found");
          }
        } else {
          LOG.info("DcsMaster znode [" + parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_MASTER
  ...
  private void registerInRunning(int instance) {
    String znode = parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_SERVERS_RUNNING + "/" + hostName
        + ":" + instance + ":" + infoPort + ":" + System.currentTimeMillis();
    try {
