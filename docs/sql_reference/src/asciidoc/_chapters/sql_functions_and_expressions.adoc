...
+
Besides the `_format-string_` listed above, following character string literals are also valid:
...
* This example caculates the number of days since Julian date and converts the `_DATE_` value to the character value.
+
```
...
* This example caculates the number of days since Julian date and converts the `_TIMESTAMP_` value to the character value.
+
```
