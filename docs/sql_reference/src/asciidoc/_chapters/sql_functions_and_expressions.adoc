...
a| * `_datetime-expression_` +
Week of year. +
** The value 1 is returned for datetime that occurs in the first 7 days of the year if the year begins on a Sunday. +
...
** The value 54 is returned for datetime that starts on Saturday where December 31 is in the 54th full or partial week in a leap year.
a| * `_datetime-expression_` 
** DATE
** TIMESTAMP
| WOM
