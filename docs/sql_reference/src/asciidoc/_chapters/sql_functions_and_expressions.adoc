...
[[syntax_description_of_floor]]
=== Syntax Description of FLOOR

`_numeric-expression_`

is an SQL numeric value expression to be rounded down. For more information, see <<numeric_value_expressions,Numeric Value Expressions>>.
...
+
** If `_numeric-expression_` is a negative number, note that a *smaller* value has a *larger* absolute value.
...
The CEIL (CEILING) function, which is *reverse* of the FLOOR function, rounds up `_numeric-expression_` and returns the smallest integer greater than or equal to `_numeric-expression_`. 
...
[[comparison_among_functions_of_rounding_numbers]]
==== Comparison among Functions of Rounding Numbers
...
|Rounds `_numeric-expression_` *downward* to an integer. 
...
|Rounds `_numeric-expression_` *upward* to an integer.

|TRUNC +
(TRUNCATE)

|`TRUNC (_numeric-expression_, _num_)` +

`TRUNCATE (_numeric-expression_, _num_)`

|Truncates `_numeric-expression_` on `_num_` decimal positions without rounding.
...
|`ROUND (_numeric-expression_, _num_)` 

|Rounds `_numeric-expression_` on `_num_` decimal positions.

This is the most complicated one of the four functions. For more information, see <<round_function,ROUND Function>>.

|The result has the same scale as `_numeric-expression_`.
...
[[decimal_numbers_have_multiple_digits_right_to_the_decimal]]
...
[[decimal_numbers_have_multiple_digits_left_to_the_decimal]]
