...
CREATE TABLE HIVE.TPCDS_SF1000_ORC.customer_address1
  ...
  tblproperties
 (
    "orc.bloom.filter.columns"="CA_COUNTY, CA_CITY, CA_GMT_OFFSET, CA_LOCATION_TYPE"
...
REGISTER /*INTERNAL*/ HIVE TABLE HIVE.TPCDS_SF1000_ORC.customer_address1;
/* ObjectUID = 8789483533085275772 */

CREATE EXTERNAL TABLE customer_address1
  (
...
+
The size of the table _customer_address1_ is 71.1M.
+
```
[trafodion@nap001 ~]$ hdfs dfs -du -h /user/hive/warehouse/tpcds_sf1000_orc.db/customer_address1
42.4 M  84.8 M  /user/hive/warehouse/tpcds_sf1000_orc.db/customer_address1/000000_0    
28.7 M  57.4 M  /user/hive/warehouse/tpcds_sf1000_orc.db/customer_address1/000001_0
```

+
It takes approximately 13 seconds to get the result using the external table.
+
```
SQL>SELECT [ LAST 1 ] * FROM customer_address1 WHERE char_length(CA_ADDRESS_ID) > 100;        
--- 0 row(s) selected. 

Start Time             2018/01/31 02:21:55.409105
End Time               2018/01/31 02:22:09.299215
Elapsed Time                      00:00:13.890110
Compile Time                      00:00:00.264101
Execution Time                    00:00:13.625538
```

** This is the definition of the table _customer_address2_, it has same structure and size as the table _customer_address1_, but it does not have an external table.
+
```
/* Hive DDL */
CREATE TABLE TPCDS_SF1000_ORC.customer_address2
  (
    CA_ADDRESS_SK                    int
  , CA_ADDRESS_ID                    string
  , CA_STREET_NUMBER                 string
...
+
It takes approximately 24 seconds to get the result not using the external table.

+
```
SQL>SELECT [ LAST 1 ] * FROM HIVE.HIVE.customer_address2 WHERE char_length(CA_ADDRESS_ID) > 100;

*** WARNING[6008] Statistics for column (CA_ADDRESS_ID) from table HIVE.TPCDS_SF1000_ORC.CUSTOMER_ADDRESS2 were not available. As a result, the access path chosen31 02:22:20]
--- 0 row(s) selected. 

Start Time             2018/01/31 02:22:20.783859
End Time               2018/01/31 02:22:45.146483
Elapsed Time                      00:00:24.362624
Compile Time                      00:00:00.023941
Execution Time                    00:00:24.307888
```
