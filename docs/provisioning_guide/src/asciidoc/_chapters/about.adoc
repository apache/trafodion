...
= About This Document
This guide describes how to provision Trafodion for end-user environments. This install allows you to store and query data using Trafodion, either via Trafodion clients
...
* *Single-Node Environments*: Typically used when you want to evalulate Trafodion.
* *Cluster (Multi-Node) Environments*: Typically used when you deploy Trafodion for application usage.

NOTE: Trafodion can be provisioned on a single-node or multi-node enviroment. Unless specifically noted, the term *cluster* is used
