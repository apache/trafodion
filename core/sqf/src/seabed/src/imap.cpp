...
//
// Implement integer map
//
SB_Imap::SB_Imap(int pv_buckets, float pv_lf)
: SB_Map_Comm(SB_ECID_MAP_I, "imap", pv_buckets, pv_lf) {
    init();
}

SB_Imap::SB_Imap(const char *pp_name, int pv_buckets, float pv_lf)
: SB_Map_Comm(SB_ECID_MAP_I, "imap", pp_name, pv_buckets, pv_lf) {
    init();
}
...
void SB_Imap::init() {
    ...
    ipp_HT = new SB_ML_Type *[iv_buckets + 1];
    for (lv_hash = 0; lv_hash <= iv_buckets; lv_hash++)
        ipp_HT[lv_hash] = NULL;
...
void SB_Imap::resize(int pv_buckets) {
    int          lv_bucketsn;
    int          lv_bucketso;
    int          lv_hashn;
    int          lv_hasho;
    ...
    if (lv_bucketsn <= iv_buckets)
        return; // forget it
    lpp_HTn = new SB_ML_Type *[lv_bucketsn + 1];
    ...
    for (lv_hasho = 0; lv_hasho < lv_bucketso; lv_hasho++) {
        for (;;) {
            ...
            lv_hashn = hash(lp_item->iv_id.i, iv_buckets);
