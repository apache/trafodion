 ...
 * Region Server side via the TrxRegionService. This class will implement the
 * supporting functionality for summing/processing the individual results
 * obtained from the TrxRegionService for each region.
 * <p>
...
public class TransactionalAggregationClient {
  ...
  R max(final long transactionId, final long startId, final TransactionalTable table, final ColumnInterpreter<R, S, P, Q, T> ci,
    ...
    table.coprocessorService(TrxRegionService.class, scan.getStartRow(), scan.getStopRow(),
        new Batch.Call<TrxRegionService, R>() {
          @Override
          public R call(TrxRegionService instance) throws IOException {
            ServerRpcController controller = new ServerRpcController();
            BlockingRpcCallback<TransactionalAggregateResponse> rpcCallback = 
                new BlockingRpcCallback<TransactionalAggregateResponse>();
  ...
  S sum(final long transactionId, final long startId, final TransactionalTable table, final ColumnInterpreter<R, S, P, Q, T> ci,
    ...
    table.coprocessorService(TrxRegionService.class, scan.getStartRow(), scan.getStopRow(),
        new Batch.Call<TrxRegionService, S>() {
          @Override
          public S call(TrxRegionService instance) throws IOException {
            ServerRpcController controller = new ServerRpcController();
            BlockingRpcCallback<TransactionalAggregateResponse> rpcCallback = 
                new BlockingRpcCallback<TransactionalAggregateResponse>();
