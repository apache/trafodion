                                      ...
                                      , _TM_Txid_External trans_id )
{
    ...
    while( notice )
    {
        // This process no longer wants to be notified of death of process
        // identified by nid, pid, verifier
        if ((( notice->Nid == nid    ) && 
             ( notice->Pid == pid    ) && 
...
void CProcessContainer::Child_Exit ( CProcess * parent )
{
    ...
    TRACE_ENTRY;
    if (trace_settings & (TRACE_SYNC | TRACE_REQUEST | TRACE_PROCESS))
        trace_printf( "%s@%d for parent (%d, %d:%d)\n"
                    , method_name, __LINE__
                    , parent->GetNid(), parent->GetPid(), parent->GetVerifier() );
           ...
           || (parent->GetType() == ProcessType_SPX) ) )
    {
        ...
        while ( parent->childRemoveFirst ( child ))
        {
            ...
            if ( process && (!process->IsUnhooked()) )
            {
                ...
                if (trace_settings & (TRACE_SYNC | TRACE_REQUEST | TRACE_PROCESS))
                    trace_printf("%s@%d - Child process %s (%d, %d:%d) exits due "
