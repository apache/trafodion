...
CExtNameServerDownReq::CExtNameServerDownReq (reqQueueMsg_t msgType, int pid,
...
CExtNameServerDownReq::~CExtNameServerDownReq()
{
    // Alter eyecatcher sequence as a debugging aid to identify deleted object
    memcpy(&eyecatcher_, "rQee", 4);
}

void CExtNameServerDownReq::populateRequestString( void )
{
...
void CExtNameServerDownReq::performRequest()
{
    ...
    if (sonar_verify_state(SONAR_ENABLED | SONAR_MONITOR_ENABLED))
       MonStats->req_type_nameserverdown_Incr();
       
    // Trace info about request
    if (trace_settings & (TRACE_REQUEST | TRACE_PROCESS))
    {
        trace_printf("%s@%d request #%ld: NameServerDown, nid=%d\n",  method_name,
    ...
    if ( requester )
    {
        ...
        Monitor->HardNameServerDown( node->GetPNid(), true );
