...
public class TrafExportSnapshot extends Configured implements Tool {
  private static final Log LOG = LogFactory.getLog(TrafExportSnapshot.class);
  ...
  public int run(String[] args) throws IOException {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (cmd.equals("-snapshot")) {
        ...
        overwrite = true;
      } else if (cmd.equals("-mr-lowlimit-mb")) {
    		mrLowLimitMB = Integer.parseInt(args[++i]);
    ...
    for (Pair<SnapshotFileInfo, Long> fileInfo: snapshotFiles) {
    	snapshotTotalDataSize += fileInfo.getSecond();
    ...
    try {
    	if ((snapshotTotalDataSize/(1024*1024) > mrLowLimitMB) || hasWALfiles) {
    	    runCopyJob(inputRoot, outputRoot, snapshotName, snapshotDir, verifyChecksum,
    	     	...
    	     	LOG.info("Copy Snapshot Data Files");
    	     	for (Pair<SnapshotFileInfo, Long> fileInfo: snapshotFiles) {
   ...
   private Path getCopyOutputPath(final Configuration conf, final SnapshotFileInfo inputInfo) 
   throws IOException {
      Path path = null;
      Path outputRoot = new Path(conf.get(CONF_OUTPUT_ROOT));
      Path outputArchive = new Path(outputRoot, HConstants.HFILE_ARCHIVE_DIRECTORY);
      switch (inputInfo.getType()) {
        case HFILE:
          Path inputPath = new Path(inputInfo.getHfile());
          String family = inputPath.getParent().getName();
