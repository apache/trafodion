...
then
   echo "***[ERROR]: the number of snapshots in backup loaction ${hdfs_backup_location} " | tee -a $log_file
...
do
  ...
  if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
      echo "****[ERROR]: Error encoutered while importing snapshot ${snapshot_name}." | tee -a $log_file
...
if [[ $? -eq 0 ]]
then
    echo "****[ERROR]: Error encoutered while restoring snapshots"   | tee -a $log_file
    echo "More information can be found in the log file ${log_file}." 
    exit 1;
fi
...
grep ERROR $tmp_log_file 
if [ $? -eq 0 ]; then
    echo '****[ERROR]: Error encoutered while deleting snapshots'    | tee -a $log_file
    exit 1;
fi
