...
enum HDRTYPE {
	...
	CLOSE_TCPIP_SESSION,
	WRITE_QUERY_SERVICE_REQUEST,
	READ_QUERY_SERVICE_RESPONSE
};

typedef struct {
	short	operation_id;
	IDL_long	dialogueId;
	IDL_unsigned_long total_length;
	IDL_unsigned_long cmp_length;
	IDL_char compress_ind;
	IDL_char compress_type;
	HDRTYPE	hdr_type;
	IDL_unsigned_long signature;
	IDL_unsigned_long version;
	IDL_char	platform;
	IDL_char	transport;
	IDL_char	swap;
	IDL_short	error;
	IDL_short	error_detail;
} 
HEADER;
...
private:
	CError* list;
	CRITICAL_SECTION2 m_ErrorCSObject;
};
...
void WINAPI
SET_ERROR(long signature, char platform, char transport, int api, ERROR_TYPE error_type, char* process, OPERATION operation, FUNCTION function, int error, int errordetail, char* error_text=NULL);
void WINAPI
