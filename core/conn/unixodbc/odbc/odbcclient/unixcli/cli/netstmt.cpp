...
SQLRETURN SQLPREPARE_(SRVR_CALL_CONTEXT *srvrCallContext)
{
	...
	{
		if (sqlStmtQueryType == 10001 && pConnection->userStreams == 0)
		{
           pStatement->setDiagRec(DRIVER_ERROR, IDS_01_000, sts, "PLANLOADTABLE not supported for single segment unless streams per segment > 1 .");
           pConnection->userStreams = 0;
           return SQL_SUCCESS_WITH_INFO;
		}
		else if (sqlStmtQueryType == 10001)
...
SQLRETURN SQLFETCH_(SRVR_CALL_CONTEXT *srvrCallContext)
{
    ...
    // main thread has to acquire a lock on m_CSTransmision->Lock inorder to reach here
	// the prefetch thread also tries to acquire a lock m_CSTransmision->Lock
	// so if we're here, the prefetch thread state can only be one of
	// - PREFETCH_STATE_WRK_UNASSIGNED, i.e. this is the first fetch
	// - PREFETCH_STATE_WRK_ASSIGNED, i.e the prefetch thread was supposed to go and prefetch, but probably never got scheduled to run by the CPU
	// - PREFETCH_STATE_WRK_DONE, i.e. the pre fetch thread has completed prefetching. 
	// we can check for prefetch results in the last case

	if (gDrvrGlobal.gDisablePreFetch == false && (pConnection->m_preFetchThread->getState() == PREFETCH_STATE_WRK_DONE) &&
	    pStatement == pConnection->m_preFetchThread->getStmtHandle())
	{
		sts = pConnection->m_preFetchThread->sts; 
		returnCode = pConnection->m_preFetchThread->returnCode;
		sqlWarningOrError = pConnection->m_preFetchThread->sqlWarningOrError;
		rowsReturned = pConnection->m_preFetchThread->rowsReturned;
		outValuesFormat = pConnection->m_preFetchThread->outValuesFormat;
		outputDataValue._length = pConnection->m_preFetchThread->outputDataValue._length;
		outputDataValue._buffer = pConnection->m_preFetchThread->outputDataValue._buffer;

        //reset
		pConnection->m_preFetchThread->initializeResults();
	...
	UDWORD_P timerTimeout = srvrCallContext->u.fetchParams.queryTimeout;
		...
		else	// With RWRS, only catalog API data takes this path
		{
			while (Index < DataValue->_length)
			{
				...
				if (SQLDataInd == 0)
				{
					...
					Index = Index + dataLengthFetchPerf(SQLDataType, SQLOctetLength, maxLength, SQLDataValue);
				}
				Index = Index + 1;
				Items++;
			}
	...
	{
	    if(pConnection->m_preFetchThread->getStmtHandle() == NULL)
		{
	       /* check that the application is not trying to select for update of (in which case it will set SQL_CONCUR_READ_ONLY */
		   if(pStatement->m_Concurrency == SQL_CONCUR_READ_ONLY && pStatement->getAsyncEnable() != SQL_ASYNC_ENABLE_ON )
		   {
		 	   if ( returnCode == SQL_SUCCESS_WITH_INFO || returnCode == SQL_SUCCESS )
			   {
			      pConnection->m_preFetchThread->setStmtHandle(pStatement);
			      pConnection->m_preFetchThread->setSrvrCallContext(srvrCallContext);
			      pConnection->m_preFetchThread->assignTask();
			      pConnection->m_preFetchThread->SignalCondition();
			   }
		   }
		}
		else
		{
		    if(pConnection->m_preFetchThread->getStmtHandle() == pStatement)
			{
				if ( !(returnCode == SQL_SUCCESS_WITH_INFO || returnCode == SQL_SUCCESS) )
				{
				    if(pConnection->m_preFetchThread->getState() == PREFETCH_STATE_WRK_ASSIGNED)
			  		   pConnection->m_preFetchThread->cancelTask();
				}
			}
		}
	}
...
SQLRETURN FREESTATEMENT(SRVR_CALL_CONTEXT *srvrCallContext)
{
    ...
    if(gDrvrGlobal.gDisablePreFetch == false)
	{
    if(pStatement == pConnection->m_preFetchThread->getStmtHandle())
	{
	    if(pConnection->m_preFetchThread->getState() == PREFETCH_STATE_WRK_ASSIGNED)
		   // pre fetch task was not started
  		   pConnection->m_preFetchThread->cancelTask();
		else
		{
...
{
   return checkNetStmt(srvrCallContext, CEE_SUCCESS,"UNKNOWN");
}
