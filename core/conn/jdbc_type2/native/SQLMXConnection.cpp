...
void setConnectAttr(JNIEnv *jenv, jobject jobj, jstring server, long dialogueId, short attribute,
					SQLUINTEGER valueNum, const char *valuePtr)
{
	FUNCTION_ENTRY("setConnectAttr",("jenv=0x%08x, server=%s, dialogueId=0x%08x, attribute=%d, valueNum=%ld, valuePtr=0x%08x",
		jenv,
...
(JNIEnv *jenv, jobject jobj, jstring server, jstring uid, jstring pwd)
{
	...
	default:
		...
		FUNCTION_RETURN_NUMERIC(0,("jdbcConnect->sqlConnect() returned %s",
			CliDebugSqlError(rc)));
	}
...
(JNIEnv *jenv, jclass jcls, jstring server, jlong dialogueId, jint charset, jstring charsetOverride)
{
	...
	FUNCTION_ENTRY("Java_org_trafodion_jdbc_t2_SQLMXConnection_getCharsetEncoding",
		...
		dialogueId,
		charset,
		DebugJString(jenv,charsetOverride)));

	// Duplicating logic for current code.  Will probably change.
	if (charset == SQLCHARSETCODE_UNKNOWN) FUNCTION_RETURN_PTR(NULL,("SQLCHARSETCODE_UNKNOWN returning null"));
