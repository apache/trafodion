...
{
    public class MVsSpaceUsageSummaryControl : SpaceUsageSummaryUserControl, ICloneToWindow
    {
        protected override string ObjectType
        {
            get { return Properties.Resources.MaterializedViews; }
        }
        public override string HeaderText
        {
            get { return "SQL Space Usage of {0} " + Properties.Resources.MaterializedViews; }
        }
        protected override string PersistenceKey
        {
            get { return "MVsSpaceUsageSummaryPersistence"; }
        }
        ...
        protected override IDataDisplayHandler TheDataDisplayHandler
        {
            get
            {
                ...
                {
                    _dataDisplayHandler = new MVsSpaceUsageSummaryDataHandler(this);
                }
        ...
        protected override string HelpTopicName
        {
            get
            {
                return HelpTopics.MVsSpaceUsage;
            }
        }

        public MVsSpaceUsageSummaryControl(DatabaseObjectsControl aDatabaseObjectsControl, TrafodionSchema aTrafodionSchema)
            : base(aDatabaseObjectsControl, aTrafodionSchema)
        {
            //SQLText = Model.Queries.GetMVSpaceUsageSummaryQueryString(aTrafodionSchema, false);
            SQLText = Model.Queries.GetTablesOrMVsSpaceUsageSummaryQueryString(aTrafodionSchema, "MVS", TrafodionMaterializedView.ObjectType, TrafodionMaterializedView.ObjectNameSpace, false);
        }

        public MVsSpaceUsageSummaryControl(MVsSpaceUsageSummaryControl clone)
        ...
        {
            string mvName = TheDataGrid.Rows[rowIndex].Cells[colIndex].Value as string;
            if (!string.IsNullOrEmpty(mvName))
            {
                string internalName = TrafodionName.InternalForm(mvName).Trim();
                TrafodionMaterializedView mv = ((TrafodionSchema)_theTrafodionObject).FindMaterializedView(internalName);
                if (mv != null && TheDatabaseObjectsControl != null)
                {
                    TheDatabaseObjectsControl.TheDatabaseTreeView.SelectTrafodionObject(mv);
                }
            }
        }
        ...
        override protected void RefetchFromRepository()
        {
            ...
            //dbConfig.SQLText = Model.Queries.GetMVSpaceUsageSummaryQueryString((TrafodionSchema)_theTrafodionObject, false);
            dbConfig.SQLText = Model.Queries.GetTablesOrMVsSpaceUsageSummaryQueryString((TrafodionSchema)_theTrafodionObject, "MVS", TrafodionMaterializedView.ObjectType, TrafodionMaterializedView.ObjectNameSpace, false); _theWidget.StartDataProvider();
        }
        ...
        override protected void RefetchLive()
        {
            _theWidget.DataProvider.Stop();
            DatabaseDataProviderConfig dbConfig = _theWidget.DataProvider.DataProviderConfig as DatabaseDataProviderConfig;
            //dbConfig.SQLText = Model.Queries.GetMVSpaceUsageSummaryQueryString((TrafodionSchema)_theTrafodionObject, true);
            dbConfig.SQLText = Model.Queries.GetTablesOrMVsSpaceUsageSummaryQueryString((TrafodionSchema)_theTrafodionObject, "MVS", TrafodionMaterializedView.ObjectType, TrafodionMaterializedView.ObjectNameSpace, true); _theWidget.StartDataProvider();
        ...
        {
            return new MVsSpaceUsageSummaryControl(this);
        }
    ...
    #region Class MVsSpaceUsageSummaryDataHandler

    public class MVsSpaceUsageSummaryDataHandler : TabularDataDisplayHandler
    {
        #region Fields

        private MVsSpaceUsageSummaryControl m_caller = null;
        ...
        public MVsSpaceUsageSummaryDataHandler(MVsSpaceUsageSummaryControl caller)
        {
            m_caller = caller;
        }
    ...
    #endregion Class MVsSpaceUsageSummaryDataHandler
}
