...
{
    /// <summary>
    /// A model object holds the sampled statistics for a table column
    /// </summary>
    public class TableColumnSampledStats : TableColumnStats
    {
        ...
        /// <summary>
        /// Constructs the sampled statistics model for the sql table column
        /// </summary>
        /// <param name="aTrafodionTableColumn"></param>
        public TableColumnSampledStats(TrafodionTableColumn aTrafodionTableColumn)
            :base(aTrafodionTableColumn)
        {
        }
        ...
        {
            string sampleClause = "";
            if (_sqlMxTableColumn.TrafodionTable.RowCount >= SAMPLING_THRESHOLD)
            {
                //If number of rows in table is >= SAMPLING_THRESHOLD, compute the sample percent
                _sampledPercent = SAMPLING_THRESHOLD / _sqlMxTableColumn.TrafodionTable.RowCount;
        ...
        public void ComputeStatsUsingSample()
        {
            ...
            try
            {
                connection = _sqlMxTableColumn.GetConnection();
                ...
                int result = Queries.ExecuteCreateTempStatsTable(aOdbcConnection, tempTableName, _sqlMxTableColumn.TrafodionTable, ColumnName, GetSampleClause());
                ...
                if (result >= 0)
                {
                    ...
                    if (odbcDataReader.Read())
                    {
                        ...
                        if(_sqlMxTableColumn.TrafodionTable.Partitions.Count > 1)
                    ...
                    odbcDataReader = Queries.ExecuteSelectTableColumnStatIntervals(aOdbcConnection, _sqlMxTableColumn.TrafodionTable, _sqlMxTableColumn.TheColumnNumber);
