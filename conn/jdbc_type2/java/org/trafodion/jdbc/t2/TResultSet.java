...
public class TResultSet implements java.sql.ResultSet
{
	...
	public org.trafodion.sql.ResultSetInfo getResultSetInfo() throws SQLException
	{
		if (out_ != null)
			out_.println(getTraceId() + "getResultSetInfo()");
		org.trafodion.sql.ResultSetInfo retValue;
		
		retValue = ((org.trafodion.sql.SQLMXResultSet)rs_).getResultSetInfo();
		return retValue;
	}
	...
	TResultSet(ResultSet rs, PrintWriter out)
	{
		...
		setTraceId(org.trafodion.sql.T2Driver.traceText
			+ org.trafodion.sql.T2Driver.dateFormat.format(new Date()) 
			+ "]:[" + Thread.currentThread() + "]:["
			+ System.identityHashCode(rs_) +  "]:"
			+ rs_.getClass().getName().substring(org.trafodion.sql.T2Driver.REMOVE_PKG_NAME,rs_.getClass().getName().length()) 
			+ ".");
	}
	...
	public String getTraceId() {
		...
		setTraceId(org.trafodion.sql.T2Driver.traceText
			...
			+ "]:[" + Thread.currentThread() + "]:["
			+ System.identityHashCode(rs_) +  "]:"
			+ className.substring(org.trafodion.sql.T2Driver.REMOVE_PKG_NAME,className.length()) 
			+ ".");
		return traceId_;
	}
