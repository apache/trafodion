...
import org.trafodion.wms.server.store.WorkloadThresholds;
...
public final class ScriptManager {
	...
	private static final String DEFAULT_SCRIPT_NAME = "hp_default_service" + PYTHON_SUFFIX;
	private static String wmsHome = null;
	...
	private ScriptManager() {
		...
		//Get -Dwms.home.dir
		wmsHome = System.getProperty("wms.home.dir");	
		
		//Start the scripts directory watcher
		watcherWorker = new ScriptManagerWatcher ("ScriptManagerWatcher",wmsHome + "/bin/scripts");
	}
	
	public synchronized void runScript(Request request, Response response) {
		String scriptName;
		
//		if(name.length() == 0)
			scriptName = DEFAULT_SCRIPT_NAME;
//		else if(! name.endsWith(".py"))
//			scriptName = name + PYTHON_SUFFIX;
//		else
//			scriptName = name;			
		...
		try {
			...
			if(engine instanceof Compilable) {
				...
				if(script == null) {
					...
					try {
						script = compilingEngine.compile(new FileReader("bin/scripts/" + scriptName));
					} catch (Exception e) {
				...
				try {
					engine.eval(new FileReader("bin/scripts/" + scriptName), bindings);
				} catch (Exception e) {
					LOG.warn(e.getMessage());
				}
			}
