...
void do_sync() {
    int len = (int) strlen(sync_data) + 1; // cast
    if (!quiet)
        printf("server (%s) about to issue sync nid=%d, len=%d, tag=%d\n",
               pname, snid, len, tag);
    int ferr = msg_mon_tmsync_issue((char *) sync_data, len, &p1handle, tag);
    util_check("msg_mon_tmsync_issue", ferr);
    Item *item = new Item();
    item->link.iv_id.i = p1handle;
    // uncomment out the following to cause problems
    // util_time_sleep_ms(100);
    handle_map.put(&item->link);
    if (!quiet)
...
int tmsync_cb(void *data, int len, int handle) {
    ...
    assert(len == (int) (strlen(sync_data) + 1));
    assert(strcmp((char *) data, sync_data) == 0);
    sync_rcvd = true;
    p1handle = handle;
    return 0;
}
            ...
            for (;;) {
                ...
                if (sre.sre_flags & XSRE_MON) {
                    ...
                    case MS_MsgType_TmSyncCommit:
                        ...
                        p2handle = msg->u.tmsync.handle[0];
                        item = (Item *) handle_map.remove(p2handle);
                        ...
                        if (!quiet)
                            printf("server (%s) received (%s) mon message, type=%s(%d), nid=%d, orig_count=%d, orig_tag=%d, orig_handle=%d, count=%d, handle[0]=%d\n",
                               pname,
                               client_mon ? "cli" : "dtm",
                               type,
                               msg->type,
                        ...
                        if (nid == snid) {
                            if (!quiet)
                                printf("server (%s) commit handle=%d\n",
                                       pname, p2handle);
                            assert(msg->u.tmsync.orig_count == 1);
                            assert(orig_tag == tag);
                            assert(item != NULL);
                            delete item;
                        } else {
                    ...
                    if ((p1handle != -1) && (p2handle != -1)) {
                        assert(p1handle == p2handle);
                        assert(commit_rcvd);
                    }
                    assert(open_rcvd);
