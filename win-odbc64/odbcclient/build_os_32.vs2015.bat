...
set ZLIB_INCLUDE_PATH=C:\zlib32_128\include
REM set this to zlib library files directory
set ZLIB_LIB_PATH=C:\zlib32_128\lib

REM set this to openssl header files directory
set OPENSSL_INCLUDE_PATH="C:\Temp\openssl32_101es\include"
REM set this to openssl library files directory
set OPENSSL_LIB_PATH="C:\Temp\openssl32_101es\lib"
...
msbuild.exe /t:rebuild Drvr35Msg_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release
...
msbuild.exe /t:rebuild Drvr35_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release /p:OpenSSLIncludeDir=%OPENSSL_INCLUDE_PATH% /p:OpenSSLLibraryDir=%OPENSSL_LIB_PATH% /p:ZlibIncludeDir=%ZLIB_INCLUDE_PATH% /p:ZlibLibDir=%ZLIB_LIB_PATH%
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
msbuild.exe /t:rebuild Drvr35Adm_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
REM msbuild.exe /t:rebuild Drvr35Trace_os.vs2015.vcxproj /p:Platform=x64 /p:Configuration=Release
...
msbuild.exe /t:rebuild TCPIPV4_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release /p:ZlibIncludeDir=%ZLIB_INCLUDE_PATH% /p:ZlibLibDir=%ZLIB_LIB_PATH%
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
msbuild.exe /t:rebuild TCPIPV6_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release /p:ZlibIncludeDir=%ZLIB_INCLUDE_PATH% /p:ZlibLibDir=%ZLIB_LIB_PATH%
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
msbuild.exe /t:rebuild TranslationDll_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
msbuild.exe /t:rebuild Drvr35Res_os.vs2015.vcxproj /p:Platform=Win32 /p:Configuration=Release
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
msbuild.exe /t:rebuild SetCertificateDirReg_os.vcxproj /p:Platform=Win32 /p:Configuration=Release
set BUILD_STATUS=%ERRORLEVEL%
if %BUILD_STATUS%==0 (
...
ISCC.exe /Q %BUILDDIR%\win-odbc64\Install\win32_installer\installer.iss
copy /Y %BUILDDIR%\win-odbc64\Install\win32_installer\Output\TFODBC32-2.2.0.exe %PACKDIR%
@echo on

if exist %PACKDIR%\TFODBC32-2.2.0.exe (
	set ALL_SUCCESS=1
)
...
if %ALL_SUCCESS%==1 (
	echo=
	echo ========================================
	echo     BUILD WIN32 RELEASE SUCCESSFULLY
	echo ========================================
	echo=
)
