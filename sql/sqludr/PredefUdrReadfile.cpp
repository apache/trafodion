...
//
//  select * from udf(log_reader());
//
// Returned columns:
//
// log_ts       timestamp(0) not null,
// severity     char(10 bytes) character set utf8 not null,
     ...
     SQLUDR_TMUDF_TRAIL_ARGS)
{
  ...
  // null indicators
  SQLUDR_INT16 *log_ts_param_ind       = (SQLUDR_INT16 *) &(output_row[udrinfo->return_values[2].ind_offset]);
  SQLUDR_INT16 *severity_param_ind     = (SQLUDR_INT16 *) &(output_row[udrinfo->return_values[2].ind_offset]);
  ...
  if (doTrace)
  {
    printf("(%d) TRAF_CPP_EVENT_LOG_READER calltype %d input %p\n", pid,
  ...
  if (doTrace)
    {
      printf("(%d) TRAF_CPP_EVENT_LOG_READER open log dir %s\n", pid, logDirName.data());
      fflush(stdout);
    }
  ...
  while (1)
  {
    ...
    if (doTrace)
      {
        printf("(%d) TRAF_CPP_EVENT_LOG_READER examining log file %s\n", pid, dirEntry->d_name);
        fflush(stdout);
      }

    // parse the file name to see whether this is a file we want to look at
    // right now, files we want to see are mxosrvr_*.log
    if (strstr(dirEntry->d_name, "mxosrvr_") == dirEntry->d_name &&
        strcmp(strstr(dirEntry->d_name, ".log"), ".log") == 0)
      {
        ...
        if (doTrace)
          {
            printf("(%d) TRAF_CPP_EVENT_LOG_READER fopen\n", pid);
            fflush(stdout);
          }
        ...
        // for now, put some dummy data into the output columns
        memcpy(log_ts_param_ptr, "2014-01-01 00:00:00", 19);
        memcpy(severity_param_ptr, "DEBUGGING ", 10);
        memset(component_param_ptr, ' ', 24);
        *node_number_param_ind
          ...
          = *query_id_param_ind
          = -1;
        *message_param_len = strlen(dirEntry->d_name);
        memcpy(message_param_ptr, dirEntry->d_name, *message_param_len);
     ...
     UDRInvocationInfo &info)
{
  ...
  if (info.getNumActualParameters() != 0)
    throw UDRException(38221,
                       "There should be no input parameters to the call to %s, got %d",
                       info.getUDRName().data(),
  ...
  outTable.addColumn(
       ColumnInfo("LOG_TS",
                  TypeInfo(TypeInfo::TIMESTAMP,
                           ...
                           false,
                           0)));
  outTable.addCharColumn   ("SEVERITY",    10);
...
// ---------------------------------------------------------
// older code, can probably be deleted later
// ---------------------------------------------------------
