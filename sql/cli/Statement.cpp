...
RETCODE Statement::checkSchemaFileLabelTS(ComDiagsArea &diagsArea)
{

  char localNodeName[9];
  short nodeNameLength;
  char schemaFileLabelName[36];
  char *schemaSurrogateTableNamePtr;

  RETCODE retcode;
  ComDiagsArea *localDiags;

  NAHeap rtmdHeap (RTMD_HEAP_STR,
    stmtHeap(),
    RTMD_BLOCKSIZE,
    RTMD_UPPER_LIMIT);

      if( NODENUMBER_TO_NODENAME_
          (-1,                // local node
  ...
  SchemaLabelInfoPtr stmtSLI;
  for(short schIdx = 0; schIdx < root_tdb->getSchCount(); schIdx++)
  {
    stmtSLI = root_tdb->schemaLabelInfoList()[schIdx];
    schemaSurrogateTableNamePtr = stmtSLI->schemaLabelName();

    if(*schemaSurrogateTableNamePtr == '$')
    {
      str_sprintf(schemaFileLabelName, "%s.%s",
        localNodeName, schemaSurrogateTableNamePtr);
    }
    else
    {
      if(*schemaSurrogateTableNamePtr == '\\')
      {
        str_cpy_all(schemaFileLabelName, schemaSurrogateTableNamePtr,
          str_len(schemaSurrogateTableNamePtr));
      }
      else
      {
        // Invalid schema surrogate table name.
        return(ERROR);
      }
    }
    ...
    if (&diagsArea)
      localDiags = (&diagsArea)->copy() ;
    else
    {
      return(ERROR);
    }

    // Get Security Timestamp from schema file label.
    retcode = ERROR;
    // (&diagsArea)->clear();
    (&diagsArea)->mergeAfter (*localDiags);
    ...
    if (retcode == ERROR)
    {
      if (root_tdb->aqrEnabled())
	{
	  // timestamp has changed. Add timestamp_mismatch error code to diags
	  // area so this query could be AQR'ed.

        char tmpSchName[512], tmpTSo[24], tmpTSn[24];

        memset(tmpSchName, 512, 0);
        memset(tmpTSo, 24, 0);
        memset(tmpTSn, 24, 0);

        sprintf(tmpSchName, "%s (%s)", stmtSLI->schemaAnsiName()
                                     , stmtSLI->schemaLabelName());
        convertInt64ToAscii(stmtSLI->schemaLabelRedefTS(), tmpTSo);
        diagsArea << DgSqlCode(-EXE_SCHEMA_SECURITY_CHANGED)
          << DgString0(tmpSchName) << DgString1(tmpTSo) << DgString2(tmpTSn);
