...
// -----------------------------------------------------------------------
// Methods for CmpMessageReadTableDef
// -----------------------------------------------------------------------
CmpReadTableDefInfo::CmpReadTableDefInfo(char *objName, 
                                         ObjNameTypeEnum nameType,
                                         RTDRequestTypeEnum requestType
                                        )
     : objName_(objName), nameType_(nameType), requestType_(requestType)
{
}

Lng32 CmpReadTableDefInfo::getLength()
{
  return ROUND8(sizeof(*this)) + ROUND8(str_len(objName_) + 1);
}

void CmpReadTableDefInfo::pack(char * buffer)
{
  Lng32 nextOffset = 0;
  CmpReadTableDefInfo* rtd = (CmpReadTableDefInfo *)buffer;
  ...
  if (objName_)
    {
      str_cpy_all(&buffer[nextOffset], objName_, str_len(objName_));
      buffer[nextOffset + str_len(objName_)] = '\0';
      rtd->objName_ = (char *)nextOffset;
      nextOffset += ROUND8(str_len(objName_) + 1);
    }
}

void CmpReadTableDefInfo::unpack(char * base)
{
  if (objName_)
    {
      objName_ = base + (Lng32)(Long)objName_;
    }
}

// -----------------------------------------------------------------------
// Methods for CmpMessageReadTableDefReply, sent back in the reply data
// to the application/caller of the CLI.
//------------------------------------------------------------------------
CmpMessageReadTableDefReply::CmpMessageReadTableDefReply(char *ansiName,
                                                         Lng32 ansiNameLen,
                                                         char *mpName,
                                                         Lng32 mpNameLen,
                                                         char *catalogName,
                                                         Lng32 catalogNameLen,
                                                         char* tableDesc,
                                                         Lng32 descLen,
                                                         CollHeap *h
                                                        ) 
 : ansiName_(ansiName), mpName_(mpName), catalogName_(catalogName), 
   catalogNameLen_(catalogNameLen), ansiNameLen_(ansiNameLen), 
   mpNameLen_(mpNameLen), objDesc_(tableDesc), descLen_(descLen), h_(h)
{
}

Lng32 CmpMessageReadTableDefReply::getLength()
{
  return ROUND8(sizeof(*this)) + 
         ROUND8(ansiNameLen_ + 1) +
         ROUND8(mpNameLen_ + 1) +
         ROUND8(catalogNameLen_ + 1) +
         ROUND8(descLen_);
          
}

void CmpMessageReadTableDefReply::pack(char * buffer)
{
  Lng32 nextOffset = 0;
  CmpMessageReadTableDefReply* rtdReply = (CmpMessageReadTableDefReply *)buffer;
  ...
  if (ansiName_)
    {
      str_cpy_all(&buffer[nextOffset], ansiName_, ansiNameLen_);
      buffer[nextOffset + ansiNameLen_] = '\0';
      rtdReply->ansiName_ = (char *)nextOffset;
      nextOffset += ROUND8(ansiNameLen_ + 1);
    }
  if (mpName_)
    {
      str_cpy_all(&buffer[nextOffset], mpName_, mpNameLen_);
      buffer[nextOffset + mpNameLen_] = '\0';
      rtdReply->mpName_ = (char *)nextOffset;
      nextOffset += ROUND8(mpNameLen_ + 1);
    }
  if (catalogName_)
    {
      str_cpy_all(&buffer[nextOffset], catalogName_, catalogNameLen_);
      buffer[nextOffset + catalogNameLen_] = '\0';
      rtdReply->catalogName_ = (char *)nextOffset;
      nextOffset += ROUND8(catalogNameLen_ + 1);
    }
  if (objDesc_)
    {
      str_cpy_all(&buffer[nextOffset], objDesc_, descLen_);
      rtdReply->objDesc_ = (char *)nextOffset;
      nextOffset += ROUND8(descLen_);
    }
}

void CmpMessageReadTableDefReply::unpack(char * base)
{
  if (ansiName_)
    {
      ansiName_ = base + (Lng32)(Long)ansiName_;
    }
  if (mpName_)
    {
      mpName_ = base + (Lng32)(Long)mpName_;
    }
  if (catalogName_)
    {
      catalogName_ = base + (Lng32)(Long)catalogName_;
    }
  if (objDesc_)
    {
      objDesc_ = base + (Lng32)(Long)objDesc_;
    }
}
