...
{
  { "UTIL_OUTPUT",       0, COM_USER_COLUMN_LIT, REC_BYTE_V_ASCII,     2000, FALSE, SQLCHARSETCODE_UTF8 , 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL, COM_UNKNOWN_DIRECTION_LIT, 0}
};
...
{
  { "EXPLAIN OUTPUT",     0,             COM_USER_COLUMN_LIT, REC_BYTE_V_ASCII,     4000, FALSE, SQLCHARSETCODE_UTF8, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL, COM_UNKNOWN_DIRECTION_LIT, 0 }
};

static const ComTdbVirtTableColumnInfo exeUtilDisplayExplainVirtTableOptionXColumnInfo[] =
{
  { "EXPLAIN OUTPUT(FORMATTED)",   0,    COM_USER_COLUMN_LIT, REC_BYTE_V_ASCII,     79, FALSE, SQLCHARSETCODE_UTF8, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL, COM_UNKNOWN_DIRECTION_LIT, 0  }
};
...
{
  { "GET_UID_OUTPUT",   0,    COM_USER_COLUMN_LIT, REC_BIN64_SIGNED,     8, FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0}
};
...
class ComTdbExeUtilUserLoad : public ComTdbExeUtil
{
  friend class ExExeUtilUserLoadFastTcb;
public:
  ComTdbExeUtilUserLoad()
  : ComTdbExeUtil()
  {}

  ComTdbExeUtilUserLoad(char * tableName,
			ULng32 tableNameLen,
			ex_expr * input_expr,
			ULng32 input_rowlen,
  ...
  ex_expr * rwrsInputSizeExpr()      { return rwrsInputSizeExpr_; }
  ex_expr * rwrsMaxInputRowlenExpr() { return rwrsMaxInputRowlenExpr_;}
  ex_expr * rwrsBufferAddrExpr()     { return rwrsBufferAddrExpr_;}

  char * excpTabInsertStmt() { return excpTabInsertStmt_; }

  unsigned short childTuppIndex() { return childTuppIndex_; }

  // ---------------------------------------------------------------------
  // Redefine virtual functions required for Versioning.
  //----------------------------------------------------------------------
  virtual short getClassSize() {return (short)sizeof(ComTdbExeUtilUserLoad);}
  ...
  virtual Int32 numExpressions() const
  {
    return ComTdbExeUtil::numExpressions() + 4;
  }
  ...
  virtual ex_expr * getExpressionNode(Int32 pos)
  {
    if (pos < ComTdbExeUtil::numExpressions())
      return ComTdbExeUtil::getExpressionNode(pos);
    else if (pos == ComTdbExeUtil::numExpressions())
      return rwrsInputSizeExpr();
    else if (pos == ComTdbExeUtil::numExpressions()+1)
      return rwrsMaxInputRowlenExpr();
    else if (pos == ComTdbExeUtil::numExpressions()+2)
      return rwrsBufferAddrExpr();
    else if (pos == ComTdbExeUtil::numExpressions()+3)
      return excpRowExpr_;
    else
      return(NULL);
  }
  ...
  void setNoAlter(NABoolean v)
  {(v ? flags_ |= NO_ALTER : flags_ &= ~NO_ALTER); };
  NABoolean noAlter() { return (flags_ & NO_ALTER) != 0; };
 ...
 enum
  {
    NO_ALTER             = 0x0001,
    DO_FAST_LOAD         = 0x0002,
    ROWWISE_ROWSET       = 0x0004,
    USER_SIDETREE_INSERT = 0x0008,
    SORT_FROM_TOP        = 0x0010
  };

  ExExprPtr rwrsInputSizeExpr_;                       // 00-07
  ExExprPtr rwrsMaxInputRowlenExpr_;                  // 08-15
  ExExprPtr rwrsBufferAddrExpr_;                      // 16-23

  NABasicPtr excpTabInsertStmt_;                      // 24-31

  UInt32 flags_;                                      // 32-35

  unsigned short childTuppIndex_;                     // 36-37

  unsigned short excpRowAtpIndex_;                    // 38-39
  ExExprPtr excpRowExpr_;                             // 40-47
  UInt32 excpRowLen_;                                 // 48-51
  Lng32 numTableCols_;                                 // 52-55

  Int64 loadId_;                                      // 56-63

  Lng32 excpRowsPercentage_;                           // 64-67
  Lng32 excpRowsNumber_;                               // 68-71

  char fillersComTdbExeUtilMaintainObject_[40];       // 72-111
};

class ComTdbExeUtilSidetreeInsert : public ComTdbExeUtil
{
  friend class ExExeUtilSidetreeInsertTcb;
  friend class ExExeUtilSidetreeInsertUnauditTcb;
public:
  ComTdbExeUtilSidetreeInsert()
  : ComTdbExeUtil()
  {}

  ComTdbExeUtilSidetreeInsert(char * tableName,
			      ULng32 tableNameLen,
			      char * originalQuery,
			      char * sidetreeQuery,
			      ex_cri_desc * work_cri_desc,
  ...
  // Redefine virtual functions required for Versioning.
  //----------------------------------------------------------------------
  virtual short getClassSize() {return (short)sizeof(ComTdbExeUtilSidetreeInsert);}

  virtual const char *getNodeName() const
  {
    return "FAST_INSERT_UTIL";
  };

  char * originalQuery() { return originalQuery_; }
  char * sidetreeQuery() { return sidetreeQuery_; }
  ...
  void setAlterAudit(NABoolean v)
  {(v ? flags_ |= ALTER_AUDIT : flags_ &= ~ALTER_AUDIT); };
  NABoolean alterAudit() { return (flags_ & ALTER_AUDIT) != 0; };

  void setDisplayWarnings(NABoolean v)
  {(v ? flags_ |= DISPLAY_WARNINGS : flags_ &= ~DISPLAY_WARNINGS); };
  NABoolean displayWarnings() { return (flags_ & DISPLAY_WARNINGS) != 0; };

private:
 enum
  {
    ALTER_AUDIT             = 0x0001,
    DISPLAY_WARNINGS        = 0x0002
  };

  NABasicPtr originalQuery_;                      // 00-07
  NABasicPtr sidetreeQuery_;                      // 08-15
  UInt32 flags_;                                  // 16-19

  char fillersComTdbExeUtilSidetreeInsert_[20];   // 20-39
};
...
{                                                                                     
  ...
  { "SCHEMA_NAME",      1,    COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "TABLE_NAME",          2,   COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "COLUMN_NAME",       3,  COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "SQL_DATA_TYPE",     4,  COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    24,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "FS_DATA_TYPE",        5,  COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,    4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "COLUMN_SIZE",         6,   COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,    4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "CHARACTER_SET",     7,   COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,     40,  FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0}, 
  { "COLUMN_PRECISION", 8,  COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,    4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "COLUMN_SCALE",       9,   COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,    4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "DT_CODE",                 10,  COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,    4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "NULLABLE",                11, COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,     4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},  
  { "COLUMN_NUMBER",    12,     COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,     4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0}, 
  { "DATETIME_QUALIFIER", 13,    COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    28,   FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "DATETIME_START_FIELD", 14,  COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,   4,   FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "DATETIME_END_FIELD",   15, COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,   4,   FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "DEFAULT_VALUE",        16, COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    240,   FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0} 
};
...
{                                                                                     
  ...
  { "TABLE_NAME",        2, COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "COLUMN_NAME",     3,    COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "ORDINAL_POSITION", 4,          COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,     4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0}
};
...
{                                                                                     
  ...
  { "TABLE_NAME",      2,   COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "COLUMN_NAME",    3,     COM_USER_COLUMN_LIT, REC_BYTE_F_ASCII,    256, FALSE , SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "" ,NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0},
  { "KEY_SEQ",      4,     COM_USER_COLUMN_LIT, REC_BIN32_SIGNED,     4,    FALSE, SQLCHARSETCODE_UNKNOWN, 0, 0, 0, 0, 0, 0, 0, COM_NO_DEFAULT, "",NULL,NULL,COM_UNKNOWN_DIRECTION_LIT, 0}
};
