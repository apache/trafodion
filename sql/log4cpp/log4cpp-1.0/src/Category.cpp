...
// added to fix deadlock/cores issues on SQ
                    HierarchyMaintainer::getDefaultMaintainer().eraseAppenderFromAllCategory(*i);
                    delete (*i);
                }
            }
   ...
   /* assume lock is held */
   void Category::eraseAppender(Appender* appender) {
       AppenderSet::iterator i = _appender.find(appender);
       if (_appender.end() != i) {
           OwnsAppenderMap::iterator i2;
           if (ownsAppender(*i, i2)) {
