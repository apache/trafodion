...
// -----------------------------------------------------------------------
// methods for class InterpretAsRow
// -----------------------------------------------------------------------
//! InterpretAsRow::InterpretAsRow Constructor 
InterpretAsRow::InterpretAsRow(ItemExpr *auditRowImage,
                               ItemExpr *mfMap,
                               AuditCompressionEnum compressedAudit,
                               const CorrName &objectName,
                               CollHeap *oHeap) :
    TableValuedFunction(NULL, REL_INTERPRET_AS_ROW, oHeap)
{
   objectName_ = objectName;
   auditRowImage_ = auditRowImage;
   modifiedFieldMap_ = mfMap;
   compressedAudit_ = compressedAudit;
}

//! InterpretAsRow::InterpretAsRow Constructor 
InterpretAsRow::InterpretAsRow(CollHeap *oHeap) :
   TableValuedFunction(NULL, REL_INTERPRET_AS_ROW, oHeap)
{
   auditRowImage_ = NULL;
   modifiedFieldMap_ = NULL;
   compressedAudit_ = UNCOMPRESSED_AUDIT;
}

//! InterpretAsRow::~InterpretAsRow Destructor 
InterpretAsRow::~InterpretAsRow() {}

const NAString InterpretAsRow::getText() const
{
   return "interpret_as_row";
}

//! InterpretAsRow::topHash method
HashValue InterpretAsRow::topHash()
{
   HashValue result = TableValuedFunction::topHash();

   result ^= getTableDesc();

   result ^= auditRowImageVid_;
...
//! InterpretAsRow::duplicateMatch method
NABoolean InterpretAsRow::duplicateMatch(const RelExpr &other) const
{
   if (NOT TableValuedFunction::duplicateMatch(other))
      return FALSE;

   InterpretAsRow &o = (InterpretAsRow &)other;
   if (NOT (objectName_ == o.objectName_) OR
       NOT (getTableDesc() == o.getTableDesc()) OR
       NOT (compressedAudit_ == o.compressedAudit_))
      return FALSE;

   return TRUE;
}

//! InterpretAsRow::copyTopNode method
RelExpr * InterpretAsRow::copyTopNode(RelExpr *derivedNode, CollHeap * outHeap)
{
   InterpretAsRow *result;
   CMPASSERT (derivedNode == NULL);
   result = new (outHeap) InterpretAsRow(outHeap);

   if (auditRowImage_ != NULL)
      result->auditRowImage_ = auditRowImage_->copyTree(outHeap)->castToItemExpr();

   result->auditRowImageVid_ = auditRowImageVid_;

   if (modifiedFieldMap_ != NULL)
      result->modifiedFieldMap_ = modifiedFieldMap_->copyTree(outHeap)->castToItemExpr();

   result->mfMapVid_ = mfMapVid_;

   result->objectName_ = objectName_;

   result->compressedAudit_ = compressedAudit_;

   result->columnsToExtract_ = columnsToExtract_;

   return TableValuedFunction::copyTopNode(result, outHeap);
}


//! InterpretAsRow::pushdownCoveredExpr method
// InterpretAsRow::pushdownCoveredExpr() is the method which determines the
// columns to extract from the audit row image. Prior to this method being
// called, columnsToExtract_ is set to all the columns in the table. This
// method prunes that list, and only includes columns that are required
// for evaluating the selection predicate, or that are required by the
// parent node. Note that the output columns from the INTERPRET_AS_ROW function
// and the extraction columns may not match, if there is a selection predicate.
// For example, in the following case:
// select a from
//    (select c1, c2
//     from table (interpret_as_row(:ai, NULL, 0, TABLE t))) as tab(a,b)
//     where b < 10;
// the output col is a (or c1), whereas the extraction cols must be
// c1 and c2, since there is a selection predicate of b (or c2) < 10.
// Also, the output of this function may be a function itself, for example,
// AUDIT_IMAGE. In this case, all the columns needed by AUDIT_IMAGE need
// to be extracted.
void InterpretAsRow::pushdownCoveredExpr(const ValueIdSet &outputExprOnOperator,
                                         const ValueIdSet &newExternalInputs,
                                         ValueIdSet &predicatesOnOperator,
                                         const ValueIdSet *nonPredNonOutputExprOnOperator,
                                         Lng32 childIndex
...
{
  ValueIdSet extractionCols (columnsToExtract_);
  ValueId refVal;
  ValueIdSet exprOnOperator ;
  if (nonPredNonOutputExprOnOperator)
    exprOnOperator = *nonPredNonOutputExprOnOperator;


  // Prune from the columnsToExtract_ ValueIdSet, those expressions
  // that are not needed by the parent, given by the values in
  // setOfValuesReqdByParent/getGroupAttr()->getCharacteristicOutputs(),
  // or by the selectionPred.
  //
  for(ValueId extractId = extractionCols.init();
      extractionCols.next(extractId);
      extractionCols.advance(extractId))
  {

      if(!outputExprOnOperator.referencesTheGivenValue(extractId, refVal) &&
         !exprOnOperator.referencesTheGivenValue(extractId, refVal) &&
         !selectionPred().referencesTheGivenValue(extractId, refVal))
        columnsToExtract_ -= extractId;
  }

  // No need to call RelExpr::pushdownCoveredExpr() since we are a leaf node
  // just return.
}
