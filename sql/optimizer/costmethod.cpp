...
//**********************************************************************
// Next few methods deal with costing of insert operator
//**********************************************************************
void CostMethodDP2Insert::cacheParameters(RelExpr *op, const Context *myContext)
{
   CostMethod::cacheParameters(op, myContext);

   DP2Insert* insOp = (DP2Insert *)op;
   CIDesc_ = (IndexDesc *)insOp->getIndexDesc();
   indexLevels_ = CIDesc_->getIndexLevels();
   kbPerBlock_ = CIDesc_->getBlockSizeInKb();
   rowsPerBlock_ =
     (((kbPerBlock_ * csOneKiloBytes) /
       CIDesc_->getNAFileSet()->getRecordLength()).minCsOne()).getFloor();
   // for vertical partitioning rows per block in core test035
   // was equal to 0 because row length in fileSte_ was 6709 ???
   // I added uplifting it to one to prevent posible problems when dividing
   // by rowsPerBlock_
   ...
   entriesInIndexBlock_ = (kbPerBlock_ * 1024 /
                        (CIDesc_->getKeyLength() + 8)).getFloor();

   DCMPASSERT(entriesInIndexBlock_.getValue() != 0);

   // Pushdown property
   pushDownProperty_ =
     (myContext->getPlan() && myContext->getPlan()->getPhysicalProperty()) ? 
     ((PhysicalProperty *)myContext->getPlan()->getPhysicalProperty())->getPushDownProperty() : NULL;
   // Materialized view logging
   usedForMvLogging_ =
     ((((Insert *)op_)->getInliningInfo()).isUsedForMvLogging() == TRUE);

} // CostMethodDP2Insert::cacheParameters()

//************************************************************************
// Compute number of inserts
//************************************************************************
CostScalar CostMethodDP2Insert::calculateNumberOfInserts()
{
    return getNumberOfInserts();
} // CostMethodDP2Insert::calculateNumberOfInserts()

//****************************************************************
//
// SEEK Estimation

// For small number of inserts, we say there will only be two seeks:
// one for the data blocks, and one for the index blocks. In all there are
// four seeks (two for read and two for write)
//
// For large number of random inserts, number of seeks would be equivalent to
// number of inserts. For ordered inserts, number of seeks would be
// number of blocks divided by number of blocks that could be written in
// one seek
//
//****************************************************************
void CostMethodDP2Insert::calculateIOCosts()
{
  seeksLR_ = 4.0;
  seeksFR_ = 4.0;
  transferFR_ =  (indexLevels_ * kbPerBlock_) * csTwo;
  transferLR_ = (numInserts_  * kbPerBlock_) * csTwo;

  // Cost the materialized view logging pushdown plan similar to the
  // sorted (probes in order) insert plan.
  // This will favor the pushdown plan as the pushdown plan will be
  // cheaper without the extra exchanges and cost of messaging.
  if (usedForMvLogging_ && pushDownProperty_!= NULL)
  {
    // assumption: read-ahead same as write ahead?
    // we assume that on average there is a seek for
    // DP2_CACHE_1024_BLOCKS.
    seeksLR_ = (numInserts_ / CostPrimitives::getBasicCostFactor
                (DP2_MAX_READ_PER_ACCESS_IN_KB)).minCsOne();
    seeksFR_ = 0;
    transferLR_ = ((csOne + totalIndexBlocks_) +
                   (numInserts_ + totalIndexBlocks_)) * kbPerBlock_;
    if (transferFR_ > transferLR_)
      transferLR_ = transferFR_;
    return;
  }

  // this may happen for small number of inserts
  if (transferFR_ > transferLR_)
     transferLR_ = transferFR_;

  // do not increase if it is entry sequenced file
  // as rows are inserted in the order they are received
  if (NOT (CIDesc_->getNAFileSet()->isKeySequenced()) )
    return;

  // also if the clustering key is just SYSKEY, do not increase seeks
  if ((CIDesc_->getIndexKey().entries() == 1) &&
      (CIDesc_->getNAFileSet()->hasSyskey()))
    return;

  // Number of rows fit in DP2 cache? smaller number of inserts?
  CostScalar dataBlocks = numInserts_;
  ...
  if (! probesInOrder_)
  {
     if (numInserts_ < 100)
      ...
      CostPrimitives::getBasicCostFactor(DP2_MAX_READ_PER_ACCESS_IN_KB))
    return;

  // for simple inserts we do not divide number of blocks by number of
  // write ahead (or read ahead) blocks so as to model the DP2
  // and executor behaviour. For other kinds of inserts we do the
  // division
  if (CmpCommon::getDefault(COMP_BOOL_50) == DF_OFF)
  {
    seeksLR_ += numInserts_;
  }
}

//*****************************************************************
// In this method we compute the cpu and idle costs.
//*****************************************************************

void CostMethodDP2Insert::calculateCpuAndIdleCosts()
{
  // We are doing binary search inside index blocks

  CostScalar commonComputation = indexLevels_ *
                                 CIDesc_->getIndexKey().entries() *
       CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON);

  if ( CmpCommon::getDefault(COMP_BOOL_50) == DF_OFF AND
       entriesInIndexBlock_.isGreaterThanOne() )
    commonComputation *=floor(1.443*log(entriesInIndexBlock_.value()));
  else
    commonComputation *= (entriesInIndexBlock_ / csTwo).minCsOne();

  // For buffered inserts, we only need to pay the first row
  // cost for read. For unordered inserts, the read cost is the
  // same as the write cost.
  if (probesInOrder_ ||
      (usedForMvLogging_ && pushDownProperty_ != NULL))
  {
    cpuLR_ = commonComputation + (commonComputation * numInserts_);
  }
  else
  {
    cpuLR_ = (commonComputation * numInserts_) * csTwo;
  }
  cpuFR_ = commonComputation * csTwo;

  // LOCKING COST (part of the CPU cost)
  // This cost is only paid once, for write.
  commonComputation = CostPrimitives::getBasicCostFactor(CPUCOST_LOCK_ROW);

  cpuLR_ += commonComputation *
            (noOfProbes_ / countOfAsynchronousStreams_).minCsOne();

  cpuFR_ += commonComputation;

  //FILE OPEN COST (blocking cost, so must be added to idle time)
  //this is the cost for opening all the partitions but the first partition.
  //The first partition is opened before any other partitions
  //opening the first partition, the necessary info on the file is also
  //acquired so it is more expensive.
  ...
  idleFR_ = idleLR_ =
     ...
     MAXOF(0, (activePartitions_.getValue()-1)) *
     CostPrimitives::getBasicCostFactor(MSCF_ET_CPU);
} // CostMethodDP2Insert::calculateCpuAndIdleCosts()

//***********************************************************************
// Seeks and transfer costs are computed for VSBB inserts
//***********************************************************************
void CostMethodDP2VSBBInsert::calculateIOCosts()
{
  if ((probesInOrder_ ||
       (usedForMvLogging_ && pushDownProperty_ != NULL)) &&
      CmpCommon::getDefault(COMP_BOOL_50) == DF_OFF)
  {
    // assumption: read-ahead same as write ahead?
    // we assume that on average there is a seek for
    // DP2_CACHE_1024_BLOCKS.
  ...
  else if (CmpCommon::getDefault(COMP_BOOL_50) == DF_ON)
  {
    seeksLR_ = 4.0;
  }
  else
  ...
  transferFR_ = (indexLevels_ * kbPerBlock_) * 2;

  // VSBB Insert read costs are needed..
  ...
  {
    transferLR_ = ((numInserts_ + totalIndexBlocks_) * kbPerBlock_) * csTwo;
  }

  // this may happen for very small tables
  if (transferFR_ > transferLR_)
     transferLR_ = transferFR_;

} // CostMethodDP2VSBBInsert::calculateIOCosts()

//*******************************************************************
//Side-Tree insert:writing to unaudited empty tables: no read costs
//exist
//*******************************************************************
void CostMethodDP2SideTreeInsert::calculateIOCosts()
{
  // Cost the materialized view logging pushdown plan similar to the
  // sorted (probes in order) insert plan.
  // This will favor the pushdown plan as the pushdown plan will be
  ...
  {
    // assumption: read-ahead same as write ahead?
    // no reading is needed; dividing by two to model the compact b-tree
    seeksLR_ = ((numInserts_ /
                 CostPrimitives::getBasicCostFactor
                   (DP2_MAX_READ_PER_ACCESS_IN_KB))/csTwo).minCsOne();
  }
  else if (CmpCommon::getDefault(COMP_BOOL_50) == DF_ON)
  {
    seeksLR_ = 4.0;
  ...
  {
    // b-tree would not be very compact in this case
    // because of random inserts
    seeksLR_= numInserts_;
  }

  transferFR_ = indexLevels_ * kbPerBlock_;

  // sidetree insert always appends; table is empty;
  //  so read costs are not included in transfer
  // if probes are not in order, number of inserts would be number of probes
  // want to keep the transfer costs high to favor buffered insert
  ...
  if ((probesInOrder_ ||
      ...
      CmpCommon::getDefault(COMP_BOOL_50) == DF_OFF)
  {
    transferLR_ = (numInserts_ + totalIndexBlocks_) * kbPerBlock_;
  }
  else if (probesInOrder_ ||
           (usedForMvLogging_ && pushDownProperty_ != NULL))
  {
    transferLR_ =  (numInserts_ + totalIndexBlocks_) * kbPerBlock_;
  }
  ...
  // this may happen for really small files
  if (transferLR_ < transferFR_)
    transferLR_ = transferFR_;

} // CostMethodDP2SideTreeInsert::calculateIOCosts()

//*****************************************************************
//*****************************************************************
// This method would return actual number of rows if probes are not in Order.
// It would return number of blocks the rows fit in, if rows are in Order
//*****************************************************************
CostScalar CostMethodDP2Insert::getNumberOfInserts()
{
  CostScalar numOfProbesPerStream =
     (noOfProbes_ / countOfAsynchronousStreams_).minCsOne();

  CostScalar numOfInserts;
  // probesInOrder_ would be true if rows are in order and VSBB is turned off
  // Cost the materialized view logging pushdown plan similar to the
  // sorted (probes in order) insert plan.
  // This will favor the pushdown plan as the pushdown plan will be
  // cheaper without the extra exchanges and cost of messaging.
  if (probesInOrder_ ||
      (usedForMvLogging_ && pushDownProperty_ != NULL))
  {
    numOfInserts = (numOfProbesPerStream / rowsPerBlock_).getCeiling();
  }
  else
  {
    numOfInserts= numOfProbesPerStream;
  }
  return numOfInserts;
} // CostMethodDP2Insert::getNumberOfInserts()

//**************************************************************
// the method returns either number of blocks or number of rows
// depending on whether rows are in order or not
//**************************************************************
CostScalar CostMethodDP2SideTreeInsert::calculateNumberOfInserts()
{
  return getNumberOfInserts();
} // CostMethodDP2Insert::calculateNumberOfInserts()
...
// depending on whether rows are in order or not
//***********************************************************
CostScalar CostMethodDP2VSBBInsert::calculateNumberOfInserts()
{
  return getNumberOfInserts();
} // CostMethodDP2Insert::calculateNumberOfInserts()

//**************************************************************
// This method computes the cost vector of the insert operation
// it is the main method and calls most of other methods
//**************************************************************
Cost*
CostMethodDP2Insert::computeOperatorCostInternal(RelExpr* op,
                                                 const Context* myContext,
                                                 Lng32& countOfStreams)
{
  cacheParameters(op,myContext);
  ...
  DP2Insert* insOp = (DP2Insert *)op;   // downcast
  ...
  if (ippForMe != NULL)  // input physical properties exist?
  {
    ...
    ValueIdList targetSortKey = CIDesc_->getOrderOfKeyValues();
  ...
  // First we compute how many inserts are taking place:
  // if rows are in-order, then this would be equivalent
  // to how many equivalent blocks are being inserted;
  // otherwise, this is equivalnet to how many rows are
  // being inserted per a partition
  numInserts_= calculateNumberOfInserts();

  // Determine number of total index blocks
  // It's hard to accurately know how many total index blocks there
  // are for insert, since the table is constantly expanding.
  // Assume the total # of index blocks is the average # of index blocks
  // for the # of index levels the table has to start with. Also assume
  // index blocks always stay in cache after they are read.
  totalIndexBlocks_ = CIDesc_->getEstimatedIndexBlocksLowerBound(numInserts_);

  // The above call under-estimates number of index blocks for large inserts;
  // in the following code we try to adjust the under-estimation.
  // ( Case 10-040421-6460.)
  // Adjust it assuming
  // that index blocks are 70% full. The last 2 conditions guarantee the
  // exit from the while loop.

  double avgSlack = 0.7;

  // why we are comparing noOfProbes to 10000?
  // in the following code, we are interested in change
  // in indexBlocks. We are assuming that that change is
  // minimal if number of inserts is less than 10000.
  if ( (CmpCommon::getDefault(COMP_BOOL_50) == DF_OFF ) AND
       (noOfProbesPerPartition > 10000)
     )
  {
    CostScalar numIndexBlocksOnOneLevel =
      (noOfProbesPerPartition * avgSlack/entriesInIndexBlock_).getCeiling();
    while( numIndexBlocksOnOneLevel.isGreaterThanOne() )
    {
      totalIndexBlocks_ +=numIndexBlocksOnOneLevel;
      indexLevels_+=csOne;
      numIndexBlocksOnOneLevel=
        (numIndexBlocksOnOneLevel*0.7/entriesInIndexBlock_).getCeiling();
    }
  }

  // We try to compute number of random I/Os or bulk sequential I/Os:
  // we primarily do this by computing estimates number of seeks and
  // transfer costs
  calculateIOCosts();


  // compute cpu and idle costs: costs for traversing index block
  calculateCpuAndIdleCosts();

  // ---------------------------------------------------------------------
  // Synthesize the cost vectors.
  // ---------------------------------------------------------------------
  cvFR.setInstrToCPUTime(cpuFR_);
  cvFR.addSeeksToIOTime(seeksFR_);
  cvFR.addKBytesToIOTime(transferFR_);
  cvFR.setIdleTime(idleFR_);
  cvFR.setNumProbes(noOfProbesPerPartition);

  cvLR.setInstrToCPUTime(cpuLR_);
  cvLR.addSeeksToIOTime(seeksLR_);
  cvLR.addKBytesToIOTime(transferLR_);
  cvLR.setIdleTime(idleLR_);
...
}  // CostMethodDP2Insert::computeOperatorCostInternal()

void CostMethodDP2Insert::cleanUp()
{
  CIDesc_= NULL;
  probesInOrder_ = FALSE;
  indexLevels_ = csZero;
  ...
  cpuLR_ = csZero;
  cpuFR_ = csZero;
  entriesInIndexBlock_ = csZero;

  pushDownProperty_ = NULL;
  usedForMvLogging_ = FALSE;
  // Clean up fields in base class
  CostMethod::cleanUp();
}
...
// ----QUICKSEARCH FOR CostMethodDP2UpdateOrDelete......................
...
// -----------------------------------------------------------------------
// CostMethodDP2UpdateOrDelete::allKeyColumnsHaveHistogramStatistics().
// -----------------------------------------------------------------------
NABoolean CostMethodDP2UpdateOrDelete::allKeyColumnsHaveHistogramStatistics(
...
// -----------------------------------------------------------------------
// CostMethodDP2UpdateOrDelete::numRowsToScanWhenAllKeyColumnsHaveHistograms().
// -----------------------------------------------------------------------
#pragma nowarn(262)   // warning elimination
CostScalar
CostMethodDP2UpdateOrDelete::numRowsToScanWhenAllKeyColumnsHaveHistograms(
...
// -----------------------------------------------------------------------
// CostMethodDP2UpdateOrDelete::computePartialScanCostForDP2CursorOperation().
// -----------------------------------------------------------------------
void CostMethodDP2UpdateOrDelete::computePartialScanCostForDP2CursorOperation(
  ...
  ) const
{
  ...
  // Compute SCAN COST to read the blocks containing the
  // the rows to be updated.

  // $$$ Need to account for cache hits on data blocks.

  // SEEK COST

  // Cost the materialized view logging pushdown plan similar to the
  // sorted (probes in order) update plan.
  // This will favor the pushdown plan as the pushdown plan will be
  // cheaper without the extra exchanges and cost of messaging.
  if (probesInOrder ||
      (usedForMvLogging_ && pushDownProperty_ != NULL))
  {
    ...
    seeksLR = (numRowsToScan / rowsPerBlock).getCeiling();
    // The # of index blocks to read is based on the number of data
    // blocks that must be read
    totalIndexBlocks =
      CIDesc->getEstimatedIndexBlocksLowerBound(seeksLR);
    seeksLR = seeksLR + totalIndexBlocks;
  }
  else
  {
    totalIndexBlocks =
      CIDesc->getEstimatedIndexBlocksLowerBound(numRowsToScan);
    seeksLR = numRowsToScan + totalIndexBlocks;
  }
  ...
  /*
  if (probesInOrder)
    transferLR = ((numRowsToScan / rowsPerBlock).getCeiling() +
                  totalIndexBlocks) * kbPerBlock;
  else
    transferLR = (numRowsToScan + totalIndexBlocks) * kbPerBlock;
  */
  // Almost the same logic as in seek cost.
  // Avoid unnecessary computation by using the seek costs above.
  transferLR = seeksLR * kbPerBlock;
  transferFR = indexLevels * kbPerBlock;


  // CPU COST

  // Cost for traversing the Btree structure to find the row to update
  // plus overhead for each request.

  // Common computation here is explained above.
  commonComputation =
    (indexLevels + csOne) *
    MIN_ONE_CS( rowsPerBlock / csTwo ) *
    CIDesc->getIndexKey().entries() *
    CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON)
    + // overhead
    CostPrimitives::getBasicCostFactor(CPUCOST_DATARQST_OVHD);

  cpuFR = commonComputation;
  cpuLR = commonComputation * numRowsToScan;


  // LOCKING COST (part of the CPU cost)
  // Common computation here is the cpu cost to lock a row.
  commonComputation = CostPrimitives::getBasicCostFactor(CPUCOST_LOCK_ROW);
...
// -----------------------------------------------------------------------
// CostMethodDP2UpdateOrDelete::computePartialScanCostForDP2SubSetOperation().
// -----------------------------------------------------------------------
void CostMethodDP2UpdateOrDelete::computePartialScanCostForDP2SubSetOperation(
  CostScalar & seeksFR, CostScalar & transferFR, CostScalar & cpuFR,
  CostScalar & seeksLR, CostScalar & transferLR, CostScalar & cpuLR,
  const IndexDesc * CIDesc,
  const CostScalar & numRowsToScan,
  ...
  ) const
{
  ...
  transferLR =
      ((numRowsToScan / rowsPerBlock).getCeiling()
    + (indexLevels - csOne)) * kbPerBlock;
  transferFR = kbPerBlock;

  // CPU COST

  // Cost for traversing the Btree structure to find the 1st row to read
  // Only done once for sequential scans
  commonComputation =
    (indexLevels + csOne) *
    MIN_ONE_CS( rowsPerBlock / csTwo ) *
    CIDesc->getIndexKey().entries() *
  ...
  cpuLR = commonComputation;
  cpuFR = commonComputation;

  // Add in an overhead for each request
  cpuLR += CostPrimitives::getBasicCostFactor(CPUCOST_DATARQST_OVHD);
  cpuFR += CostPrimitives::getBasicCostFactor(CPUCOST_DATARQST_OVHD);

  // Compute cost for evaluating executor predicates
  commonComputation = numExecutorPreds *
    CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON );

  cpuLR += commonComputation * numRowsToScan;
  cpuFR += commonComputation;

  // Add in the cost to make a copy of every row to bring it into
  // exeindp2 from dp2
  // Common computation here is the cpu cost to copy a record (row).
  commonComputation =
    recordSizeInKb * csOneKiloBytes	    // record size in bytes
    * CostPrimitives::getBasicCostFactor(CPUCOST_COPY_ROW_PER_BYTE)
    ;

  cpuLR +=
    CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_COPY_ROW_OVERHEAD))
    + (numRowsToScan * commonComputation)
    ;
  ...
  // is a catch all cost *per KB* (despite the name)
  // This catches all overhead that happens when rows are moved
  // from the dp2 cache into the exe in dp2.

  // Common computation here is the cpu cost of the overhead of scanning a row.
  commonComputation = recordSizeInKb *
    CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_SCAN_OVH_PER_ROW));
...
// -----------------------------------------------------------------------
// CostMethodDP2UpdateOrDelete::computeIdleCost().
// -----------------------------------------------------------------------
CostScalar CostMethodDP2UpdateOrDelete::computeIdleCost(
  const CostScalar & activePartitions )
{
  return
    CostPrimitives::getBasicCostFactor(CPUCOST_SUBSET_OPEN_AFTER_FIRST) *
...
void CostMethodDP2Update::cacheParameters(RelExpr *op, const Context *myContext)
{
   CostMethod::cacheParameters(op, myContext);
   ...
   // Materialized view logging
   usedForMvLogging_ =
     ((((Update *)op_)->getInliningInfo()).isUsedForMvLogging() == TRUE);

} // CostMethodDP2Update::cacheParameters()

// -----------------------------------------------------------------------
// CostMethodDP2Update::computeOperatorCostInternal().
// -----------------------------------------------------------------------
// LCOV_EXCL_START :cnu -- OCM code
Cost*
CostMethodDP2Update::computeOperatorCostInternal(RelExpr* op,
                                                 const Context* myContext,
                                                 Lng32& countOfStreams)
{
  cacheParameters(op,myContext);
  ...
  DP2Update* updOp = (DP2Update *)op;   // downcast
  ...
  CostScalar cpuFR(csZero),	 cpuLR(csZero),
	     seeksFR(csZero),	 seeksLR(csZero),
	     transferFR(csZero), transferLR(csZero),
	     idleFR(csZero),	 idleLR(csZero);
  ...
  if (ippForMe != NULL)  // input physical properties exist?
  {
    ...
    // For update, a partial order is ok.
  ...
  // For update subset and unique, # of probes should always be 1,
  // so we won't bother computing # of probes per partition, we'll
  // just use the default (1).
  CostScalar noOfProbesPerPartition(csOne);

  CostScalar numRowsToUpdate(csOne);
  CostScalar numRowsToScan(csOne);

  CostScalar commonComputation;

  SimpleCostVector cvFRscan;
  SimpleCostVector cvLRscan;
  SimpleCostVector cvFRupdate;
  SimpleCostVector cvLRupdate;


  // ************************************************************
  // FIRST: Determine # of rows to scan/update
  // ************************************************************

  if (updOp->getOperatorType() == REL_DP2_UPDATE_CURSOR)
  {
    // Determine the number of probes per stream. Use this number as
    // the number of rows to read and update (this is "per-stream" costing).
    noOfProbesPerPartition =
      ( (noOfProbes_ / countOfAsynchronousStreams).getCeiling() ).minCsOne();
    numRowsToUpdate = numRowsToScan = noOfProbesPerPartition;
  }
  else if (updOp->getOperatorType() == REL_DP2_UPDATE_SUBSET)
  {
    numRowsToUpdate = ((myRowCount_/activePartitions).getCeiling()).minCsOne();
  ...
  else // REL_DP2_UPDATE_UNIQUE
  {
    ...
    // we'll always do the update
    numRowsToUpdate = csOne;
  }


  // ************************************************************
  // SECOND : Compute SCAN COST
  // ************************************************************

  if (updOp->getOperatorType() == REL_DP2_UPDATE_CURSOR)
  {
    // Compute the seeks, transfer and cpu cost for first row
    // and last row.
    computePartialScanCostForDP2CursorOperation(
      seeksFR, transferFR, cpuFR,
      seeksLR, transferLR, cpuLR,
      CIDesc,
      numRowsToScan,
      probesInOrder
      );
    ...
    // In addition to the ones computed above, we need the following.

    // There is a cpu copy/transfer cost for cursor updates,
    // because we must read the row again (via a GET UNIQUE call
    // via the primary key supplied by the flow node) to get the
    // other column values for that row in case they are needed by
    // a SET clause expression.
    ...
    cpuLR +=
	commonComputation * numRowsToScan
    ...
    // FILE OPEN COST (blocking cost, so must be added to idle time)
    idleFR = idleLR = computeIdleCost( activePartitions );


    // ---------------------------------------------------------------------
    // Synthesize the cost vectors.
    // ---------------------------------------------------------------------
    cvFRscan.setInstrToCPUTime(cpuFR);
    cvFRscan.addSeeksToIOTime(seeksFR);
    cvFRscan.addKBytesToIOTime(transferFR);
    cvFRscan.setIdleTime(idleFR);
    cvFRscan.setNumProbes(noOfProbesPerPartition);

    cvLRscan.setInstrToCPUTime(cpuLR);
    cvLRscan.addSeeksToIOTime(seeksLR);
    cvLRscan.addKBytesToIOTime(transferLR);
    cvLRscan.setIdleTime(idleLR);
    cvLRscan.setNumProbes(noOfProbesPerPartition);


    // Also: Compute Update SET clause evaluation cost

    // Cost for SET clause expression evaluation and assignment
    // (part of the CPU cost).
    // $$$ Right now, we are using the constant for the cost of a
    // predicate evaluation. Really need a constant for the cost of
    // an assignment, and then compute that cost for all columns
    // being updated; then add in an expression evaluation cost for
    // all SET clause expressions that are not constants or columns.

    // Common computation here is explained above.
    commonComputation = CostScalar(updOp->newRecExpr().entries()) *
      CostPrimitives::getBasicCostFactor(CPUCOST_PREDICATE_COMPARISON);

    cpuLR = commonComputation * numRowsToUpdate;
    cpuFR = commonComputation;

    // DM UPDATE COST (part of the CPU cost)
    // cost of shipping the updated rows back to DP2
    cpuLR += numRowsToUpdate *
      CostPrimitives::getBasicCostFactor(CPUCOST_DM_UPDATE);
    cpuFR += CostPrimitives::getBasicCostFactor(CPUCOST_DM_UPDATE);

    // ************************************************************
    // No write cost, since all writes are done asynchronously
    // ************************************************************

    // ---------------------------------------------------------------------
    // Synthesize the cost vectors.
    // ---------------------------------------------------------------------
    cvFRupdate.setInstrToCPUTime(cpuFR);
    cvFRupdate.setNumProbes(noOfProbesPerPartition);

    cvLRupdate.setInstrToCPUTime(cpuLR);
    cvLRupdate.setNumProbes(noOfProbesPerPartition);


  }  // end REL_DP2_UPDATE_CURSOR

  else // REL_DP2_UPDATE_SUBSET, REL_DP2_UPDATE_UNIQUE
  {
    // Compute the seeks, transfer and cpu cost for first row
    // and last row.
    computePartialScanCostForDP2SubSetOperation(
      seeksFR, transferFR, cpuFR,
      seeksLR, transferLR, cpuLR,
      CIDesc,
      numRowsToScan,
      updOp->executorPred().entries()
      );
    ...
    // Add in the cost to make a copy of every row to ship the updated
    // rows back to DP2
    ...
    cpuLR +=
      CostScalar(CostPrimitives::getBasicCostFactor(CPUCOST_COPY_ROW_OVERHEAD))
      + (numRowsToUpdate * commonComputation)
      ;
    ...
    // CPUCOST_SCAN_OVH_PER_ROW
    // is a catch all cost *per KB* (despite the name)
    // This catches all overhead that happens when rows to update are moved
    // from the exe in dp2 back to DP2.
  ...
  }  // end REL_DP2_UPDATE_SUBSET,REL_DP2_UPDATE_UNIQUE



  // Add up scan cost + update SET clause cost
  SimpleCostVector cvFR = cvFRscan + cvFRupdate;
  SimpleCostVector cvLR = cvLRscan + cvLRupdate;
  ...
  if ( CmpCommon::getDefault( OPTIMIZER_PRINT_COST ) == DF_ON )
    {
      pfp = stdout;
      fprintf(pfp, "DP2Update elapsed time: ");
      fprintf(pfp,"%f", costPtr->
              convertToElapsedTime(
                   myContext->getReqdPhysicalProperty()).
              value());
...
void CostMethodDP2Update::cleanUp()
{
  pushDownProperty_ = NULL;
  usedForMvLogging_ = FALSE;
  // Clean up fields in base class
...
// -----------------------------------------------------------------------
// CostMethodDP2Delete::computeOperatorCostInternal().
// -----------------------------------------------------------------------
// LCOV_EXCL_START :cnu -- OCM code
Cost*
CostMethodDP2Delete::computeOperatorCostInternal(RelExpr* op,
                                                 const Context* myContext,
                                                 Lng32& countOfStreams)
{
  cacheParameters(op,myContext);
  ...
  DP2Delete* delOp = (DP2Delete *)op;   // downcast
  ...
  CostScalar cpuFR(csZero),	 cpuLR(csZero),
	     seeksFR(csZero),	 seeksLR(csZero),
	     transferFR(csZero), transferLR(csZero),
	     lockingFR(csZero),	 lockingLR(csZero),
	     idleFR(csZero),	 idleLR(csZero);
  ...
  // For delete subset, range, and unique, # of probes should always be 1,
  // so we won't bother computing # of probes per partition, we'll
  // just use the default (1).
  CostScalar noOfProbesPerPartition(csOne);
  ...
  // ************************************************************
  // FIRST: Determine # of rows to scan/Delete
  // ************************************************************

  if (delOp->getOperatorType() == REL_DP2_DELETE_CURSOR)
  {
    // Determine the number of probes per stream. Use this number as
    // the number of rows to read and delete (this is "per-stream" costing).
    noOfProbesPerPartition =
      ((noOfProbes_ / countOfAsynchronousStreams).getCeiling()).minCsOne();
    numRowsToDelete = numRowsToScan = noOfProbesPerPartition;
  }
  else if ((delOp->getOperatorType() == REL_DP2_DELETE_SUBSET))
#if 0
// use subset delete instead, done as part of SQ SQL code cleanup effort
           || (delOp->getOperatorType() == REL_DP2_DELETE_RANGE))
#endif // if 0
  {
    noOfProbesPerPartition =
      ((noOfProbes_ / countOfAsynchronousStreams).getCeiling()).minCsOne();
  ...
  if (delOp->getOperatorType() == REL_DP2_DELETE_CURSOR)
  {
    ...
    // Synthesize the cost vectors.
    // ---------------------------------------------------------------------
    cvFR.setInstrToCPUTime(cpuFR);
    cvFR.addSeeksToIOTime(seeksFR);
    cvFR.addKBytesToIOTime(transferFR);
    cvFR.setIdleTime(idleFR);
    cvFR.setNumProbes(noOfProbesPerPartition);

    cvLR.setInstrToCPUTime(cpuLR);
    cvLR.addSeeksToIOTime(seeksLR);
    cvLR.addKBytesToIOTime(transferLR);
    cvLR.setIdleTime(idleLR);
    cvLR.setNumProbes(noOfProbesPerPartition);

  }  // end REL_DP2_DELETE_CURSOR

  else // REL_DP2_DELETE_SUBSET, REL_DP2_DELETE_RANGE, REL_DP2_DELETE_UNIQUE
  {
    ...
    // ---------------------------------------------------------------------
    // Scale last row cost scalars up by the no of probes.
    // ---------------------------------------------------------------------
    cpuLR *= noOfProbesPerPartition;
    seeksLR *= noOfProbesPerPartition;
    transferLR *= noOfProbesPerPartition;
  ...
  }  // end REL_DP2_DELETE_SUBSET,REL_DP2_DELETE_RANGE,REL_DP2_DELETE_UNIQUE
  ...
  if ( CmpCommon::getDefault( OPTIMIZER_PRINT_COST ) == DF_ON )
    {
      pfp = stdout;
      fprintf(pfp, "DP2 Delete elapsed time: ");
      fprintf(pfp,"%f", costPtr->
              convertToElapsedTime(
                   myContext->getReqdPhysicalProperty()).
              value());
...
/**********************************************************************/
// End cost methods for WRITE operations
/**********************************************************************/
