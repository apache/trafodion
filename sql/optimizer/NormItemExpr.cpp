...
// -----------------------------------------------------------------------
// SeqGenSubquery::transformNode()
// -----------------------------------------------------------------------
void SeqGenSubquery::transformNode(NormWA & normWARef,
                                   ExprValueId & locationOfPointerToMe,
                                   ExprGroupId & introduceSemiJoinHere, // it's the nextValueFor operator here.
				   const ValueIdSet & externalInputs)
{
  if (nodeIsTransformed())
    {
  ...
  // Create the system HostVar for the result of the NextValueFor.
  //
  ItemExpr *resultNextValue =
    new (heap) HostVar("_sys_hostNextValue",
                       new (heap) SQLLargeInt(TRUE   /*signed  */,
                                              FALSE  /*not null*/),
                       TRUE); /* is system-supplied */
  
  resultNextValue->synthTypeAndValueId();

  ValueIdSet outputValues;
  NextValueFor *nextValueFor = NULL;

  // ---------------------------------------------------------------------
  // Allocate a NextValueFor, create its group attributes and make it
  // the parent of the SequenceGenerator.
  // ---------------------------------------------------------------------
  ...
  NABoolean usingInternal = FALSE;
  NABoolean usingSG = FALSE;
  
  if (childOfRoot->getOperatorType() == REL_SEQUENCE_GENERATOR)
    usingSG = TRUE;
  
  // dummy values clause.
  if (childOfRoot->getOperatorType() == REL_RENAME_TABLE) 
    usingInternal = TRUE;
  
  
  if (usingSG)
    {
      SequenceGenerator *seqGen = (SequenceGenerator *)childOfRoot;
      
      // There should only be one entry - the current value from
      // the Update that is the selectList of the subquery.
      ItemExpr *currentValue = getSubquery()->selectList();
      
      seqGen->setCurrentValueFromSGTable(currentValue->getValueId());
 
      // The outputs will have sgCacheSize (N) and current_value. The
      // increment is actually available in the SGAttributes that is
      // stored locally.
      seqGen->getPotentialOutputValues(outputValues);
      
      // SG operator has no inputs; just outputs.
      // So, add to the characteristic outputs
      seqGen->getGroupAttr()->addCharacteristicOutputs(outputValues);
      
      // -----------------------------------------------------------------
      // Allocate the next value for operator.
      // -----------------------------------------------------------------
      nextValueFor = new(normWARef.wHeap())
	NextValueFor(introduceSemiJoinHere, 
		     childOfRoot, 
		     resultNextValue->getValueId());
      
      nextValueFor->setSequenceValueIsUnique(seqGen->isSequenceValueUnique());

      // We need N and the SGTable.current_value here at
      // the nextValueFor operator for computing the next value.

      // add the current value that comes from the Update to
      // the outputsFromSG_.
      nextValueFor->addToOutputsFromSG(currentValue->getValueId());
      
      // Now add the sgCacheSize(N), which is stored in seqGen->sgCacheSizeHV_
      nextValueFor->addToOutputsFromSG(seqGen->sgCacheSizeHV());
  ...
  nextValueFor->setGroupAttr(new(normWARef.wHeap()) GroupAttributes());


  // ---------------------------------------------------------------------
  // Initialize the dataflow for the nextValueFor that was
  // allocated above.
  // ---------------------------------------------------------------------
  outputValues.clear();
  nextValueFor->getPotentialOutputValues(outputValues);
  // add the nextValue as an output.
  outputValues += resultNextValue->getValueId();
  nextValueFor->getGroupAttr()->addCharacteristicOutputs(outputValues);

  // make nextValue the locationOfPointerToMe.
  locationOfPointerToMe = resultNextValue;
  
  // subtract from the external inputs to the joins values that are
  // produced by the children of the join.
  ValueIdSet realExternalInputs = externalInputs;
  realExternalInputs -= outputValues;
  nextValueFor->getGroupAttr()->addCharacteristicInputs(realExternalInputs);

  // ---------------------------------------------------------------------
  // Assign the SemiJoin (nextValueFor) to the original query tree.
  // A new SemiJoin (nextValueFor) is added in introduceSemiJoinHere. 
  // It has not been transformed yet. It will eventually be transformed 
  // either by it's parent or one of it's children if the SemiJoin becomes
  // became the ancestor of the child while the child was being
  // transformed.
  // ---------------------------------------------------------------------
  introduceSemiJoinHere = nextValueFor;
