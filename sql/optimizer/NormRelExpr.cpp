...
// -----------------------------------------------------------------------
// InterpretAsRow::transformNode()
// -----------------------------------------------------------------------
void InterpretAsRow::transformNode(NormWA &normWARef,
                                   ExprGroupId &locationOfPointerToMe)
{
   ItemExpr *iePtr;
   ExprValueId exprValIdPtr;
   ...
   // Let TableValuedFunction::transformNode() do the work.
   TableValuedFunction::transformNode(normWARef,locationOfPointerToMe);

   iePtr = auditRowImageVid_.getItemExpr();
   exprValIdPtr = iePtr;
   auditRowImageVid_.getItemExpr()->transformNode
                                   (normWARef, exprValIdPtr,
                                    locationOfPointerToMe,
                                    getGroupAttr()->getCharacteristicInputs());

   iePtr = mfMapVid_.getItemExpr();
   exprValIdPtr = iePtr;
   mfMapVid_.getItemExpr()->transformNode
                              (normWARef, exprValIdPtr,
                               locationOfPointerToMe,
                               getGroupAttr()->getCharacteristicInputs());
 
   if (columnsToExtract_.transformNode(normWARef, locationOfPointerToMe,
                                       getGroupAttr()->getCharacteristicInputs()))
   {
      // There should be no subqueries.
      CMPASSERT(0);
   }

   // Pull up the predicates and recompute the required inputs
   // Recompute outer references and maximize outputs. 
   // This is when the select list becomes part of InterpretasRow's 
   // characteristic outputs, and the auditRowImage_ and modifiedFieldMap_
   // inputs become part of InterpretAsRow's characteristic inputs.
   pullUpPreds();
...
void InterpretAsRow::recomputeOuterReferences()
{
   // This is used to recompute the external dataflow input values (namely
   // audit row image and mfMap) that are referenced by InterpretAsRow after
   // the predicate pull up is complete.
   ValueIdSet outerRefs = getGroupAttr()->getCharacteristicInputs();

   ValueIdSet exprSet = auditRowImageVid_;

   if (mfMapVid_.getItemExpr()->getOperatorType() != ITM_CONSTANT)
   {
      // Only add the modified field map to the characteristic inputs if
      // it is not a null constant.
      exprSet += mfMapVid_;
   }
...
RelExpr * InterpretAsRow::normalizeNode(NormWA & normWARef)
{
  // Let TableValuedFunction::normalizeNode() do the work
  return TableValuedFunction::normalizeNode(normWARef);
}
...
void
SequenceGenerator::transformNode(NormWA &   normWARef,
                                 ExprGroupId & locationOfPointerToMe)
{
  CMPASSERT( locationOfPointerToMe.getPtr() == this );
  ...
  child(0)->getGroupAttr()->addCharacteristicInput(sgCacheSizeHV_);
  child(0)->getGroupAttr()->addCharacteristicInput(sgIncrementHV_);
...
} // SequenceGenerator::transformNode()

void SequenceGenerator::rewriteNode(NormWA & normWARef)
{
  // ---------------------------------------------------------------------
  // Rewrite the expressions in the selection predicates and
  // in the Group Attributes.
  ...
  // ---------------------------------------------------------------------
  // Rewrite sgValuesList
  // ---------------------------------------------------------------------
  
  sgCacheSizeHV_.normalizeNode(normWARef);
  sgIncrementHV_.normalizeNode(normWARef);
  currentValueFromSGTable_.normalizeNode(normWARef);
  ...
  // but we don't want the SGTable's update's rowsAffected to be computed
  // (if I insert one row into the table, I want to see "1 row(s) inserted",
  // not 1 + update to the SGTable!).
  // 
  // This could have been done earlier on in the binder, but here is where
  // the update is an immediate child of SequenceGenerator. 
  RelExpr *sgTableUpdate = child(0);
  CMPASSERT(sgTableUpdate->getOperatorType() == REL_UNARY_UPDATE);
  ((GenericUpdate *)sgTableUpdate)->rowsAffected() = GenericUpdate::DO_NOT_COMPUTE_ROWSAFFECTED;
}

void SequenceGenerator::recomputeOuterReferences()
{
  // ---------------------------------------------------------------------
  // Delete all those input values that are no longer referenced on
  // this operator because the predicates that reference them have 
  ...
  // Weed out those expressions not needed by my selectionPred
  ValueIdSet exprSet = getSelectionPred();

  exprSet.weedOutUnreferenced(outerRefs);
  
  // Find values needed by my child
  exprSet = child(0).getPtr()->getGroupAttr()->getCharacteristicInputs();

  // These are the values that will be provided as inputs to my child
  ValueIdSet availableVals;
  availableVals += sgCacheSizeHV_;
  availableVals += sgIncrementHV_;

  exprSet.removeCoveredExprs(availableVals);
  outerRefs += exprSet;

  getGroupAttr()->setCharacteristicInputs(outerRefs);
} // SequenceGenerator::recomputeOuterReferences()  
