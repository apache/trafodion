...
// -----------------------------------------------------------------------
THREAD_P class CostWeight* DefaultCostWeight = NULL;
THREAD_P class PerformanceGoal* DefaultPerformanceGoal  = NULL;
THREAD_P class PerformanceGoal* ResourcePerformanceGoal = NULL;

// -----------------------------------------------------------------------
// global function to initialize global cost variables
// -----------------------------------------------------------------------
NABoolean InitCostVariables()
{
  NABoolean rtnStatus = TRUE;      // assume the best

  if (DefaultCostWeight == NULL &&
      (DefaultCostWeight = new ResourceConsumptionWeight(1,
						     1,
						     1,
						     0,
						     0
  ...
  if (DefaultPerformanceGoal)
    {
      // Free the previous performance goal so there won't be
      // a leak in the global heap.
      delete DefaultPerformanceGoal;
      DefaultPerformanceGoal = NULL;
    }

  if (goalTok == DF_LASTROW)
    {
      DefaultPerformanceGoal = new OptimizeForLastRow();
    }
// LCOV_EXCL_START
  else if (goalTok == DF_FIRSTROW)
    {
      DefaultPerformanceGoal = new OptimizeForFirstRow();
    }
  else if (goalTok == DF_RESOURCES)
    {
      DefaultPerformanceGoal = new OptimizeForResourceConsumption();
    }
  else
    {
      // Unknown token.
...
// LCOV_EXCL_STOP
  ...
  if (ResourcePerformanceGoal == NULL)
    {
      // This one is a hardcoded performance goal, no consulting defaults
      ResourcePerformanceGoal = new OptimizeForResourceConsumption();
    }
...
{
  if (DefaultCostWeight != NULL)
    delete DefaultCostWeight;
  if (DefaultPerformanceGoal != NULL)
    delete DefaultPerformanceGoal;
  if (ResourcePerformanceGoal != NULL)
    delete ResourcePerformanceGoal;

  DefaultCostWeight = NULL;
  DefaultPerformanceGoal = NULL;
  ResourcePerformanceGoal = NULL;
}
