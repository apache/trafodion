		     ...
		     ValueIdSet & setOfPredicates)
{
  ...
  ValueIdSet genCCPredicates;       // generated preds for computed columns
  ...
  // if setOfKeyPredicates contains refereces to columns that we have 
  // a computed columns on, add the expression for the computed column to.
  // (this is enabled by default, turn off for troubleshooting)
  if (CmpCommon::getDefault(MTD_GENERATE_CC_PREDS) == DF_ON) {
    addComputedColumnPredicates(setOfKeyPredicates, genCCPredicates);
    setComputedKeyPredicates(genCCPredicates);
  }
  ...
  setOfPredicates -= genCCPredicates; // use those only for key preds
					 ...
					 CollIndex disjunctNumber) const
{
  ...
  // include the predicate on diving columns, if necessary
  if (CmpCommon::getDefault(MTD_GENERATE_CC_PREDS) == DF_ON) {
    commonPreds += getComputedKeyPredicates();
  }
...
//be changed because the signature was also changed in
//the parent class ScanKey
void SearchKey::addComputedColumnPredicates(ValueIdSet &predicates,          /* in/out */
...
const
{
  ...
  ValueIdSet keyCols(getKeyColumns());
                  ...
                  for (ValueId p = potentialPredicates.init();
                       ...
                       potentialPredicates.advance(p))
                    {
                      if (p.getItemExpr()->getOperatorType() != ITM_VEG_PREDICATE &&
                          p.getItemExpr()->getOperatorType() != ITM_EQUAL)
                        potentialPredicates -= p;
                    }
                }
                   ...
                   keyColsReferencedByCompExpr.advance(u))
                {
                  ...
                  for (ValueId p=potentialPredicates.init();
                       ...
                       potentialPredicates.advance(p))
                    {
                      ...
                      ValueId dummy;
                      if (isAKeyPredicateForColumn(p,keyValueExpr,dummy,u))
                        {
