...
void CreateImplementationRules(RuleSet* set)
{
  ...
  r = new(CmpCommon::contextHeap()) DP2DeleteRule
    ("DP2 Delete Implementation rule",
     new(CmpCommon::contextHeap())
       Delete(anyCorrName,
              NULL,
              REL_UNARY_DELETE,
  ...
  r = new(CmpCommon::contextHeap()) DP2UpdateRule
    ("DP2 Update Implementation rule",
     new(CmpCommon::contextHeap())
       Update(anyCorrName,
              NULL,
              REL_UNARY_UPDATE,
  ...
  r = new(CmpCommon::contextHeap()) DP2InsertCursorRule
    ("Implementation rule for Insert",
     new(CmpCommon::contextHeap())
       Insert(anyCorrName,
              NULL,
              REL_LEAF_INSERT,
  ...
  r = new(CmpCommon::contextHeap()) DP2DeleteCursorRule
    ("Impl: delete using a cursor",
     new(CmpCommon::contextHeap())
       Delete(anyCorrName,
              NULL,
              REL_LEAF_DELETE,
  ...
  r = new(CmpCommon::contextHeap()) DP2UpdateCursorRule
    ("Impl: Update using a cursor",
     new(CmpCommon::contextHeap())
       Update(anyCorrName,
              NULL,
              REL_LEAF_UPDATE,
  ...
  r = new(CmpCommon::contextHeap()) PhysicalParallelLabelCreateRule 
    ("Implement ParallelLabelCreate by a PhysicalParallelLabelCreate",
     new(CmpCommon::contextHeap())
                   ParallelLabelCreate(CmpCommon::contextHeap()),
     new(CmpCommon::contextHeap())
                   PhysicalParallelLabelCreate(CmpCommon::contextHeap())
    );
  set->insert(r);
  set->enable(r->getNumber());

  r = new(CmpCommon::contextHeap()) PhysicalParallelLabelDropRule
    ("Implement ParallelLabelDrop by a PhysicalParallelLabelDrop",
     new(CmpCommon::contextHeap())
                   ParallelLabelDrop(CmpCommon::contextHeap()),
     new(CmpCommon::contextHeap())
                   PhysicalParallelLabelDrop(CmpCommon::contextHeap())
    );
  set->insert(r);
  set->enable(r->getNumber());

  r = new(CmpCommon::contextHeap()) PhysicalParallelLabelAlterRule 
    ("Implement ParallelLabelAlter by a PhysicalParallelLabelAlter",
     new(CmpCommon::contextHeap())
                   ParallelLabelAlter(CmpCommon::contextHeap()),
     new(CmpCommon::contextHeap())
                   PhysicalParallelLabelAlter(CmpCommon::contextHeap())
    );
  set->insert(r);
  set->enable(r->getNumber());

  r = new(CmpCommon::contextHeap()) PhysicalParallelLabelPurgedataRule 
    ("Implement ParallelLabelPurgedata by a PhysicalParallelLabelPurgedata",
     new(CmpCommon::contextHeap())
                   ParallelLabelPurgedata(CmpCommon::contextHeap()),
     new(CmpCommon::contextHeap())
                   PhysicalParallelLabelPurgedata(CmpCommon::contextHeap())
    );
  set->insert(r);
  set->enable(r->getNumber());
...
// -----------------------------------------------------------------------
// methods for class DP2UpdateRule
// -----------------------------------------------------------------------

DP2UpdateRule::~DP2UpdateRule() {} // LCOV_EXCL_LINE 

NABoolean DP2UpdateRule::topMatch(RelExpr * relExpr, Context *context)
{
    // QSTUFF
    // in case we find the generic update root with a selection predicate
    // we force a cursor update to find a place for the selection
    // predicate at the join operator generated by the transformation rules;
    // the selection expression would disappear if assigned to a subset
    // operator; we assume that this is a rare case and that in future all
    // those joins are executed in the disk process - optimized transparently
    // into subset operations during code generation or by implementation
    // rules, or through support of generic update selection predicates
    // by the subset operator.  All that is important to distinguish
    // between the following two types of queries:
    // select * from (delete from x) y where x > 1; vs
    // select * from (delete from x where x > 1) y;
    ...
    // DP2Updates can only execute in DP2.
    if (NOT context->getReqdPhysicalProperty()->executeInDP2())
      return FALSE;
...
RelExpr * DP2UpdateRule::nextSubstitute(RelExpr * before,
                                        // QSTUFF
                                        Context * context,
                                        // QSTUFF
                                        RuleSubstituteMemory *& /*memory*/)
{
  ...
  // Also, return if this is an Update Where Current Of,
  // since subset/unique update does not work with that currently.
  // However, there exists runtime support to do Updc with subset/unique.
  // Need to remove this check, test and fix if it doesn't work. TBD.
  //  if (bef->updateCurrentOf())
  //    return NULL;

  const IndexDesc * CIdesc = scan->getTableDesc()->getClusteringIndex();
  const IndexDesc * UIdesc = bef->getTableDesc()->getClusteringIndex();
  ...
  if (before->getGroupAttr()->isStream() &&
      CIdesc->isPartitioned() &&
      CmpCommon::getDefault(ATTEMPT_ASYNCHRONOUS_ACCESS) == DF_OFF) {
    if(!((*CmpCommon::diags()).containsForFile(4320,
          ((CIdesc->getNAFileSet()->getExtFileSetName()).data()))))
      *CmpCommon::diags() << DgSqlCode(4320)
             << DgTableName(CIdesc->getNAFileSet()->getExtFileSetName());
      return NULL;
  }

  ValueIdSet nonKeyPredicates (scan->selectionPred());
  ValueIdSet nonPartKeyPredicates (scan->selectionPred());
  ValueIdSet externalInputs = bef->getGroupAttr()->getCharacteristicInputs();

  // From the selection predicates on the scan, divide predicates into
  // key predicates (on the primary index) vs. non key predicates.
  ValueIdSet nonKeyColumnSet;
  CIdesc->getNonKeyColumnSet(nonKeyColumnSet);
  SearchKey * skey = new(CmpCommon::statementHeap())
    SearchKey (CIdesc->getIndexKey(),
               CIdesc->getOrderOfKeyValues(),
               externalInputs, TRUE,
               nonKeyPredicates,
               nonKeyColumnSet,
               CIdesc,
  ...
  // QSTUFF
  OrderComparison cc = SAME_ORDER;
  if (context->requiresOrder()) {
    if (before->getGroupAttr()->isStream() && CIdesc->isPartitioned()) {
      // 10-010109-0583 "select ... from stream ...order by" causes hang
      // if stream is partn'd."
    ...
    ValueIdList sortKey = CIdesc->getOrderOfKeyValues();

    // Fix for solution 10-101006-3619
    // make sure we don't dereference a NULL pointer
    if (context->getReqdPhysicalProperty()->getSortKey() != NULL) {
      cc = sortKey.satisfiesReqdOrder(
                 *context->getReqdPhysicalProperty()->getSortKey(),
                  before->getGroupAttr());
    }

    if (cc == DIFFERENT_ORDER)  return FALSE;

    if (context->getReqdPhysicalProperty()->getArrangedCols() AND
        (sortKey.satisfiesReqdArrangement(
         *context->getReqdPhysicalProperty()->getArrangedCols()) == FALSE))
    {
      return FALSE;
    }
  ...
  if (skey->isUnique())
    opType = REL_DP2_UPDATE_UNIQUE;
  else
  {
    // Due to a restriction from DP2, update subset is not allowed for
    // entry sequenced files.

    // No longer needed.
    if (CIdesc->getNAFileSet()->isEntrySequenced())
      return NULL;

    opType = REL_DP2_UPDATE_SUBSET;
  }

  // Build the new DP2Update node
  result = new(CmpCommon::statementHeap())
    DP2Update(bef->getTableName(),
              bef->getTableDesc(),
              opType);

  // QSTUFF
  result->setReverseScan(cc == INVERSE_ORDER);
  // QSTUFF

  copyCommonGenericUpdateFields(result, bef);
  copyCommonUpdateFields(result, bef);

  // ## Next we undo one assignment that the preceding function did.
  // ## I assume SearchKey and BeginKey are supposed to be mutually exclusive?
  // ## ?? The better fix might be for GenericUpdate::preCodeGen to
  // ## unconditionally call begin/endKeyPred_.replaceVEGExpressions()
  // ## (currently it only does so if no SearchKey).
  result->beginKeyPred().clear();
  result->setSearchKey(skey);
  if (CIdesc->isPartitioned())
    {
      // build a search key for the partitioning key
      ValueIdSet dummySet;
      SearchKey * partKey = new(CmpCommon::statementHeap())
        SearchKey (CIdesc->getPartitioningKey(),
                   CIdesc->getOrderOfPartitioningKeyValues(),
                   externalInputs,
                   TRUE,
                   nonPartKeyPredicates,
                   dummySet, CIdesc, UIdesc);

      result->setPartKey(partKey);
    }
  result->executorPred() += nonKeyPredicates;
  result->setFirstNRows(scan->getFirstNRows());
  return result;
}

// -----------------------------------------------------------------------
// methods for class DP2DeleteRule
// -----------------------------------------------------------------------

DP2DeleteRule::~DP2DeleteRule() {} // LCOV_EXCL_LINE 

NABoolean DP2DeleteRule::topMatch(RelExpr * relExpr, Context *context)
{
  // QSTUFF
  // please see comments for DP2UpdateRule
  if (NOT ((GenericUpdate *)relExpr)->selectionPred().isEmpty())
    return FALSE;
    // QSTUFF
  if (NOT Rule::topMatch(relExpr,context))
  ...
  // DP2Deletes can only execute in DP2.
  if (NOT context->getReqdPhysicalProperty()->executeInDP2())
    return FALSE;
...
RelExpr * DP2DeleteRule::nextSubstitute(RelExpr * before,
                                        // QSTUFF
                                        Context * context,
                                        // QSTUFF
                                        RuleSubstituteMemory *& /*memory*/)
{
  ...
  const IndexDesc * CIdesc = scan->getTableDesc()->getClusteringIndex();
  const IndexDesc * DIdesc = bef->getTableDesc()->getClusteringIndex();
  ...
  OperatorTypeEnum opType;
  if (skey->isUnique())
    opType = REL_DP2_DELETE_UNIQUE;
  else
  {
    // The DELETE RANGE option will be selected if:
    // 1) No index maintenance needs to be performed.
    // 2) No column values need to be returned (this check is done
    //    in anticipation of the support for DELETE RETURNING statement).
    // 3) There are no executor predicates OR
    //    a define exists to force range delete.  The latter define exists
    //    to test the performance impact of range delete w/ executor preds.
    //    After testing, we hope to determine the selectivity threshold of
    //    when range delete should be chosen in the presence of predicates.

//  if (scan->getTableDesc()->getIndexes().entries() == 1 &&
//      bef->getGroupAttr()->getCharacteristicOutputs().isEmpty() &&
//      (nonKeyPredicates.isEmpty() || getenv("FORCE_DELETE_RANGE")))
//    opType = REL_DP2_DELETE_RANGE;
//  else
      opType = REL_DP2_DELETE_SUBSET;
  }

  // Build the new DP2Delete node
  result = new(CmpCommon::statementHeap())
    DP2Delete(bef->getTableName(), bef->getTableDesc(), opType);
  ...
  // copy fast delete fields
  result->setIsFastDelete(bef->isFastDelete());
...
// -----------------------------------------------------------------------
// methods for class DP2InsertCursorRule
// -----------------------------------------------------------------------

DP2InsertCursorRule::~DP2InsertCursorRule() {} // LCOV_EXCL_LINE 

NABoolean DP2InsertCursorRule::topMatch(RelExpr * relExpr, Context *context)
{
  if (NOT Rule::topMatch(relExpr,context))
    return FALSE;

  // DP2Insert can only execute in DP2.
  if (NOT context->getReqdPhysicalProperty()->executeInDP2())
    return FALSE;
...
RelExpr * DP2InsertCursorRule::nextSubstitute(RelExpr * before,
                                              Context * context,
                                              RuleSubstituteMemory *& /*mem*/)
{
  ...
  // -- MVs.
  // The following constant values are heuristic parameters.
  // They should be tuned by using performance tests.
  const Lng32 VSBB_USER_THRESHOLD      = 1000;
  const Lng32 VSBB_SYSTEM_THRESHOLD    = 1000;
  const Lng32 VSBB_ANYWHERE_THRESHOLD  =    2;
  ...
  // UPD_ORDERED is set for Import operations. Since the import stored
  // procedure does not pass order and cardinality information to the
  // optimizer, consider the data to be large and sorted.
  NABoolean vsbbTestMode =
    (CmpCommon::getDefault(VSBB_TEST_MODE) == DF_ON);
  NABoolean orderedImportOperation =
  ...
  result = new(CmpCommon::statementHeap()) DP2Insert(bef);
  ...
  result->enableAqrWnrEmpty()    = bef->enableAqrWnrEmpty();

  if (result->getGroupAttr()->isEmbeddedInsert())
    result->executorPred() += bef->selectionPred();

  // ---------------------------------------------------------------------
  // Determine whether some flavor of VSBB insert can be used
  // ---------------------------------------------------------------------

  NABoolean isEntrySequencedTable =
    result->getTableDesc()->getClusteringIndex()->getNAFileSet()->isEntrySequenced();

  Insert::InsertType itype = bef->getInsertType();
  Insert::InsertType origItype = bef->getInsertType();


  // Special syntax is used to specify the InsertType -
  // insert using SIDETREES/VSBB into....
  // If the InsertType is set using the special syntax
  // given above, then the itype is set to the one
  // specified in the syntax.
  // If the InsertType is not set then, the value
  // will be SIMPLE_INSERT, since its the default values.
  // We look into the the defaults table to see if the
  // user has any preferences.

  if (itype == Insert::SIMPLE_INSERT)
    {
      // get an opinion from the defaults table
      DefaultToken vsbbTok = CmpCommon::getDefault(INSERT_VSBB);

      if (vsbbTok == DF_OFF)
	itype = Insert::VSBB_INSERT_ANYWHERE;
      else if (vsbbTok == DF_SYSTEM)
        itype = Insert::VSBB_INSERT_SYSTEM;
      else if (vsbbTok == DF_USER)
        itype = Insert::VSBB_INSERT_USER;
      else if (vsbbTok == DF_LOADNODUP)
        itype = Insert::VSBB_LOAD_NO_DUP_KEY_CHECK;
    }

  if (CmpCommon::getDefault(DISABLE_BUFFERED_INSERTS) == DF_ON)
    {
      itype = Insert::SIMPLE_INSERT;
    }

  if ((itype == Insert::VSBB_INSERT_SYSTEM) && 
      ((numberOfRowsToBeInserted <  MIN_ROWS_FOR_VSBB))) 
    itype = Insert::SIMPLE_INSERT;

  // Blocksize must be 4K to do sidetree insert. This is a DP2 restriction.
  // Use VSBB insert, if blocksize is not 4K.
  //  if (itype == Insert::VSBB_LOAD AND
  //     result->getTableDesc()->getClusteringIndex()->getNAFileSet()->
  //      getBlockSize() != 4096)
  //   itype = Insert::VSBB_INSERT_SYSTEM;

  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  //  Please read this note when you update logic for VSBB_LOAD (side
  // tree insert). The same logic has been used in the method
  // Insert::isSideTreeInsertFeasible() in RelExpr.cpp.
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  if (OSIM_isNTbehavior() && !OSIM_runningSimulation())
  {
  // Utilities assume that VSBB insert will be chosen, even if sidetree
  // insert is specified, if the blocksize is not 4K or this is an audited
  // table.This should be fixed in utilities by not issuing 'using sideinserts'
  // syntax in these conditions. Or we may add a CQD that would do this
  // automatically out here.
  // For external users who are using the 'using sideinserts' syntax,
  // an error will be returned.
  if ((((itype == Insert::VSBB_LOAD) ||
	(itype == Insert::VSBB_LOAD_AUDITED)) &&
       (result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	->getBlockSize() < 4096)) ||
      ((itype == Insert::VSBB_LOAD) &&
       ((result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	 ->isAudited()))))
    {
      itype = Insert::VSBB_INSERT_SYSTEM;
  ...
  //
  // Note: due to an existing bug somewhere else(readtabledef?
  // NATable?), the blocksize is not being set correctly. It is always being
  // set to 4K. For this case, the error will not be returned out here but
  // will be returned at runtime from DP2(error 1181).
  if (((itype == Insert::VSBB_LOAD) ||
      (itype == Insert::VSBB_LOAD_USER_SIDEINSERTS) ||
      (itype == Insert::VSBB_LOAD_AUDITED)))
    {
      if ((result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->isCompressed()) ||
	  (result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->getBlockSize() < 4096) ||
	  (NOT result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->isKeySequenced()) ||
	  ((itype == Insert::VSBB_LOAD) &&
	   (result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->isAudited())))
	{
	  NAString errCond("", CmpCommon::statementHeap());
	  if (result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	       ->isCompressed())
	    errCond.append("on compressed tables");
	  else if (result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->getBlockSize() < 4096)
	    errCond.append("on tables with blocksize not equal to 4K");
	  else if (NOT result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->isKeySequenced())
	    errCond.append("on entry sequenced tables");
	  else if (result->getTableDesc()->getClusteringIndex()->getNAFileSet()
	   ->isAudited())
	    errCond.append("on audited tables");

	  // sometimes optimizer drives this rule multiple times even in case
	  // of an error(for ex, if default ARKCMP_FAKE_HW is set).
	  // Do not add this error, if it has already been added.
	  if (CmpCommon::diags()->mainSQLCODE() != -4034)
	    *CmpCommon::diags() << DgSqlCode(-4034)
				<< DgString0("insert")
				<< DgString1("using")
				<< DgString2("sideinserts")
				<< DgString3(errCond);
  ...
  // VSBB insert can't be used if it is not allowed or if
  // index maintenance needs to be done or for vertically
  // partitioned tables or if the number of PAs would
  // exceed the maximum allowed or if INSERT is part of CS
  // and the whole CompoundStmt is being pushed into DP2.
  // Override defaults table.
  if (NOT bef->bufferedInsertsAllowed() OR
      NOT bef->getTableDesc()->getVerticalPartitions().isEmpty() OR    
      ((insPartFunc != NULL) AND
       (insPartFunc->getCountOfPartitions() >
        getDefaultAsLong(MAX_ACCESS_NODES_PER_ESP))) OR
      (context AND context->getReqdPhysicalProperty()->getPushDownRequirement()))
    {
      itype = Insert::SIMPLE_INSERT;
    }

  // MV --.
  if (bef->getInliningInfo().hasPipelinedActions())
  {
    // there are pipelined actions (RI, IM, triggers or MV logging).

    if (isEntrySequencedTable || (bef->getInliningInfo().isEffectiveGU()) ||
	 (bef->getTolerateNonFatalError() == RelExpr::NOT_ATOMIC_) )
    {
      // Outputs are not supported from VSBB/SideInsert nodes on Entry
      // Sequenced tables.
      itype = Insert::SIMPLE_INSERT;
    }
    else
      switch (itype)
      {
	case Insert::VSBB_INSERT_SYSTEM:
	case Insert::VSBB_INSERT_USER:
	  // VSBB range logging for the MV log.
	  // Do we need to project the mid-range rows for IM, RI or triggers?
	  if (bef->getInliningInfo().isProjectMidRangeRows())
  	    result->setProjectMidRangeRows();

	  if (vsbbTestMode || orderedImportOperation)
	  {
	    // In VSBB test mode, we don't make any more conditions,
	    // except that range logging is required.
            if (rangeLoggingRequired)
	  ...
	  {
	    // Normal mode - Make sure the data is sorted and large enough,
	    // and that range logging is required.
	    NABoolean dataIsSorted = bef->isDataSorted(bef, context);

	    Lng32 threshold = (itype == Insert::VSBB_INSERT_USER) ?
			     VSBB_USER_THRESHOLD                 :
	                     VSBB_SYSTEM_THRESHOLD;
	    NABoolean dataIsOverThreshold =
	      (numberOfRowsToBeInserted >= threshold);

	    if (!dataIsSorted        ||
	        !dataIsOverThreshold ||
		!rangeLoggingRequired )
              itype = Insert::VSBB_INSERT_ANYWHERE;
	    else
	      result->setRequireStrongRangeLock();
	  }
	...
	case Insert::VSBB_LOAD:
	case Insert::VSBB_LOAD_USER_SIDEINSERTS:
	case Insert::VSBB_LOAD_AUDITED:
	case Insert::VSBB_LOAD_APPEND:
	case Insert::VSBB_LOAD_NO_DUP_KEY_CHECK:
	case Insert::VSBB_LOAD_APPEND_NO_DUP_KEY_CHECK:
	  ...
	  // Also, if automatic range logging is disabled, the SideInsert node
	  // does not support single row logging (as does the VSBB node), so
	  // the Simple insert will be used instead.
          if (!rangeLoggingRequired)
	    itype = Insert::SIMPLE_INSERT;

	  break;

	case Insert::VSBB_INSERT_ANYWHERE:
	  // If not in VSBB test mode, use single inserts for a very
	  // small number of rows.
	  if (!vsbbTestMode &&
	      (numberOfRowsToBeInserted < VSBB_ANYWHERE_THRESHOLD))
	    itype = Insert::SIMPLE_INSERT;
	  break;
  ...
  // Implement InsertAnywhere as simple insert until a new TCB class will
  // be written. 
  if (itype == Insert::VSBB_INSERT_ANYWHERE)
    itype = Insert::SIMPLE_INSERT;

  if ((origItype == Insert::VSBB_LOAD_USER_SIDEINSERTS) &&
      (itype != Insert::VSBB_LOAD_USER_SIDEINSERTS))
    {
      *CmpCommon::diags() << DgSqlCode(-4034)
			  << DgString0("insert")
  ...
  DP2VSBBInsert *vsbbInsert;
  DP2SideTreeInsert *sideTreeInsert;
  switch (itype)
  {
    case Insert::SIMPLE_INSERT:
       break;
    case Insert::VSBB_INSERT_SYSTEM:
    case Insert::VSBB_INSERT_USER:
      {
	vsbbInsert = new (CmpCommon::statementHeap())
          DP2VSBBInsert(result) ;
	
	copyCommonGenericUpdateFields(vsbbInsert, result);
	
	vsbbInsert->setPartKey(skey);
	vsbbInsert->rrKeyExpr()              = result->rrKeyExpr();
	vsbbInsert->rowPosInput()            = result->rowPosInput();
	vsbbInsert->partNumInput()           = result->partNumInput();
	vsbbInsert->totalNumPartsInput()     = result->totalNumPartsInput();
	vsbbInsert->reqdOrder()              = result->reqdOrder();
	vsbbInsert->setNoBeginCommitSTInsert
	  (result->noBeginSTInsert(), result->noCommitSTInsert());
	
	vsbbInsert->enableTransformToSTI() = result->enableTransformToSTI();
        vsbbInsert->enableAqrWnrEmpty()    = result->enableAqrWnrEmpty();
	
	Lng32 v = getDefaultAsLong(TRANSFORM_TO_SIDETREE_INSERT_MIN_ROWS);
	if ((vsbbInsert->enableTransformToSTI()) &&
	    ((v > 0) && (numberOfRowsToBeInserted <  v)))
	  vsbbInsert->enableTransformToSTI() = FALSE;
	
	if (result->getGroupAttr()->isEmbeddedInsert())
	  vsbbInsert->executorPred() += result->selectionPred();
	
	
	/*if (result->getProjectMidRangeRows())
          vsbbInsert->setProjectMidRangeRows();
	  
	  if (result->getRequireStrongRangeLock())
	  vsbbInsert->setRequireStrongRangeLock();*/
    ...
    case Insert::VSBB_LOAD_NO_DUP_KEY_CHECK:
    case Insert::VSBB_LOAD_APPEND_NO_DUP_KEY_CHECK:
       sideTreeInsert = new (CmpCommon::statementHeap())
           DP2SideTreeInsert(result);

       copyCommonGenericUpdateFields(sideTreeInsert, result);

       sideTreeInsert->setPartKey(skey);
       sideTreeInsert->rrKeyExpr()              = result->rrKeyExpr();
       sideTreeInsert->rowPosInput()            = result->rowPosInput();
       sideTreeInsert->partNumInput()           = result->partNumInput();
       sideTreeInsert->totalNumPartsInput()     = result->totalNumPartsInput();
       sideTreeInsert->reqdOrder()              = result->reqdOrder();
       sideTreeInsert->setNoBeginCommitSTInsert
	 (result->noBeginSTInsert(), result->noCommitSTInsert());
       //       sideTreeInsert->enableTransformToSTI() = result->enableTransformToSTI();

       if (result->getGroupAttr()->isEmbeddedInsert())
	 sideTreeInsert->executorPred() += result->selectionPred();

       result=sideTreeInsert;
       break;
    default:
       break;
  }

  result->setInsertType(itype);
...
// -----------------------------------------------------------------------
// methods for class DP2DeleteCursorRule
// -----------------------------------------------------------------------

DP2DeleteCursorRule::~DP2DeleteCursorRule() {} // LCOV_EXCL_LINE 

NABoolean DP2DeleteCursorRule::topMatch(RelExpr * relExpr, Context *context)
{
  if (NOT Rule::topMatch(relExpr,context))
    return FALSE;
  ...
  CMPASSERT(relExpr->getOperatorType() == REL_LEAF_DELETE);
  Delete * delNode = (Delete *) relExpr;
  // if a merge stmt has an insert clause do not choose a cursor_delete
  // the no match will not cause a row to be inserted as expected.
  if (delNode->isMerge())
    return FALSE;
...
RelExpr * DP2DeleteCursorRule::nextSubstitute(RelExpr * before,
                                              Context * /*context*/,
                                              RuleSubstituteMemory *& /*mem*/)
{
  CMPASSERT(before->getOperatorType() == REL_LEAF_DELETE);
  CMPASSERT(before->getArity() == 0);
  ...
  result = new(CmpCommon::statementHeap())
    DP2Delete(bef->getTableName(), bef->getTableDesc(), REL_DP2_DELETE_CURSOR);
  ...
  // copy fast delete fields
//  result->setIsFastDeletePossible(bef->isFastDeletePossible());
  result->setIsFastDelete(bef->isFastDelete());

  // in case of a cursor delete the scan index of the delete table
  // cannot be anymore the index of the scan table but must be
  // the index of the delete table, we therefore reset the scan Index
  // of the delete table to its own clustering keys.. and see what happens
  //...for subset deletes this is not an issue
  result->setScanIndexDesc(bef->getTableDesc()->getClusteringIndex());
  // QSTUFF

  // If the table to be deleted from is partitioned, add partitioning key
  // predicates so that the executor will only access those partitions
  // that actually need to be deleted from.
  const IndexDesc * CIdesc = result->getTableDesc()->getClusteringIndex();
  if (CIdesc->isPartitioned())
  {
    // build a search key for the partitioning key
    // The partitioning key predicates are taken from the begin key of
    // the delete node (which was set up by the binder).
    ValueIdSet tempKeyPreds(result->beginKeyPred());
    ValueIdSet dummySet;
    ValueIdSet externalInputs = result->getGroupAttr()->
...
// -----------------------------------------------------------------------
// methods for class DP2UpdateCursorRule
// -----------------------------------------------------------------------

DP2UpdateCursorRule::~DP2UpdateCursorRule() {} // LCOV_EXCL_LINE 

NABoolean DP2UpdateCursorRule::topMatch(RelExpr * relExpr, Context *context)
{
  if (NOT Rule::topMatch(relExpr,context))
    return FALSE;
  ...
  CMPASSERT(relExpr->getOperatorType() == REL_LEAF_UPDATE);
  Update * updNode = (Update *) relExpr;
  // if a merge stmt has an insert clause do not choose a cursor_update
  // the no match will not cause a row to be inserted as expected.
  if (updNode->isMerge())
    return FALSE;
...
RelExpr * DP2UpdateCursorRule::nextSubstitute(RelExpr * before,
                                              Context * /*context*/,
                                              RuleSubstituteMemory *& /*memory*/)
{
  CMPASSERT(before->getOperatorType() == REL_LEAF_UPDATE);
  CMPASSERT(before->getArity() == 0);
  ...
  result = new(CmpCommon::statementHeap())
    DP2Update(bef->getTableName(), bef->getTableDesc(), REL_DP2_UPDATE_CURSOR);

  copyCommonGenericUpdateFields(result, bef);
  copyCommonUpdateFields(result, bef);

  // If the table to be updated is partitioned, add partitioning key
  // predicates so that the executor will only access those partitions
  // that actually need to be updated.
  const IndexDesc * CIdesc = result->getTableDesc()->getClusteringIndex();
  if (CIdesc->isPartitioned())
  {
    // build a search key for the partitioning key
    // The partitioning key predicates are taken from the begin key of
    // the update node (which was set up by the binder).
    ValueIdSet tempKeyPreds(result->beginKeyPred());
    ValueIdSet dummySet;
    ValueIdSet externalInputs = result->getGroupAttr()->
                                  getCharacteristicInputs();
...
// -----------------------------------------------------------------------
// methods for class PhysicalParallelLabelCreateRule
// -----------------------------------------------------------------------
PhysicalParallelLabelCreateRule::~PhysicalParallelLabelCreateRule() {} // LCOV_EXCL_LINE 

NABoolean PhysicalParallelLabelCreateRule::topMatch(RelExpr *relExpr,
                                                    Context *context)
{
   if (NOT Rule::topMatch(relExpr,context))
      return FALSE;
...
RelExpr * PhysicalParallelLabelCreateRule::nextSubstitute
                                              (RelExpr *before,
                                               Context *context,
                                               RuleSubstituteMemory * &memory)
{
   RelExpr *result;
   result = new (CmpCommon::statementHeap())
                     PhysicalParallelLabelCreate(*((PhysicalParallelLabelCreate*)before));
...
// -----------------------------------------------------------------------
// methods for class PhysicalParallelLabelDropRule
// -----------------------------------------------------------------------
PhysicalParallelLabelDropRule::~PhysicalParallelLabelDropRule() {} // LCOV_EXCL_LINE 

NABoolean PhysicalParallelLabelDropRule::topMatch(RelExpr *relExpr,
                                                  Context *context)
{
   if (NOT Rule::topMatch(relExpr,context))
      return FALSE;
...
RelExpr * PhysicalParallelLabelDropRule::nextSubstitute
                                              (RelExpr *before,
                                               Context *context,
                                               RuleSubstituteMemory * &memory)
{
   RelExpr *result;
   result = new (CmpCommon::statementHeap())
                     PhysicalParallelLabelDrop(*((PhysicalParallelLabelDrop*)before));
...
// -----------------------------------------------------------------------
// methods for class PhysicalParallelLabelAlterRule
// -----------------------------------------------------------------------
PhysicalParallelLabelAlterRule::~PhysicalParallelLabelAlterRule() {} // LCOV_EXCL_LINE 

NABoolean PhysicalParallelLabelAlterRule::topMatch(RelExpr *relExpr,
                                                   Context *context)
{
   if (NOT Rule::topMatch(relExpr,context))
      return FALSE;
...
RelExpr * PhysicalParallelLabelAlterRule::nextSubstitute
                                              (RelExpr *before,
                                               Context *context,
                                               RuleSubstituteMemory * &memory)
{
   RelExpr *result;
   result = new (CmpCommon::statementHeap())
                     PhysicalParallelLabelAlter(*((PhysicalParallelLabelAlter*)before));
...
// -----------------------------------------------------------------------
// methods for class PhysicalParallelLabelPurgedataRule
// -----------------------------------------------------------------------
PhysicalParallelLabelPurgedataRule::~PhysicalParallelLabelPurgedataRule() {} // LCOV_EXCL_LINE 

NABoolean PhysicalParallelLabelPurgedataRule::topMatch(RelExpr *relExpr,
                                                    Context *context)
{
   if (NOT Rule::topMatch(relExpr,context))
      return FALSE;
...
RelExpr * PhysicalParallelLabelPurgedataRule::nextSubstitute
                                              (RelExpr *before,
                                               Context *context,
                                               RuleSubstituteMemory * &memory)
{
   RelExpr *result;
   result = new (CmpCommon::statementHeap())
                     PhysicalParallelLabelPurgedata(*((PhysicalParallelLabelPurgedata*)before));
