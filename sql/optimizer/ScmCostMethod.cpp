...
// -----------------------------------------------------------------------
// CostMethodDP2Delete::scmComputeOperatorCostInternal().
// -----------------------------------------------------------------------
Cost*
CostMethodDP2Delete::scmComputeOperatorCostInternal(RelExpr* op,
						    const Context* myContext,
						    Lng32& countOfStreams)
{
  cacheParameters(op,myContext);
  ...
  if (ippForMe != NULL)  // input physical properties exist?
  {
    ...
    // to the same source column, example: Sol: 10-040416-5166, where we have
    // INSERT INTO b6table1
    //            ( SELECT f, h_to_f, f, 8.4
    //            FROM btre211
    //            );
    // Hence we use lists here instead of sets.
    // Check to see if there are any duplicates in the source Characteristics 
    // inputs
    ...
    if (((upOperator->getTableName().getSpecialType() == 
         ...
         (upOperator->getTableName().getSpecialType() ==
         ...
         (bottomValuesSet.entries() != bottomValues.entries() ) )
    {
      ...
      // columns of characteristics inputs, otherwise we shall use the
  ...
  CostScalar tuplesProcessed(csOne);
  CostScalar tuplesProduced(csOne);

  // ************************************************************
  // FIRST: Determine tuples processed and tuples produced.
  // ************************************************************
  ...
  if (delOp->getOperatorType() == REL_DP2_DELETE_CURSOR)
  {
    ...
    tuplesProcessed = tuplesProduced = noOfProbesPerPartition;
  }
...
// use subset delete instead, done as part of SQ SQL code cleanup effort
...
#endif // if 0
  {
    ...
    if ( NOT allKeyColumnsHaveHistogramStatistics( histograms, CIDesc ) )
    {
      ...
      tuplesProcessed = tuplesProduced;
    }
    else
    {
      tuplesProcessed = numRowsToScanWhenAllKeyColumnsHaveHistograms(
                          histograms,
                          keyPredsByCol,
                          activePartitions,
                          CIDesc
  ...
  else // REL_DP2_DELETE_UNIQUE
  {
    ...
    activeCpus = csOne;
    streamsPerCpu = csOne;
    tuplesProcessed = csOne;
    // assume the 1 row always satisfies any executor predicates so
    // we'll always do the Delete
    tuplesProduced = csOne;
  }

  CostScalar ioSeq(csZero);
  CostScalar ioRand(csZero);
  ...
  // ************************************************************
  // SECOND : Compute IO (random and sequential) COST
  // ************************************************************
  if (delOp->getOperatorType() == REL_DP2_DELETE_CURSOR)
  {
    ...
    if (probesInOrder)
    {
      ...
      seeks = (noOfProbesPerPartition / rowsPerBlock).getCeiling();
      ioRand = seeks * indexLevels + // io for index blocks
               seeks;                // io for data blocks
    }
    else
      // Each probe from left child of NJ_FLOW operator results 
      // in one random io. And you need to travel index blocks to access
      // data blocks.
      ioRand = noOfProbesPerPartition * indexLevels + // io for index blocks
               noOfProbesPerPartition; // io for data blocks
    
  }  // end REL_DP2_DELETE_CURSOR
  else // REL_DP2_DELETE_SUBSET, REL_DP2_DELETE_RANGE, REL_DP2_DELETE_UNIQUE
  {
    ioRand = indexLevels;
    ioSeq = (tuplesProcessed / rowsPerBlock).getCeiling();
  }
...
// -----------------------------------------------------------------------
// CostMethodDP2Update::scmComputeOperatorCostInternal().
// -----------------------------------------------------------------------
Cost*
CostMethodDP2Update::scmComputeOperatorCostInternal(RelExpr* op,
                                            const Context* myContext,
                                            Lng32& countOfStreams)
{
  cacheParameters(op,myContext);
  ...
  // For update subset, range, and unique, # of probes should always be 1,
  // so we won't bother computing # of probes per partition, we'll
  // just use the default (1).
  CostScalar noOfProbesPerPartition(csOne);
  ...
  return dp2UpdateCost;

} // CostMethodDP2Update::scmComputeOperatorCosInternal
...
//**************************************************************
Cost*
CostMethodDP2Insert::scmComputeOperatorCostInternal(RelExpr* op,
						    const Context* myContext,
						    Lng32& countOfStreams)
{
  DP2Insert* insOp = (DP2Insert *)op;   // downcast

  // Calls computeOperatorCostInternal() to compute most of the 
  // details, like number of inserts, number of IOs, etc.
  ...
  CostScalar ioRand = seeksLR_;
  CostScalar ioSeq = transferLR_;
  ...
  if ( printCost )
  {
    ...
    dp2InsertCost->getScmCplr().print(pfp);
    fprintf(pfp, "DP2Insert elapsed time: ");
    fprintf(pfp,"%f", dp2InsertCost->convertToElapsedTime(myContext->getReqdPhysicalProperty()).value());
    fprintf(pfp,"\n");
  }
#endif
  ...
  // a different NCM cost object.
  delete costPtr;

  return dp2InsertCost;
}  // CostMethodDP2Insert::scmComputeOperatorCostInternal()
