...
public:
 ...
 // set display on/off.
  ...
  inline NABoolean containsSG() const
  { return containsSG_; }

  inline void setContainsSG(NABoolean h)
  { containsSG_ = h; }
  ...
  inline NABoolean isUniqueSGInsertType() const
  { return isUniqueSGInsertType_; }

  inline void setIsUniqueSGInsertType(NABoolean h)
  { isUniqueSGInsertType_ = h; }
  ...
  // TRUE, if the statement uses an internal sequence generator
  NABoolean containsSG_;

  // TRUE, if query has mandatory cross products
  NABoolean hasMandatoryXP_;

  // TRUE, if the statement is a UNIQUE statement type.
  // Currently used for determining the INSERT query type
  // for statements with an internal sequence generator.
  NABoolean isUniqueSGInsertType_;
...
// -----------------------------------------------------------------------
// The PartitionAccess operator replaces the Exchange operator
// during the code generation phase. It drives the execution of
// a query plan frgament within DP2 and provides the interface
// for retrieving rows from DP2 to the executor.
// It can access one or more partitions sequentially. If the
// query plan requires an asynchronous access to many partitions,
// one PartitionAccess must be allocated for each partition, or
// set of partitions, that is to be accessed asynchronously.
// -----------------------------------------------------------------------
class PartitionAccess : public RelExpr
{
public:

  // constructors and desctructor
//warning elimination (removed "inline")
  PartitionAccess(const IndexDesc *indexDesc,
			 const SearchKey *partSearchKey,
			 RelExpr* child = NULL,
			 const PartitioningFunction* partFunc = NULL,
			 CollHeap *oHeap = CmpCommon::statementHeap())
    : RelExpr(REL_PARTITION_ACCESS, child, NULL, oHeap),
      indexDesc_(indexDesc),
      partSearchKey_(partSearchKey),
      partFunc_(partFunc),
      beginKeyIsExclusive_(FALSE),
      endKeyIsExclusive_(FALSE),
      beginKeyExclusionExpr_(NULL),
      endKeyExclusionExpr_(NULL),
      beginPartNoExpr_(NULL),
      endPartNoExpr_(NULL),
      beginPartSelection_(NULL),
      endPartSelection_(NULL),
      sequenceGenerationExpr_(NULL),
      sequenceInitExpr_(NULL),
      reversePartitionAccess_(FALSE),
      sidSpaceAlreadySetup_(FALSE),
      childIsIUD_(FALSE),
      paDp2AlignmentOpt_(FALSE)
  {}

  virtual ~PartitionAccess();

  const IndexDesc *getIndexDesc() const             { return indexDesc_; }
  void setIndexDesc(const IndexDesc * val)                {indexDesc_ = val;}
  ...
  void setBeginPartNoExpr(ItemExpr *x)           { beginPartNoExpr_ = x; }
  void setEndPartNoExpr(ItemExpr *x)               { endPartNoExpr_ = x; }

  void setBeginPartSelection(ItemExpr *x)     { beginPartSelection_ = x; }
  void setEndPartSelection(ItemExpr *x)         { endPartSelection_ = x; }

  void setSequenceGenerationExpr(ItemExpr *x)  { sequenceGenerationExpr_ = x;}
  void setSequenceInitExpr(ItemExpr *x)  { sequenceInitExpr_ = x;}

  void setReversePartitionAccess()      {reversePartitionAccess_ = TRUE; }
  void setSidSpaceAlreadySetup(NABoolean val)  {sidSpaceAlreadySetup_ = val;}
  NABoolean getSidSpaceAlreadySetup() {return sidSpaceAlreadySetup_;}


  const PartitioningFunction *getPartFunc() const    { return partFunc_; }

  const ItemExpr *getSequenceGenerationExpr() const
  {return sequenceGenerationExpr_;}

  const ItemExpr *getSequenceInitExpr() const
  {return sequenceInitExpr_;}

  const NABoolean doPaDp2AlignmentOpt() const { return paDp2AlignmentOpt_; }
  void setDoPaDp2AlignmentOpt(NABoolean v) { paDp2AlignmentOpt_ = v; }
  ...
  //returns true if all partitions are probed
  //returns false if only 1 partition is probed
  inline NABoolean allPartitionsProbed() 
  { 
    if (beginPartSelection_) 
      return FALSE; 
    return TRUE;
  }
...
private:
  ...
  void createSequenceGenerationExpr(ValueId &valId, BindWA* bindWA);
  ...
  // partitioning function for this node (this node gets introduced
  // during preCodeGen and doesn't have physical properties)
  ...
  ValueIdList inputExpr_;	// Input to Partition Access Node
  ValueIdList outputExpr_;	// Output of DP2_Root Node
  ...
  // ---------------------------------------------------------------------
  ItemExpr *beginKeyExclusionExpr_;
  ItemExpr *endKeyExclusionExpr_;

  // ---------------------------------------------------------------------
  // Expressions to calculate the partition numbers directly, without
  // going through begin and end key expressions
  // ---------------------------------------------------------------------
  ItemExpr *beginPartNoExpr_;
  ItemExpr *endPartNoExpr_;

  ItemExpr *beginPartSelection_;
  ItemExpr *endPartSelection_;

  ItemExpr *sequenceGenerationExpr_;
  ItemExpr *sequenceInitExpr_;

  // A list of inputs to the sequenceGenerationExpr.  This
  // list is populated when the sequenceGenerationExpr is
  // created.  The sequenceGenerationExpr needs two inputs:
  // the base value and a counter.
  //
  ValueIdList sequenceGenerationExprInputs_;

  // initial values for sequenceInitExpr_
  ValueIdList initValuesForSequenceInitExpr_;

  // ---------------------------------------------------------------------
  // Indicate if this PA should setup space for sid (System IDentity)
  // FALSE - setup space. TRUE - don't setup space. It has already been done
  // some other place(ex_split_top).
  // ---------------------------------------------------------------------
  NABoolean sidSpaceAlreadySetup_;

  // ---------------------------------------------------------------------
  // Indicate if the table/index partitions are accessed in reverse order
  // ---------------------------------------------------------------------
  NABoolean reversePartitionAccess_;

  // ---------------------------------------------------------------------
  // If set, do the optimization of aligning PA operator with dp2 datavol.
  // With this optimization, a PA node that communicates with a particular
  // DP2 once, will always communicate with that dp2. 
  // This optimization reduces the dp2 space usage by not sending sessions
  // to multiple dp2s.
  // This flag is set and used in SplitTop operator.
  // It is currently only set for inner tables of a nested join.
  // ---------------------------------------------------------------------
  NABoolean paDp2AlignmentOpt_;

  // Evaluate during PreCodegen whether any child operator is Generic
  // Update.
  NABoolean childIsIUD_;

}; // class PartitionAccess


// This is the base class for all the parallel label operations, including
// drop, create, and alter. This class can be used to implement other
// parallel label operations in the future.
class ParallelLabelOp : public RelExpr
{
   public:

   // Constructor
   ParallelLabelOp (const CorrName &objectName, NABoolean isParallel,
                    OperatorTypeEnum otype,
                    CollHeap *oHeap = CmpCommon::statementHeap()
                    );

   // Copy constructor
   ParallelLabelOp(const ParallelLabelOp &otherObj);

   // default constructor
   ParallelLabelOp(OperatorTypeEnum otype,
                   CollHeap *oHeap = CmpCommon::statementHeap()
                  );

   // destructor
   virtual ~ParallelLabelOp();
   ...
   // is this a logical or a physical node?
   virtual NABoolean isLogical() const = 0;
   virtual NABoolean isPhysical() const = 0;
   ...
   virtual HashValue topHash() = 0;
   virtual NABoolean duplicateMatch(const RelExpr & other) const = 0;

   // silence Coverity cid 809: bad_override "ParallelLabelOp::getTableName"
   // does not override "RelExpr::getTableNname" because type qualifiers
   // do not match.
   const CorrName & getTableName() const { return objectName_; }
   virtual CorrName &getTableName()   { return objectName_; }

   virtual void setTableDesc(TableDesc *tdesc) { tabId_ = tdesc; }

   virtual TableDesc *getTableDesc(void) const { return tabId_; }

   virtual NABoolean isParallelOp(void) const { return isParallelOp_; }

   virtual Int32 populateRtmdObject (RtmdCompileTimeObj *rtmd,
                                   Generator * generator);
   // the next 6 methods are used when values are to be returned
   // from a parallel label op.
   ...
   virtual void getPotentialOutputValues(ValueIdSet &vs) const;
   TableDesc * getVirtualTableDesc() const {return virtualTabId_;}
   void setVirtualTableDesc(TableDesc * vtdesc) {virtualTabId_ = vtdesc;}
   ...
   // a unique identifer for the table specified by objectName_
   TableDesc *tabId_;

   NABoolean isParallelOp_;
   ...
   // currently only used with label purgedata.
   OptSqlTableOpenInfo *stoi_;

};

// This class implements the node for creating labels and resource forks
// in parallel, i.e. using ESPs. This is used internally only, by
// the catalog manager, to solve the performance bottleneck of creating
// partitions of a table serially.
class ParallelLabelCreate : public ParallelLabelOp
{
   public:

   // Constructor
   ParallelLabelCreate(const CorrName &objectName,
                       NABoolean isParallel,
                       CollHeap *oHeap = CmpCommon::statementHeap());

   // Copy constructor
   ParallelLabelCreate(const ParallelLabelCreate &otherObj);

   // default constructor
   ParallelLabelCreate(CollHeap *oHeap = CmpCommon::statementHeap());

   // destructor
   virtual ~ParallelLabelCreate();
...
class PhysicalParallelLabelCreate: public ParallelLabelCreate
{
public:
  PhysicalParallelLabelCreate(CollHeap *oHeap = CmpCommon::statementHeap())
    : ParallelLabelCreate(oHeap)   {};
  PhysicalParallelLabelCreate(const ParallelLabelCreate& other) : ParallelLabelCreate(other)     {};
...
}; // class PhysicalParallelLabelCreate

// This class implements the node for dropping labels and resource forks
// in parallel, i.e. using ESPs. This is used internally only, by
// the catalog manager, to solve the performance bottleneck of dropping
// partitions of a table serially.
class ParallelLabelDrop: public ParallelLabelOp
{
   public:

   // Constructor
   ParallelLabelDrop(const CorrName &objectName, NABoolean isParallel,
                     CollHeap *oHeap = CmpCommon::statementHeap());

   // Copy constructor
   ParallelLabelDrop(const ParallelLabelDrop &otherObj);

   // default constructor
   ParallelLabelDrop(CollHeap *oHeap = CmpCommon::statementHeap());

   // destructor
   virtual ~ParallelLabelDrop();
...
class PhysicalParallelLabelDrop: public ParallelLabelDrop
{
public:
  PhysicalParallelLabelDrop(CollHeap *oHeap = CmpCommon::statementHeap())
    : ParallelLabelDrop(oHeap)   {};
  PhysicalParallelLabelDrop(const ParallelLabelDrop& other) : ParallelLabelDrop(other)     {};
  ...
  virtual short codeGen(Generator *generator);
}; // class PhysicalParallelLabelDrop

// This class implements the node for altering labels and resource forks
// in parallel, i.e. using ESPs. This is used internally only, by
// the catalog manager, to solve the performance bottleneck of altering
// label attributes in multiple partitions of a table serially.
// e.g. OSV(Object schema version) update when upgrading or downgrading
// the SQL/MX version.
class ParallelLabelAlter : public ParallelLabelOp
{
   public:

   // Constructor
   ParallelLabelAlter(const CorrName &objectName, // actual table/index name
		      NABoolean isParallel,	  // parallel or not
		      UInt32 opcode,	  // opcode to be altered
		      const NAString &newValue,   // new value for update
		      NABoolean isRfork = FALSE,  // TRUE, if alter is being
                                		  // done to resource fork
		      CollHeap *oHeap = CmpCommon::statementHeap());

   // Copy constructor
   ParallelLabelAlter(const ParallelLabelAlter &otherObj);

   // default constructor
   ParallelLabelAlter(CollHeap *oHeap = CmpCommon::statementHeap());

   // destructor
   virtual ~ParallelLabelAlter();
   ...
   // getters and setters
   virtual UInt32 getAltOpCode() const { return altOpCode; }
   virtual void setAltOpCode(UInt32 opc) { altOpCode = opc; }
   virtual const NAString getNewValue() const { return newValue; }
   virtual void setNewValue(const NAString &nv) { newValue = nv; }

   virtual NABoolean isRfork() const { return isRfork_; }
...
class PhysicalParallelLabelAlter: public ParallelLabelAlter
{
public:
  PhysicalParallelLabelAlter(CollHeap *oHeap = CmpCommon::statementHeap())
    : ParallelLabelAlter(oHeap)   {};
  PhysicalParallelLabelAlter(const ParallelLabelAlter& other)
    : ParallelLabelAlter(other)   {};
  ...
  virtual short codeGen(Generator *generator);
}; // class PhysicalParallelLabelAlter

// This class implements the node for resetting labels(purgedata)
// in parallel, i.e. using ESPs. 
class ParallelLabelPurgedata : public ParallelLabelOp
{
   public:

   // Constructor
   ParallelLabelPurgedata(const CorrName &objectName,
     NABoolean isParallel,
     CollHeap *oHeap = CmpCommon::statementHeap());

   // Copy constructor
   ParallelLabelPurgedata(const ParallelLabelPurgedata &otherObj);

   // default constructor
   ParallelLabelPurgedata(CollHeap *oHeap = CmpCommon::statementHeap());

   // destructor
   virtual ~ParallelLabelPurgedata();
...
class PhysicalParallelLabelPurgedata: public ParallelLabelPurgedata
{
public:
  PhysicalParallelLabelPurgedata(CollHeap *oHeap = CmpCommon::statementHeap())
    : ParallelLabelPurgedata(oHeap)   {};
  PhysicalParallelLabelPurgedata(const ParallelLabelPurgedata& other) : ParallelLabelPurgedata(other)     {};
  ...
  virtual short codeGen(Generator *generator);
}; // class PhysicalParallelLabelPurgedata

class DiskLabelStatistics : public ParallelLabelOp
{
   public:

   // Constructor
   DiskLabelStatistics(const CorrName &objectName,
		       NABoolean isParallel,
		       NABoolean labelCountStats,
		       CollHeap *oHeap = CmpCommon::statementHeap());
  ...
  NABoolean labelCountStats() { return labelCountStats_; }

private:
  NABoolean labelCountStats_;
...
class SequenceGenerator : public RelExpr
{
public:

  // constructor
 SequenceGenerator(RelExpr* child,
                   SequenceGeneratorAttributes &sgAttributes,
                   CollHeap *oHeap = CmpCommon::statementHeap())
   : RelExpr(REL_SEQUENCE_GENERATOR, child, NULL, oHeap),
     sgAttributes_(sgAttributes),
     sgIncrementHV_(NULL_VALUE_ID),
     sgCacheSizeHV_(NULL_VALUE_ID),
     currentValueFromSGTable_(NULL_VALUE_ID),
     addedExchange_(FALSE),
     nvfEstRowsUsed_(0),
     nvfNumESPs_(0)
  {
  }

 SequenceGenerator(CollHeap *oHeap = CmpCommon::statementHeap())
   : RelExpr(REL_SEQUENCE_GENERATOR, NULL, NULL, oHeap),
     sgIncrementHV_(NULL_VALUE_ID),
     sgCacheSizeHV_(NULL_VALUE_ID),
     currentValueFromSGTable_(NULL_VALUE_ID),
     addedExchange_(FALSE),
     ...
     nvfNumESPs_(0)
  {
    SequenceGeneratorAttributes *pSGAttributes = new (oHeap) SequenceGeneratorAttributes(oHeap);
    sgAttributes_ = *pSGAttributes;
  }
  ...
  inline ValueId sgIncrementHV() 
                   {return sgIncrementHV_;}
  inline void setSGIncrementHV(ValueId vid) 
                    {sgIncrementHV_ = vid;}

  inline ValueId sgCacheSizeHV() 
                   {return sgCacheSizeHV_;}
  inline void setSGCacheSizeHV(ValueId vid) 
                    {sgCacheSizeHV_ = vid;}

  inline ValueId currentValueFromSGTable() 
                   {return currentValueFromSGTable_;}
  inline void setCurrentValueFromSGTable(ValueId vid) 
                    {currentValueFromSGTable_ = vid;}

  inline SequenceGeneratorAttributes &getSgAttributes() {return sgAttributes_;}

  inline void setSgAttributes(SequenceGeneratorAttributes &sga) 
  {sgAttributes_ = sga;}

  inline void setNVFEstRowsUsed(double nvfEstRowsUsed) { nvfEstRowsUsed_ = nvfEstRowsUsed; };
  inline double getNVFEstRowsUsed() { return nvfEstRowsUsed_; };
  
  inline void setNVFNumESPs(Lng32 nvfNumESPs) { nvfNumESPs_ = nvfNumESPs; };
  inline Lng32 getNVFNumESPs() { return nvfNumESPs_; };

  static ItemExpr *createSequenceSubqueryExpression(const NAString &sgName,
						    SequenceGeneratorAttributes &sgAttributes, 
						    BindWA *bindWA);

  inline NABoolean isSequenceValueUnique() const 
  { return sgAttributes_.getSGCycleOption(); }

private:

  SequenceGeneratorAttributes sgAttributes_;

  // SG Increment. Serves as input to the child which is 
  // the UPDATE of SG Table.
  ValueId sgIncrementHV_;

  // sgCacheSize - denote N the number of values that can be generated
  // per request. The SG operator sends sgCacheSize as output to the 
  // NextValueFor operator.
  ValueId sgCacheSizeHV_;

  // Current_Value from the SG Table. The SG operator sends
  // the currentValueFromSGTable as output.
  ValueId currentValueFromSGTable_;

  NABoolean addedExchange_;

  // NextValueFor maximum cardinality estimate and number of ESPs
  // used to help refine the SEQUENCE GENERATOR CACHE TDB settings.
  double nvfEstRowsUsed_;
...
class NextValueFor : public RelExpr
{
public:

  // constructor
 NextValueFor(RelExpr* child1 = NULL,
	     RelExpr* child2 = NULL,
	     ValueId resultValueId = NULL_VALUE_ID,
	     CollHeap *oHeap = CmpCommon::statementHeap());
  
  NextValueFor(const NextValueFor &other);

  virtual ~NextValueFor();
  ...
  // method is redefined to say that it requires the current_value, increment and N
  // from the SequenceGenerator operator.
  ...
  ValueId &getResultValue() { return resultValueId_; }
  ValueIdList &getOutputsFromSG() {return outputsFromSG_;}
 
  void setResultValueId (ValueId &vid) 
  {
    resultValueId_ = vid;
  }

  void addToOutputsFromSG(ValueId vid) {outputsFromSG_.insert(vid);}

  inline void setSequenceValueIsUnique(NABoolean uniq)
  {sequenceValueIsUnique_ = uniq;}

  inline NABoolean isSequenceValueUnique(void) const
  { return sequenceValueIsUnique_; }

private:

  // Value id representing the next value for the sequence generator
  ValueId resultValueId_;

  NABoolean sequenceValueIsUnique_;
  
  // we are expecting current_value and N
  ValueIdList outputsFromSG_;
