     ...
     if (childExpr)
        {
           ...
           if (childExpr && childExpr->getOperator() == REL_DP2_SCAN_UNIQUE)
              probeIsUnique = TRUE;
        }
  }
...
// Member functions for DP2 Scan
// -----------------------------------------------------------------------
const NAString DP2Scan::getText() const
{
  // ---------------------------------------------------------------------
  // returns:
  //
  // DP2_[unique_][rev_]file_scan t c	    for a primary index scan on
  // 					    table t with correlation name c
  // DP2_[unique_][rev_]index_scan ix(t c)  for an index scan on index ix
  //					    of table t
  //
  // The "unique_" qualification is added if this is a unique scan.
  // The "rev_" qualification is added for reverse scans.
  // Remember, only the text up to the first blank is used by EXPLAIN.
  // ---------------------------------------------------------------------
...
/*
  ...
  if (getIndexDesc() == NULL OR getIndexDesc()->isClusteringIndex())
    ...
    op += "index_scan ";
    tname = getIndexDesc()->getNAFileSet()->getExtFileSetName() +
	    "(" + tname + ")";
  }
*/
...
const NAString DP2Scan::getTypeText() const
{
  NAString descr(CmpCommon::statementHeap());
  NAString tname(getTableName().getText(),CmpCommon::statementHeap());
  ...
  if (isFullScanPresent())
  {
    descr += "full scan ";
  }
  else if (getOperatorType() == REL_DP2_SCAN_UNIQUE)
  {
    descr += "unique access ";
  }
  else
...
ParallelLabelOp::ParallelLabelOp(const CorrName &objectName,
           ...
           RelExpr(oType, NULL, NULL, oHeap)
{
   ...
   isParallelOp_ = isParallel;
   tabId_ = NULL;
   virtualTabId_ = NULL;
   stoi_ = NULL;
}

ParallelLabelOp::ParallelLabelOp(const ParallelLabelOp &otherObj) :
           RelExpr(otherObj.getOperatorType())
{
   objectName_ = otherObj.objectName_;
   tabId_ = otherObj.tabId_;
   isParallelOp_ = otherObj.isParallelOp_;
   virtualTabId_ = otherObj.virtualTabId_;
   stoi_ = otherObj.stoi_;
}

ParallelLabelOp::ParallelLabelOp(OperatorTypeEnum oType, CollHeap *oHeap) :
           RelExpr(oType, NULL, NULL, oHeap)
{
}

NABoolean ParallelLabelOp::duplicateMatch(const RelExpr &other) const
{
   if (NOT RelExpr::duplicateMatch(other))
      return FALSE;

   ParallelLabelOp &o = (ParallelLabelOp &)other;

   if (NOT (objectName_ == o.getTableName()) OR
       NOT (tabId_ == o.getTableDesc()) OR
       NOT (isParallelOp_ == o.isParallelOp()))
      return FALSE;

   return TRUE;
}

ParallelLabelOp::~ParallelLabelOp() {}

void ParallelLabelOp::getPotentialOutputValues(ValueIdSet & outputValues) const
{
  outputValues.clear();
  if ((getVirtualTableDesc()) &&
      (((ParallelLabelOp*)this)->producesOutput()))
    outputValues.insertList(getVirtualTableDesc()->getColumnList());
} // ParallelLabelOp::getPotentialOutputValues()

ParallelLabelCreate::ParallelLabelCreate(const CorrName &objectName,
                                         NABoolean isParallel,
                                         CollHeap *oHeap) :
     ParallelLabelOp(objectName, isParallel,
		     REL_PARALLEL_LABEL_CREATE, oHeap)
{
}

ParallelLabelCreate::ParallelLabelCreate(CollHeap *oHeap) :
            ParallelLabelOp(REL_PARALLEL_LABEL_CREATE, oHeap)
{
}

ParallelLabelCreate::ParallelLabelCreate(const ParallelLabelCreate &otherObj) :
            ParallelLabelOp(otherObj)
{
}

ParallelLabelCreate::~ParallelLabelCreate() {}

const NAString ParallelLabelCreate::getText() const
{
   return "parallel_label_create";
}

HashValue ParallelLabelCreate::topHash()
{
   HashValue result = RelExpr::topHash();
...
NABoolean ParallelLabelCreate::duplicateMatch(const RelExpr &other) const
{
   return ParallelLabelOp::duplicateMatch(other);
}

RelExpr * ParallelLabelCreate::copyTopNode(RelExpr *derivedNode,
                                           CollHeap * outHeap)
{
   ParallelLabelCreate *result;
   CMPASSERT (derivedNode == NULL);
   result = new (outHeap) ParallelLabelCreate(getTableName(),
                                              isParallelOp(), outHeap);
   return ParallelLabelOp::copyTopNode(result, outHeap);
}

ParallelLabelDrop::ParallelLabelDrop(const CorrName &objectName,
                                     NABoolean isParallel,
                                     CollHeap *oHeap) :
            ParallelLabelOp(objectName, isParallel,
                            REL_PARALLEL_LABEL_DROP, oHeap)
{
}

ParallelLabelDrop::ParallelLabelDrop(CollHeap *oHeap) :
            ParallelLabelOp(REL_PARALLEL_LABEL_DROP, oHeap)
{
}

ParallelLabelDrop::ParallelLabelDrop(const ParallelLabelDrop &otherObj) :
            ParallelLabelOp(otherObj)
{
}

ParallelLabelDrop::~ParallelLabelDrop() {}

const NAString ParallelLabelDrop::getText() const
{
   return "parallel_label_drop";
}

HashValue ParallelLabelDrop::topHash()
{
   HashValue result = RelExpr::topHash();
...
NABoolean ParallelLabelDrop::duplicateMatch(const RelExpr &other) const
{
   return ParallelLabelOp::duplicateMatch(other);
}

RelExpr * ParallelLabelDrop::copyTopNode(RelExpr *derivedNode,
                                         CollHeap * outHeap)
{
   ParallelLabelDrop *result;
   CMPASSERT (derivedNode == NULL);
   result = new (outHeap) ParallelLabelDrop(getTableName(),
                                            isParallelOp(), outHeap);

   return ParallelLabelOp::copyTopNode(result, outHeap);
}

// ********** Parallel Label Alter *****************

// constructor
ParallelLabelAlter::ParallelLabelAlter(const CorrName &objectName, // actual table/index name
                                       NABoolean isParallel,	   // parallel or not
				       UInt32 opc,	   // opcode to be altered
				       const NAString &val,        // new value for update
				       NABoolean isRfork,         // TRUE, if alter is being
				                                  // done to resource fork
				       CollHeap *oHeap) :
        ParallelLabelOp(objectName, isParallel, REL_PARALLEL_LABEL_ALTER, oHeap),
        altOpCode(opc),
        newValue(val, oHeap),
	isRfork_(isRfork)
{
...
// default constructor
ParallelLabelAlter::ParallelLabelAlter(CollHeap *oHeap) :
            ParallelLabelOp(REL_PARALLEL_LABEL_ALTER, oHeap),
            newValue(oHeap),
	    isRfork_(FALSE)
{
}
...
// copy constructor
ParallelLabelAlter::ParallelLabelAlter(const ParallelLabelAlter &otherObj) :
            ParallelLabelOp(otherObj),
            altOpCode(otherObj.altOpCode),
	    isRfork_(otherObj.isRfork_),
            newValue(otherObj.newValue, CmpCommon::statementHeap())
{
}

// destructor
ParallelLabelAlter::~ParallelLabelAlter() {}

const NAString ParallelLabelAlter::getText() const
{
   return "parallel_label_alter";
}

HashValue ParallelLabelAlter::topHash()
{
   HashValue result = RelExpr::topHash();
...
NABoolean ParallelLabelAlter::duplicateMatch(const RelExpr & other) const
{
   if (NOT ParallelLabelOp::duplicateMatch(other))
      return FALSE;

   ParallelLabelAlter &o = (ParallelLabelAlter &)other;

   if (NOT (altOpCode == o.getAltOpCode()) OR
       NOT (newValue == o.getNewValue()) OR
       NOT (isRfork_ == o.isRfork()))
      return FALSE;

   return TRUE;
}

RelExpr * ParallelLabelAlter::copyTopNode(RelExpr * derivedNode,
                                          CollHeap * outHeap)
{
   ParallelLabelAlter *result;
   CMPASSERT (derivedNode == NULL);

   result = new (outHeap) ParallelLabelAlter(getTableName(),
                                             isParallelOp(),
					     getAltOpCode(),
					     getNewValue(),
					     isRfork(),
...
ParallelLabelPurgedata::ParallelLabelPurgedata(const CorrName &objectName,
                                         NABoolean isParallel,
                                         CollHeap *oHeap) :
     ParallelLabelOp(objectName, isParallel,
		     REL_PARALLEL_LABEL_PURGEDATA, oHeap)
{
}

ParallelLabelPurgedata::ParallelLabelPurgedata(CollHeap *oHeap) :
            ParallelLabelOp(REL_PARALLEL_LABEL_PURGEDATA, oHeap)
{
}

ParallelLabelPurgedata::ParallelLabelPurgedata(const ParallelLabelPurgedata &otherObj) :
            ParallelLabelOp(otherObj)
{
}

ParallelLabelPurgedata::~ParallelLabelPurgedata() {}

const NAString ParallelLabelPurgedata::getText() const
{
   return "parallel_label_purgedata";
}

HashValue ParallelLabelPurgedata::topHash()
{
   HashValue result = RelExpr::topHash();
...
NABoolean ParallelLabelPurgedata::duplicateMatch(const RelExpr &other) const
{
   return ParallelLabelOp::duplicateMatch(other);
}

RelExpr * ParallelLabelPurgedata::copyTopNode(RelExpr *derivedNode,
                                           CollHeap * outHeap)
{
   ParallelLabelPurgedata *result;
   CMPASSERT (derivedNode == NULL);
   result = new (outHeap) ParallelLabelPurgedata(getTableName(),
                                              isParallelOp(), outHeap);
   return ParallelLabelOp::copyTopNode(result, outHeap);
}
...
////////////////////////////////////////////////////////////////////
DiskLabelStatistics::DiskLabelStatistics(const CorrName &objectName,
					 NABoolean isParallel,
					 NABoolean labelCountStats,
					 CollHeap *oHeap) :
     ParallelLabelOp(objectName, isParallel,
		     REL_PARALLEL_LABEL_DISK_COUNT, oHeap),
     labelCountStats_(labelCountStats)
{
}

const NAString DiskLabelStatistics::getText() const
{
   return "disk_label_stats";
}

HashValue DiskLabelStatistics::topHash()
{
   HashValue result = RelExpr::topHash();
...
NABoolean DiskLabelStatistics::duplicateMatch(const RelExpr & other) const
{
   return ParallelLabelOp::duplicateMatch(other);
}

RelExpr * DiskLabelStatistics::copyTopNode(RelExpr *derivedNode, CollHeap* outHeap)
{
  DiskLabelStatistics *result;

  if (derivedNode == NULL)
    result = new (outHeap) DiskLabelStatistics(getTableName(),
					       isParallelOp(),
					       labelCountStats_,
					       outHeap);
  else
    result = (DiskLabelStatistics *) derivedNode;
