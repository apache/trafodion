 ...
 // my required phys props (always non-NULL)
  ...
  if ( partReq && partReq->castToRequireExactlyOnePartition() ) {
    myPartFunc = new (CmpCommon::statementHeap())
     ...
     PartitioningFunction * ixDescPartFunc = 
     ...
     else if ( ixDescPartFunc )
     {
       ...
       Lng32 oldPartns = myPartFunc->getCountOfPartitions();
...
Context* SequenceGenerator::createContextForAChild(Context* myContext,
                                                   PlanWorkSpace* pws,
                                                   Lng32& childIndex)
{
  const ReqdPhysicalProperty* rppForMe =
  ...
  // If my requirement is single partition or replicate no broadcast
  // pass it on. Otherwise return NULL. Do not create a requirement
  // here that will change the requirement.
  if(NOT (myPartReq->getCountOfPartitions() == 1 || 
     myPartReq->isRequirementReplicateNoBroadcast())
     ) 
    return NULL;
...
PhysicalProperty*
SequenceGenerator::synthPhysicalProperty(const Context *myContext,
                                         const Lng32 planNumber)
{
  // Simply propagate the physical property
  PhysicalProperty *sppForMe = new(CmpCommon::statementHeap())
    PhysicalProperty(*myContext->getPhysicalPropertyOfSolutionForChild(0));
...
Context* NextValueFor::createContextForAChild(Context* myContext,
                                              PlanWorkSpace* pws,
                                              Lng32& childIndex)
{

  // If the NextValueFor has only one child, then this is using the
  // internally computed ID values (it is not using the SG Table).
  // For this case, the default implementation of
  // createContextForAChild is sufficient.  The unary NVF operator
  // does not need to create any special contexts.
  //
  if(getArity() == 1)
    return RelExpr::createContextForAChild(myContext, pws, childIndex);

  // ---------------------------------------------------------------------
  // Generate one context for each child.
  // If enough Contexts are generated, return NULL
  // to signal completion.
  // ---------------------------------------------------------------------
  if (pws->getCountOfChildContexts() ==  getArity())
  ...
  // Cannot execute in DP2, so don't even try.
  if (rppForMe->executeInDP2())
    {
      return NULL;
    }
  ...
  switch (pws->getCountOfChildContexts())
    ...
    case 0:
      ...
      // -------------------------------------------------------------------
      // Create the Context for left child:
      // -------------------------------------------------------------------
      if ( currentPlanIsAcceptable(planNumber,rppForMe))
        {
          RequirementGenerator rg(child(0), rppForMe);
          ...
          if (child0RowCount.getCeiling() > CURRSTMT_OPTDEFAULTS->numberOfRowsParallelThreshold()) {
          ...
          // Determine if we need to pass a sort requirement to child
          //
          if(rppForMe->getSortKey()) {

            // Remove any INVERSE ops from sort key.
            ValueIdList simplifiedSortKey = rppForMe->getSortKey()->removeInverseOrder();

            // Does the sort key contain the generated ID value.
            NABoolean containsID = rppForMe->getSortKey()->contains(getResultValue());

            // Does the sort key contain INVERSE(ID)
            NABoolean containsInvID = (simplifiedSortKey.contains(getResultValue()) &&
                                       !containsID);

            // If the leading column of the sort key is the ID,
            // then NextValueFor can satisfy this directly.
            // No need to send this requirement to the child
            if(rppForMe->getSortKey()->at(0) == getResultValue())
              {
                rg.removeSortKey();
              }

            // If it does not contain the ID value and the source is a
            // Rowset (tables == 0), then no need to send requirement.
            // Assume that the ROWSET is already sorted. (see Nested_Join).
            else if (!containsInvID &&
                     child(0).getGroupAttr()->getNumBaseTables() == 0)
              {
                rg.removeSortKey();
              }

            // If the sort key contains the ID, then we will send the
            // sort key upto the ID as the requirement for the child.
            //
            else if(containsID) 
              {
                // Make a copy of the sort key.
                ValueIdList sortKey = *(rppForMe->getSortKey());
      
                Int32 i = (Int32)sortKey.entries() - 1;

                // Remove everything after (and including) the ID value.
                while(i >= 0) 
                  {
                    ValueId id = sortKey[i];
                    sortKey.removeAt(i--);
                    if(id == getResultValue()) 
                      break;
                  }

                // remove the sort key requirement (add new one below)
                rg.removeSortKey();
                ...
                // Add the new sort requirement.
                rg.addSortKey(sortKey,
                              childSortOrderTypeReq,
                              childDp2SortOrderPartReq);
                
              }

            // If the sort key contains INVERSE(ID), then the child cannot
            // satisfy this, so no need to send to child.
            else if(containsInvID)
              {
                rg.removeSortKey();
              }
      ...
      if ( currentPlanIsAcceptable(planNumber,rppForMe) )
        {
          ...
          Context* childContext = pws->getChildContext(0,0);
          ...
          if((childContext != NULL) AND childContext->hasOptimalSolution())
            {
	      // Get the physical property of the left child
              const PhysicalProperty *sppForChild =
                childContext->getPhysicalPropertyForSolution();

              PartitioningFunction *leftChildPartFunc =
                sppForChild->getPartitioningFunction();
              
              PartitioningRequirement *partReqForChild;

              // Use the single partitioning function for the right hand child,
	      // if the left child is single partitioned. This is the serial plan case.
	      // For the parallel plan case, make it replicate no broadcast.
	      // Each instance of the SG Operator (right child) has access to
	      // the SG Table.
              if (leftChildPartFunc->isASinglePartitionPartitioningFunction()) {
                partReqForChild = leftChildPartFunc->makePartitioningRequirement();
              } else {
                partReqForChild = new (CmpCommon::statementHeap() )
                  RequireReplicateNoBroadcast(leftChildPartFunc, TRUE);
              }
  ...
  if (rppForChild != NULL)
    {
      ...
      else
      {
        ...
        inputLPForChild = EstLogPropSharedPtr(
                 new(CmpCommon::statementHeap()) EstLogProp(*inputLPForChild));

        CostScalar outputCard = inputLPForChild->getResultCardinality();

	// Assume that we will only probe the SG Table every 10,000 rows.
        CostScalar inputCard = outputCard/10000;
        
	// Assume that there are atleast 5 requests to the SG Operator
	// accessing the SG Table.
        inputCard = MAXOF(inputCard, 5);

	// No more than the output cardinality (outputCard)
        inputCard = MINOF(inputCard, outputCard);

        inputLPForChild->setResultCardinality(inputCard);
      }
      result = shareContext(childIndex, rppForChild,
                            myContext->getInputPhysicalProperty(),
                            costLimit, myContext,
...
PhysicalProperty*
NextValueFor::synthPhysicalProperty(const Context *myContext,
                                         const Lng32 planNumber)
{
  PhysicalProperty* sppForMe = NULL;
  const ReqdPhysicalProperty* rppForMe =
  ...
  NABoolean satisfiesSortKey = FALSE;
  
  // Determine if we satisfy the required key.
  if(rppForMe->getSortKey() && getArity() == 2) 
    {
      ...
      // If the leading column of the sort key is the ID,
      // then NextValueFor can satisfy this directly.
      if(rppForMe->getSortKey()->entries() >= 1 &&
         rppForMe->getSortKey()->at(0) == getResultValue())
        {
          satisfiesSortKey = TRUE;
        }

      // If it does not contain the ID value and the source is a
      // Rowset (tables == 0), we can satisfy the sort key
      // Assume that the ROWSET is already sorted. (see Nested_Join).
      else if (!containsInvID &&
               child(0).getGroupAttr()->getNumBaseTables() == 0)
        {
      ...
      // If the sort key contains the ID, then we will satisfy the sort requirement
      // if the child satisfies sort key upto but not including the ID Value
      else if(containsID) 
        {
          // Make a copy of the sort key.
          ValueIdList sortKey = *(rppForMe->getSortKey());
          ...
          // If the child satisfies the prefix, then we satisfy the whole.
          if(sppOfChild->getSortKey().satisfiesReqdOrder(sortKey, NULL) == SAME_ORDER)
            {
              satisfiesSortKey = TRUE;
            }
        }
  ...
  // If we satisfy the sort requirement, reflect that in our physical properties.
  // otherwise we cannot add any more to the sort key.
  if(satisfiesSortKey)
    {
      sppForMe =
      new (CmpCommon::statementHeap())
   ...
   {
      // Simply propagate the physical property
      sppForMe = new(CmpCommon::statementHeap())
	PhysicalProperty(*sppOfChild);
   }
...
} // NextValueFor::synthPhysicalProperty()

CostMethod*
NextValueFor::costMethod() const
{
  static THREAD_P CostMethodFixedCostPerRow *m = NULL;
  if (m == NULL)
    m = new (GetCliGlobals()->exCollHeap())
            CostMethodFixedCostPerRow( 1.0  // constant cost for the node
                                     , 0.5  // cost per child row
                                     , 0.5  // cost per output row
                                     );
  return m;
} 
