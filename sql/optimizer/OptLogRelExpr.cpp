...
void
SequenceGenerator::synthEstLogProp(const EstLogPropSharedPtr& inputEstLogProp)
{
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp))
    return;
...
} // SequenceGenerator::synthEstLogProp

void
SequenceGenerator::synthLogProp(NormWA * normWAPtr)
{
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  ...
  getGroupAttr()->setEmbeddedIUD(NO_OPERATOR_TYPE);
...
} // SequenceGenerator::synthLogProp()

void
NextValueFor::synthEstLogProp(const EstLogPropSharedPtr& inputEstLogProp)
{
  if (getGroupAttr()->isPropSynthesized(inputEstLogProp) == TRUE) 
    return;
...
} // NextValueFor::synthEstLogProp

void
NextValueFor::synthLogProp(NormWA * normWAPtr)
{
  // Check to see whether this GA has already been associated
  // with a logExpr for synthesis.  If so, no need to resynthesize
  // for this equivalent log. expression.
  ...
  // there should be no selection predicates in this operator.
  // if there are any predicates, the constraints below
  // may not be valid.
  ...
  // Add a uniqness constraint for the generated value.
  if(isSequenceValueUnique()) 
    getGroupAttr()->addConstraint(new(CmpCommon::statementHeap())
                                  UniqueOptConstraint(getResultValue()));

} // NextValueFor::synthLogProp()
