...
{
  // Validate DIVISION BY clause in a DDL statement
  // Check that
  // a) the DIVISION BY expressions reference only
  //    clustering key columns
  // b) the DIVISION BY expressions conform to the
  //    supported types of expressions
  ValueIdSet clusteringKeyCols;
  ValueIdSet allCols = getColumnList();

  // Find the base columns of the clustering key, excluding the
  // division columns
  for (CollIndex j=0; j<getClusteringIndex()->getIndexKey().entries(); j++)
    {
      ItemExpr *ck = getClusteringIndex()->getIndexKey()[j].getItemExpr();
      ...
      if (!((BaseColumn *) ck)->getNAColumn()->isDivisioningColumn())
        clusteringKeyCols += ck->getValueId();
    }
  
  for (CollIndex i=0; i<colList_.entries(); i++)
    {
      BaseColumn *bc = (BaseColumn *) colList_[i].getItemExpr();

      CMPASSERT(bc->getOperatorType() == ITM_BASECOLUMN);

      if (((BaseColumn *) bc)->getNAColumn()->isDivisioningColumn())
        {
          ItemExpr *divExpr = bc->getComputedColumnExpr().getItemExpr();
          ValueIdSet underlyingCols;
          ValueIdSet compExprSet = bc->getComputedColumnExpr();
          
          // Get the underlying columns. Note: We can't call
          // BaseColumn::getUnderlyingColumnsForCC() here, because
          // that method assumes a valid DIVISION BY clause and only
          // looks for clustering key columns
          // don't use: bc->getUnderlyingColumnsForCC(underlyingCols);
          underlyingCols.accumulateReferencedValues(allCols, compExprSet);

          // are the underlying columns a subset of the clustering key?
          if (!clusteringKeyCols.contains(underlyingCols))
            {
              NAString unparsed;
              divExpr->unparse(unparsed, BINDER_PHASE, COMPUTED_COLUMN_FORMAT);
              *CmpCommon::diags() << DgSqlCode(-4240) << DgString0(unparsed);
              bindWA->setErrStatus();
              return;
            }
            ...
            case ITM_YEARWEEKD:
              {
                ItemExpr *unboundExpr =
                  ZZZBinderFunction::tryToUndoBindTransformation(divExpr);
                if (unboundExpr)
                  divExpr = unboundExpr;
              }
            default:
          ...
          switch (divExpr->getOperatorType())
            ...
            case ITM_BASECOLUMN:
              {
                // a simple column will be allowed later, when we have
                // integrated with the new SMD tables for M6
                exprIsValid = FALSE;
            ...
            case ITM_EXTRACT_ODBC:  // for YEAR
              {
                ...
                if (ef == REC_DATE_YEAR ||
                    ...
                    ef == REC_DATE_YEARMONTH_D_EXTRACT)
                  {
                    ...
                    if (divExpr->child(0)->getOperatorType() != ITM_BASECOLUMN)
                      {
                        if ((divExpr->child(0)->getOperatorType() == ITM_PLUS ||
                             divExpr->child(0)->getOperatorType() == ITM_MINUS) &&
                            divExpr->child(0)->child(0)->getOperatorType() == ITM_BASECOLUMN)
                          {
                  ...
                  {
                    // LCOV_EXCL_START
                    // invalid type of extract field (but those get mapped to some
                    // other function, so we never reach here)
            ...
            case ITM_DATEDIFF_MONTH:
              ...
              if (divExpr->child(1)->getOperatorType() != ITM_BASECOLUMN)
                {
                  exprIsValid = FALSE;
                  missingColHere = divExpr->child(0);
                }
                ...
                if (divExpr->child(0)->getOperatorType() != ITM_BASECOLUMN)
                  {
                    if (divExpr->child(0)->getOperatorType() == ITM_PLUS)
                      {
                        if (divExpr->child(0)->child(0)->getOperatorType() != ITM_BASECOLUMN)
                          {
                            exprIsValid = FALSE;
            ...
            case ITM_SUBSTR:
              {
                ...
                // LEFT(<col>, <const>), which gets transformed in the binder to
                //                       the form above
                if (divExpr->child(0)->getOperatorType() != ITM_BASECOLUMN)
                  {
                ...
                if (exprIsValid)
                  {
                    // additional conditions: Second argument must be a
