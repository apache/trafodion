...
// This method is to fixup the SequenceGenerationExpr
// so that the upper bits (8-15) contains the partition 
// input value. This value is unique to an ESP.

static void fixupSequenceGenerationExpr(Generator * generator,
			      ItemExpr *sequenceGenerationExpr,
			      CollHeap *heap)
{

  

  // We don't want to 'fixup' the sequenceGenerationExpr if the 
  // user has requested to mask bits > 16. The default is 16.
  // Anything != 16, we assume the user requested it.
  ULng32 bitsToMask = getDefaultAsLong(IDENTITY_MASK_BITS);
  if (bitsToMask != 16)
    return;

  // The following tree is constructed in this method
  //                 |
  //                Mask           
  //       (clear upper 8 bits and lower 48 bits)
  //                  |
  //                Shift Left 24
  //                  |
  //              Cast to LargeInt
  ...
  const PartitioningFunction *myPartFunc = 
    generator->getFragmentDir()->getPartitioningFunction(myId);

  // Only add the partition information if this plan as ESPs
  // and if the tables are HASH2 partitioned.
  if(generator->getFragmentDir()->getNumESPs(myId) < 2 || 
     NOT myPartFunc->isAHash2PartitioningFunction())
    return;

  ValueIdList pivSet = myPartFunc->getPartitionInputValuesLayout();
  CMPASSERT (pivSet.entries() == 2);
  ValueId pivId = pivSet[0]; // get the first value. That is where the Low values is.
  
  ItemExpr *shiftedValueExpr =  new (heap) Shift(ITM_SHIFT_LEFT,
						 new (heap)
						 Cast(pivId.getItemExpr(),
						      new (heap) SQLLargeInt(1 /* signed */,
									     0 /* not null */)
						      ),
						 new (heap) ConstValue(24)
						 );
  // Mask value so that bytes 8-16 are set
  // and the other bits are cleared.
  Int64 maskValue = 0xff00ffffffffffffLL;

  ItemExpr *partitionInputExpr = new(heap) Mask(ITM_MASK_CLEAR,
					   shiftedValueExpr,
					   new(heap)SystemLiteral (new(heap)
								   SQLLargeInt(1 /* signed */,
									       0 /* not null */),
								   (void *)&maskValue,
								   8) 
					   );
  ...
  replaceBaseValue(sequenceGenerationExpr, partitionInputExpr, heap, FALSE);
...
//
/////////////////////////////////////////////////////////
short PartitionAccess::codeGen(Generator * generator)
{
  GenAssert(FALSE, "Partition access is not supported");
  return 0;
}

CostScalar PartitionAccess::getEstimatedRunTimeMemoryUsage(NABoolean perCPU)
{


  const Lng32 rowSize = 
      getGroupAttr()->getCharacteristicOutputs().getRowLength();
  double totalMemory = getEstRowsUsed().value() * rowSize;
  ...
  if ( perCPU == TRUE ) {
    ...
    Lng32 numBuffersLim = getDefaultAsLong(GEN_PA_NUM_BUFFERS);
    Lng32 bufSizeLim = 56000 /*getDefaultAsLong(EXPAND_LIMIT)*/;
  
    Lng32 memoryLimit = numBuffersLim * bufSizeLim;
...
double PartitionAccess::getEstimatedRunTimeMemoryUsage(ComTdb * tdb)
{
  CostScalar totalMemory = getEstimatedRunTimeMemoryUsage(FALSE);

  // divide by 2 to get an everage values since actual number of buffers
  // varies at run time based on the rate at which rows are consumed and 
  // traffic at a given a PA.  In bytes
  const double memoryLimitPerCpu = 
    (tdb->getNumBuffers() * tdb->getBufferSize())/2; 
  ...
  double memoryPerCpu = totalMemory.value()/numOfStreams ;
...
#pragma warning (default : 4244)  //warning elimination
  ...
  // A test has been added in the binding phase
  // to help determine if this statement
  // should be treated as a UNIQUE INSERT.
  // A flag will have been set indicating
  // the statement is UNIQUE.
  // For now, this is true for inserts with
  // sequence generators, with or without
  // an index.
 
  if (updDelInsert && containsSG() && isUniqueSGInsertType())
    root_tdb->setQueryType(ComTdbRoot::SQL_INSERT_UNIQUE); 
...
/////////////////////////////////////////////////////////
// SequenceGenerator::codeGen()
// 
// A new input tuple is allocated and sent down to the
// child. This input tuple contains the nextNValue(N) and
// the increment(INC) from the SGAttributes needed by the UPDATE
// to set the current_value (current_value = current_value + (N * INC)
/////////////////////////////////////////////////////////
short SequenceGenerator::codeGen(Generator * generator)
{
  ExpGenerator * expGen = generator->getExpGenerator();
  Space * space = generator->getSpace();
  ...
  // 3 tupps for child (const, temps, and inputs (nextNValues, Increment)
  ex_cri_desc * childInputDesc = new(space) ex_cri_desc(3, space);

  generator->setCriDesc(childInputDesc, Generator::DOWN);

  ValueIdList inputs;
  inputs.insert(sgCacheSizeHV_);
  inputs.insert(sgIncrementHV_);

  // Get the update inputs into the maptable.
  ExpTupleDesc *inputsTupleDesc = 0;
  ULng32 inputsTupleLength = 0;
  expGen->processValIdList(inputs,
                           ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
                           inputsTupleLength, 
                           0,
                           childInputDesc->noTuples() - 1,
			   &inputsTupleDesc,  
			   ExpTupleDesc::SHORT_FORMAT); 

  generator->getMapInfo(inputs[0])->codeGenerated();
  generator->getMapInfo(inputs[1])->codeGenerated();

  childInputDesc->setTupleDescriptor(childInputDesc->noTuples() - 1, 
				     inputsTupleDesc);
  ...
  ex_cri_desc *childOutputDesc = generator->getCriDesc(Generator::UP);

  ex_cri_desc * returnedDesc
    = new(space) ex_cri_desc(childOutputDesc->noTuples() + 1, space);
  // Only N (nextNValues) is returned to the parent. 
  Attributes *nextNValueAttr = generator->getMapInfo(inputs[0])->getAttr();
  nextNValueAttr->setAtpIndex(returnedDesc->noTuples() - 1);

  // Executor SGAttributes
  ComSequenceGeneratorAttributes *pSGAttributes = new(space)ComSequenceGeneratorAttributes
    (sgAttributes_.getSGStartValue(),
     sgAttributes_.getSGIncrement(),
     sgAttributes_.getSGMaxValue(),
     sgAttributes_.getSGMinValue(),
     sgAttributes_.getSGCycleOption(),
     sgAttributes_.getSGSQLDataType()
     );

  // Sequence generator "cache" determines the number of 
  // next values that this request can cache.
  UInt32 sgCache = getDefault(SEQUENCE_GENERATOR_CACHE);
  // Sequence Generator "cache initial" determines the start cache
  // size.  The default is one.
  UInt32 sgCacheInitial = getDefault(SEQUENCE_GENERATOR_CACHE_INITIAL);
  // Sequence Generator "cache increment" determines by what factor
  // the cache should be increased for the successive requests.
  UInt32 sgCacheIncrement = getDefault(SEQUENCE_GENERATOR_CACHE_INCREMENT);
  // The "cache maximum" puts a limit on the cache size. The default is 10,000.
  UInt32 sgCacheMaximum = getDefault(SEQUENCE_GENERATOR_CACHE_MAXIMUM);
  // Determine the number of times to retry for file system error 73 (table/row locked).
  UInt32 sgCacheRetry = getDefault(SEQUENCE_GENERATOR_CACHE_RETRY);

  double nvfEstRowsUsed = getNVFEstRowsUsed();
  Lng32  nvfNumESPs = getNVFNumESPs();

  // Determine if the sequence generator cache TDB values
  // can be set to an optimized setting based
  // on the maximum cardinality estimate and 
  // the number of ESPs captured from the NVF operator.

  // If the user has designated a direct override
  // to the SEQUENCE_GENERATOR_CACHE_INITIAL value,
  // then do not modify the value.  The only way
  // to determine if the CQD has an override, is if the
  // value is a number other than the default of 1.

  // If the user has designated a direct override
  // to the SEQUENCE_GENERATOR_CACHE_MAXIMUM value,
  // then do not modify the value.  The only way
  // to determine if the CQD has an override, is if the
  // value is a number other than the default of 10000.

  UInt32 nvfComputation = 0;

  if (nvfNumESPs > 0 && nvfEstRowsUsed > 0)
  {
    nvfComputation = (UInt32) nvfEstRowsUsed/nvfNumESPs;
    if (nvfNumESPs > 1) 
      nvfComputation += 1;

    if (nvfComputation > sgCacheInitial)
    {
      
      if (nvfComputation > sgCacheMaximum)
      {
	if (sgCacheInitial == 1)
          sgCacheInitial = nvfComputation;

	if (sgCacheMaximum == 10000)
          sgCacheMaximum = nvfComputation;
      }
      else
      {
        if (sgCacheInitial == 1)
  ...
  ComTdbSequenceGenerator *seqGenTdb
    = new(space) ComTdbSequenceGenerator(childTdb,
                                         givenDesc,      // given_cri_desc
                                         returnedDesc,   // returned cri desc
					 // Down queue size
                                         (queue_index)getDefault(SEQUENCE_GENERATOR_SIZE_DOWN),
                                         // Up queue size
					 (queue_index)getDefault(SEQUENCE_GENERATOR_SIZE_UP),
					 returnedDesc->noTuples() - 1,
					         // Index in atp of ret tuple.
					 pSGAttributes,
					 sgCache,
					 sgCacheInitial,
					 sgCacheIncrement,
					 ...
					 // Number of buffers to allocate
                                         (Lng32)getDefault(SEQUENCE_GENERATOR_NUM_BUFFERS),      
					 // Size of each buffer
                                         getDefault(SEQUENCE_GENERATOR_BUFFER_SIZE)
					 );  
  
  generator->initTdbFields(seqGenTdb);
  ...
  if(!generator->explainDisabled()) {
    generator->setExplainTuple(
       addExplainInfo(seqGenTdb, childExplainTuple, 0, generator));
  }
  generator->setCriDesc(givenDesc, Generator::DOWN);
  generator->setCriDesc(returnedDesc, Generator::UP);
  generator->setGenObj(this, seqGenTdb);

   return 0;
}

/////////////////////////////////////////////////////////
// NextValueFor::codeGen
// 
// 
// moveSGOutputExpr - this expression is used to copy
//                    the current value and N into local variables
//                    of the nextValueFor operator. This is done
//                    to free the child resources (tuple holding the
//                    current value and N) and also to speed up
//                    the next value generation.
// pNVSGAttributes - since this operator generates the next value
//                   a copy of the SGAttributes is kept here for
//                   bound checking (example: if the next value
//                   generated is within the MAXVAL in the SGAttributes)
//                    
/////////////////////////////////////////////////////////

short NextValueFor::codeGen(Generator * generator)
{
  Space * space = generator->getSpace();
  ExpGenerator *expGen = generator->getExpGenerator();
  ComTdb * leftTdb = NULL;
  ...
  ExpTupleDesc *sgOutputTupleDesc = NULL;
  Int32 workAtp = 1;
  Int32 workAtpIndex = 2;
  ...
  // 0: for constants (not used)
  // 1: for temps (not used)
  // 2: getting the current_value and N from SG Operator.
  ex_cri_desc *workCriDesc = new (space) ex_cri_desc(3, space);

  // Prepare the cardinality estimates for the SequenceGenerator codeGen.
  // The NVF maximum cardinality estimate is preserved for use by the SequenceGenerator.
  // The NVF number of ESPs are obtained and preserved for use by the SequenceGenerator.

  // Get some info about degree of parallelism and est. cardinality.
  // This info is passed to the SequenceGenerator operator
  // and is used below to adjust initial queue sizes.
  double estRowsUsed = getEstRowsUsed().getValue();
  CollIndex myId = generator->getFragmentDir()->getCurrentId();
  Lng32 numESPs = generator->getFragmentDir()->getNumESPs(myId);

  // If the NVF is not in a separate ESP,
  // Set the value to 1 to indicate at
  // least 1 SG ESP will be generated.
  if (numESPs == 0)
    numESPs++;

  if(getArity() == 2) {
    RelExpr *child1 = child(1);

    if(child1->getOperatorType() == REL_EXCHANGE) {
      child1 = child1->child(0);
    }

    GenAssert(child1->getOperatorType() == REL_SEQUENCE_GENERATOR,
              "Bad Sequence Generator Query Tree");

    SequenceGenerator *sg = 
      ((SequenceGenerator *)(child1->castToRelExpr()));

    sg->setNVFEstRowsUsed(estRowsUsed);

    sg->setNVFNumESPs(numESPs);
  }

  // Send the request to left child.
  generator->setCriDesc(givenDesc, Generator::DOWN);
  // generate code for child tree
  child(0)->codeGen(generator);
  ...
  // Returns the row from the left, plus the tuple with the nextValue
  ex_cri_desc *returnedDesc = new (space) ex_cri_desc(leftChildDesc->noTuples()+1,
                                                      space);

  // nextValueFor operator has two children when the Sequence Generator
  // is used for generating values. The left child is the source of the insert
  // and the right child is the access to SGTable.
  // nextValueFor operator has one child when an INTERNAL method (not accessing
  // the SG Table) based on timestamp is used to generate
  // the next value.
  if(getArity() == 2) {
    // Send the request to right child.
    generator->setCriDesc(givenDesc, Generator::DOWN);
    // generate code for child tree
  ...
  // The entry is the host var that holds the 
  // generated nextValue - _sys_hostNextValue.
  ValueIdList generatedNextValueList;
  generatedNextValueList.insert(resultValueId_);

  // set atp, atp index, and add to map table
  ExpTupleDesc *generatedNextValueTupleDesc = 0;
  ULng32 generatedNextValueTupleLength = 0;

  expGen->processValIdList
    (generatedNextValueList,
     ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
     generatedNextValueTupleLength,         // [OUT] target tuple length
     0,                                     // [IN] target atp number
     returnedDesc->noTuples() - 1,          // [IN] target tupp index
     &generatedNextValueTupleDesc,          // [optional OUT] target tuple desc
     ExpTupleDesc::LONG_FORMAT);            // [optional IN] target desc format
  
  GenAssert(generatedNextValueTupleLength == sizeof(Int64), "NextValueFor: bad next value");

  generator->getMapInfo(generatedNextValueList[0])->codeGenerated();

  // add this descriptor to the returnedDesc descriptor.
  returnedDesc->setTupleDescriptor(returnedDesc->noTuples()-1, generatedNextValueTupleDesc);

  // Generate a contigious move expression to copy
  // the current value and N into local variables
  
  ex_expr *moveSGOutputExpr = NULL;
  ULng32 sgOutputLength = 0;

  generator->getExpGenerator()->
    generateContiguousMoveExpr(outputsFromSG_,
			       1, // do  add conv nodes
			       workAtp,
			       workAtpIndex,
			       ExpTupleDesc::SQLARK_EXPLODED_FORMAT,
  ...
  workCriDesc->setTupleDescriptor(workAtpIndex,sgOutputTupleDesc);
   
  ComSequenceGeneratorAttributes *pNVSGAttributes = NULL;

  // Executor nvSGAttributes. It's the exact same one that is there
  // in the SG Operator.
  if(getArity() == 2) {
     RelExpr *child1 = child(1);
    ...
    SequenceGeneratorAttributes &sgAttributes = 
      ((SequenceGenerator *)(child1->castToRelExpr()))->getSgAttributes();

    pNVSGAttributes = new(space)ComSequenceGeneratorAttributes
      (sgAttributes.getSGStartValue(),
       sgAttributes.getSGIncrement(),
       sgAttributes.getSGMaxValue(),
       sgAttributes.getSGMinValue(),
       sgAttributes.getSGCycleOption(),
       sgAttributes.getSGSQLDataType()
       );
  } else {
    pNVSGAttributes = new(space)ComSequenceGeneratorAttributes
      (0,                      // Start
       ...
       0,                      // Min value
       FALSE,                  // Cycle
       COM_LARGEINT_SIGNED_SDT // SQL Datatype
       );
  }

  // Get the defined default values for max queue sizes
  queue_index downLengthDef = (queue_index)getDefault(NEXT_VALUE_FOR_SIZE_DOWN);
  queue_index upLengthDef = (queue_index)getDefault(NEXT_VALUE_FOR_SIZE_UP);

  // Set the up queue size based on the estimated rows.
  queue_index upLength = (queue_index)(estRowsUsed/numESPs);
  
  // Make sure it is less than 16K
  upLength = (upLength > 16384 ? 16384 : upLength);

  // Make sure it is at least as big as the default
  upLength = (upLength < upLengthDef ? upLengthDef : upLength);

  // For the max down length, use the defined default.

  ComTdbNextValueFor *nextValueForTdb =   
    new(space) ComTdbNextValueFor (leftTdb, // child1
                                   rightTdb, // child2
                                   givenDesc, // criDescParentDown,
                                   returnedDesc, //criDescParentUp,
                                   workCriDesc, // workCriDesc
                                   // Down queue size
                                   downLengthDef,
                                   // Up queue size
                                   upLength,
                                   // atp index of return tuple to parent.
                                   returnedDesc->noTuples() - 1, 
                                   pNVSGAttributes,
                                   moveSGOutputExpr,
                                   // Number of buffers to allocate
                                   (Lng32)getDefault(NEXT_VALUE_FOR_NUM_BUFFERS),      
                                   // Size of each buffer
                                   getDefault(NEXT_VALUE_FOR_BUFFER_SIZE)
                                   );
   
  generator->initTdbFields(nextValueForTdb);
   
  // set the initial up queue size to be 1/4 the max
  upLength = upLength/4;
  upLength = (upLength < 4 ? 4 : upLength);

  nextValueForTdb->setQueueResizeParams(downLengthDef, upLength,
                                        nextValueForTdb->getQueueResizeLimit(),
                                        nextValueForTdb->getQueueResizeFactor());
  ...
  if(!generator->explainDisabled()) {
    generator->setExplainTuple(
                               addExplainInfo(nextValueForTdb, leftExplainTuple, rightExplainTuple, generator));
  }
  
  generator->setCriDesc(givenDesc, Generator::DOWN);
  generator->setCriDesc(returnedDesc, Generator::UP);
  generator->setGenObj(this, nextValueForTdb);
