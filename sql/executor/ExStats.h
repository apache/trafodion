...
// class ExReorgStats
//////////////////////////////////////////////////////////////////
class ExReorgStats : public ExOperStats {
  friend class ExExeUtilGetReorgStatisticsTcb;

public:
  ExReorgStats(NAMemory * heap,
	       ex_tcb *tcb,
	       const ComTdb * tdb);
  ...
  NA_EIDPROC
  void copyContents(ExReorgStats* other);

  void merge(ExReorgStats* other);
  ...
  ExReorgStats * castToExReorgStats();
  ...
  void initReorgProgress();
  void updateReorgProgress(Lng32 tableNum, Lng32 partnNum,
			   UInt16 status, UInt16 percentDone);
  
  void finalizeReorgProgress();
  void computePartnProgress(CollIndex partnNum, char &tableStatus,
			    NABoolean withInit);

  struct ReorgPartnStats
  {
    char reorgStatus_;
    //    UInt16 reorgStatus_;
    //    UInt16 percentDone_;
  ...
  struct ReorgTableStats
  {
    UInt16 tableNameLen_;
    char * tableName_;
    UInt16 numPartns_;
    ReorgPartnStats * partnsList_;
    UInt16 reorgStatus_;
  };

  UInt32 numTables() { return numTables_; }
  UInt32 numPartns() { return numPartns_; }
  ReorgTableStats * getTableStatsList() { return tableStatsList_; }

private:
  UInt32 numTables_;
  UInt32 numPartns_;

  ReorgTableStats * tableStatsList_;

  Lng32 tablesTotal_;
  Lng32 tablesNotStarted_;
  Lng32 tablesReorged_;
  Lng32 tablesInProgress_;
  Lng32 tablesReorgNotNeeded_;
  Lng32 tablesInError_;

  Lng32 partnsTotal_;
  Lng32 partnsNotStarted_;
  Lng32 partnsReorged_;
  Lng32 partnsInProgress_;
  Lng32 partnsReorgNotNeeded_;
  Lng32 partnsInError_;
