				      ...
				      NAString &currCatName, NAString &currSchName)
{
  ...
  if (isSeabaseMD(catName, schName, NAString("dummy")))
    {
      *CmpCommon::diags() << DgSqlCode(-CAT_USER_CANNOT_DROP_SMD_TABLE)
			  << DgTableName(schName);
...
// Unconditioanlly use range-partitioning for salted tables. There is
// some issue with accessing regions of a salted table. This is a temp.
// workaround for it.
/*
      if (tableIsSalted)
