...
class CmpSeabaseObjectsTable
{
  public:

    CmpSeabaseObjectsTable ();

    const NAString getCatName() { return catName_; }
    const NAString getSchName() { return schName_; }
    const NAString getObjName() { return objName_; }
    Int64 getObjUID() { return objUID_; }
    ComObjectType  getObjType() { return objType_; }
    Int32 getObjOwner() { return objOwner_; }
    Int64 getRedefTime() { return redefTime_; }
    Int64 getCreateTime() { return createTime_; }

    NABoolean isValueDef() { return validDef_; }
    NABoolean isDataRetrieved() { return dataRetrieved_; }

    NABoolean getObjRow();
    NABoolean getObjRow(const NAString catName,
                        const NAString schName,
                        const NAString objName,
                        ComObjectType  objType);

    void setCatName(const NAString catName) {catName_ = catName;}
    void setSchName(const NAString schName) {schName_ = schName;}
    void setObjName(const NAString objName) {objName_ = objName;}
    void setObjType(ComObjectType  objType) {objType_ = objType;}
    void setDataRetrieved(NABoolean retrieved) {dataRetrieved_ = retrieved;}
  ...
  private:
     ...
     Int64              redefTime_;
     NABoolean          validDef_;
     NABoolean          dataRetrieved_;

     Int32 readObjTable(const NAString catName,
                        const NAString schName,
                        const NAString objName,
                        ComObjectType  objType);
};
