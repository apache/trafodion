...
// ****************************************************************************
// Methods for CmpSeabaseObjectsTable class
// ****************************************************************************

// ----------------------------------------------------------------------------
// constructor
// ----------------------------------------------------------------------------
CmpSeabaseObjectsTable::CmpSeabaseObjectsTable ()
: objUID_ (0),
  objType_ (COM_UNKNOWN_OBJECT),
  objOwner_ (NA_UserIdDefault),
  createTime_ (0),
  redefTime_ (0),
  validDef_ (FALSE),
  dataRetrieved_ (FALSE)
{}

// ----------------------------------------------------------------------------
// method: getObjRow
//
// calls getObjRow(<primary key cols>)
//
// If unable to get the data return FALSE
// ----------------------------------------------------------------------------
NABoolean CmpSeabaseObjectsTable::getObjRow( void )
{
   if ( dataRetrieved_ == FALSE )
     return FALSE;
   return getObjRow(catName_, schName_, objName_, objType_);
}

// ----------------------------------------------------------------------------
// method: getObjRow
//
// if data has not retrieved, reads the OBJECTS metadata table to get data
//
// returns TRUE if successful, FALSE if not.
// ----------------------------------------------------------------------------
NABoolean CmpSeabaseObjectsTable::getObjRow ( const NAString catName,
                                              const NAString schName,
                                              const NAString objName,
                                              ComObjectType objType)
{
  ...
  NAString stmt;
  stmt = "select object_type, object_UID, create_time, redef_time, valid_def, object_owner from ";
  stmt += CmpSeabaseDDL::getSystemCatalogStatic();
  stmt += ".\"";
  stmt += SEABASE_MD_SCHEMA;
  stmt += "\".";
  stmt += SEABASE_OBJECTS;
  ...
  // value 1: object_type (char 2)
  //   translate to ComObjectType  
  cliInterface.getPtrAndLen(1, ptr, len);
  char objTypeLit [3];
  char * pValue = &objTypeLit[0];
  size_t myLen = 2;
  char * pDest = strncpy(pValue, ptr, myLen);
  if (strncmp (objTypeLit, COM_BASE_TABLE_OBJECT_LIT, myLen) == 0)
    objType_ = COM_BASE_TABLE_OBJECT;
  else if (strncmp (objTypeLit, COM_INDEX_OBJECT_LIT, myLen) == 0)
    objType_ = COM_INDEX_OBJECT;
  else if (strncmp (objTypeLit, COM_VIEW_OBJECT_LIT, myLen) == 0)
    objType_ = COM_VIEW_OBJECT;
  else
    objType_ = COM_UNKNOWN_OBJECT;

  // value 2: object_UID (LargeInt)
  cliInterface.getPtrAndLen(2, ptr, len);
  objUID_ = *(Int64*)ptr;

  // value 3: create_time (LargeInt)
  cliInterface.getPtrAndLen(3, ptr, len);
  createTime_ = *(Int64*)ptr;

  // value 4: redef_time (LargeInt)
  cliInterface.getPtrAndLen(4, ptr, len);
  redefTime_ = *(Int64*)ptr;

  // value 5: valid_def (char 2)
  cliInterface.getPtrAndLen(5, ptr, len);
  char validDefLit[3];
  pValue = &validDefLit[0];
  strncpy(pValue, ptr, myLen);
  if (validDefLit[0] == 'Y')
    validDef_ = TRUE;
  else
    validDef_ = FALSE;
