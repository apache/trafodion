...
DcsMaster's homepage you'd point your browser at  _http://master.example.org:40010_.
...
=== _backup-masters_ 
A plain-text file which lists hosts on which the backup Master process, one host per line, should be started.
...
`example1`,`example2`, and so on, through node`example4` in this example. The DCS Master is running
on the node `example1`. DCS Servers run on nodes `example1`-`example4`. A 3-node ZooKeeper ensemble runs on `example1`,
...
Below we list the *important* Configurations. We've divided this section into required configuration 
and worth-a-look recommended configs.
      
=== Required Configurations
Review the <<os,OS>> section.

=== Recommended Configurations
==== `dcs.master.port`
The default value is 37800. This is the port the DcsMaster listener binds to
...
This allows backup DcsMaster to takeover and set the floating IP address.

==== `dcs.master.floating.ip.external.interface`
There is no default value. You must ensure the value is contains the correct interface
for your networking configuration. 

==== `dcs.master.floating.ip.external.ip.address`
There is no default value. It is importatnt that you set this to the dotted IP address appropriate for your network.
          
To change this configuration, edit _dcs-site.xml_, copy the changed file around the cluster and restart.