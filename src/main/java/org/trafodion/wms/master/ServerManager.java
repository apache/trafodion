...
import org.trafodion.wms.master.rpc.thrift.ThriftRpcServer;
...
public class ServerManager implements Callable {
	...
	public ServerManager(WmsMaster wmsMaster) throws Exception {
		try {
			...
			maxRestartAttempts = conf.getInt(Constants.WMS_MASTER_SERVER_RESTART_HANDLER_ATTEMPTS,Constants.DEFAULT_WMS_MASTER_SERVER_RESTART_HANDLER_ATTEMPTS);
			retryIntervalMillis = conf.getInt(Constants.WMS_MASTER_SERVER_RESTART_HANDLER_RETRY_INTERVAL_MILLIS,Constants.DEFAULT_WMS_MASTER_SERVER_RESTART_HANDLER_RETRY_INTERVAL_MILLIS);
	...
	class RestartHandler implements Callable<ScriptContext> {
		...
		public ScriptContext call() throws Exception {
			try {
				...
				int thriftPort = Integer.parseInt(scn.next());//thriftPort
				...
				String wmsHome = System.getProperty("wms.home.dir");				
     			...
     			//If stop-wms.sh is executed and WMS_MANAGES_ZK then zookeeper is stopped abruptly.
    			//Second scenario is when ZooKeeper fails for some reason regardless of whether WMS
    			//manages it. When either happens the WmsServer running znodes still exist in ZooKeeper
    			...
    			//WmsServer running znodes that have timestamps after last WmsMaster startup.
				if(serverStartTimestamp > startupTimestamp){
					...
					if(hostName.equalsIgnoreCase(ia.getCanonicalHostName())) 
						scriptContext.setCommand("bin/wms-daemon.sh --config " + confDir + " start server " + instance);
					else
						scriptContext.setCommand("pdsh -w " + hostName + " \"cd " + wmsHome + ";bin/wms-daemon.sh --config " + confDir + " start server " + instance + "\"");
					...
					while(true) {
						...
						LOG.info("Restarting WmsServer [" + hostName + ":" + instance + "], script [ " + scriptContext.toString() + " ]");
						...
						if(scriptContext.getExitCode() == 0) {
							...
							if (! retryCounter.shouldRetry()) {
								LOG.error("WmsServer [" + hostName + ":" + instance + "] restart failed after "
					...
					LOG.debug("No restart for " + znodePath + "\nbecause WmsServer start time [" + DateFormat.getDateTimeInstance().format(new Date(serverStartTimestamp)) + "] was before WmsMaster start time [" + DateFormat.getDateTimeInstance().format(new Date(startupTimestamp)) + "]");
				}
			} catch (Exception e) {
				e.printStackTrace();
				LOG.error(e);
	...
	public Boolean call() throws Exception {
		...
		try {
			getServersFile();
			getZkRunning();
			cep = new ComplexEventProcessor(zkc,parentZnode,conf,null);
			trpcs = new ThriftRpcServer(wmsMaster);
    ...
    private synchronized void getZkRunning() throws Exception {
    	...
    	if( ! children.isEmpty()) {
    		for(String child : children) {
				...
				int thriftPort = Integer.parseInt(scn.next()); 
	...
	public synchronized ArrayList<Request> getWorkloadsList() {
		ArrayList<Request> workloads = new ArrayList<Request>();
	   	...
	   	LOG.debug("Reading " + parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_WORKLOADS);
	   	
	   	try {
	   		List<String> children =  getChildren(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_WORKLOADS, null);

	   		if( ! children.isEmpty()) {
	   			for(String child : children) {
	   				Request request = new Request();
	   				String workloadZnode = parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_WORKLOADS + "/" + child;
	...
	public synchronized ArrayList<String> getClientsList() {
		ArrayList<String> clients = new ArrayList<String>();
	   	...
	   	try {
	   		List<String> children =  getChildren(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_CLIENTS, null);
	   		if( ! children.isEmpty()) {
	   			for(String child : children) {
	   				clients.add(child);
	   			}
		...
		try {
			...
			if( ! children.isEmpty()) {
				for(String child : children) {
					...
					String streamZnode = parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_STREAMS + "/" + child;
					Stat stat = zkc.exists(streamZnode, false);
					if (stat != null){
						byte[] bytes = zkc.getData(streamZnode, false, stat);
						try {
	...
	public synchronized ArrayList<Rule> getRulesList() {
		ArrayList<Rule> rules = new ArrayList<Rule>();

		LOG.debug("Reading " + parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_RULES);

		try {
			List<String> children =  getChildren(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_RULES, null);

			if( ! children.isEmpty()) {
				for(String child : children) {
					Rule rule = new Rule();
					String ruleZnode = parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_RULES + "/" + child;
					Stat stat = zkc.exists(ruleZnode, false);
					if (stat != null){
						byte[] bytes = zkc.getData(ruleZnode, false, stat);
						try {
