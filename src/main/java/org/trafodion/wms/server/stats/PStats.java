...
/*
		...
		opt.addOption("p",true,"zookeeper port number");
		CommandLine cmd;
		try {
			cmd = new GnuParser().parse(opt, args);
			zkhost = cmd.getOptionValue("i", Constants.LOCALHOST);
			try {
				zkport = Integer.parseInt(cmd.getOptionValue("p", Integer.toString(Constants.DEFAULT_ZOOKEEPER_CLIENT_PORT)));
			} catch (NumberFormatException e){
				LOG.error("Could not parse: ", e);
				zkport = Constants.DEFAULT_ZOOKEEPER_CLIENT_PORT;
			}
			LOG.debug("Command=" + cmd.toString());
			LOG.debug("Options: " + zkhost + " " + zkport);
			System.out.println("Options: " + zkhost + " " + zkport);
		} catch (ParseException e) {
			LOG.error("Could not parse: ", e);
			zkhost = Constants.LOCALHOST;
			zkport = Constants.DEFAULT_ZOOKEEPER_CLIENT_PORT;
		}
*/
		try {
//			ZkClient zkc = new ZkClient(zkhost, zkport);
			...
			String parentZnode = conf.get(Constants.ZOOKEEPER_ZNODE_PARENT,Constants.DEFAULT_ZOOKEEPER_ZNODE_PARENT);	   	
			Stat stat = zkc.exists(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_STATS,false);
			if(stat == null){
				LOG.error(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_STATS + " does not exist");
				throw new IOException(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_STATS + " does not exist");
			}
			String schema = Bytes.toString(zkc.getData(parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_STATS, false, stat));

			InetAddress ip = InetAddress.getLocalHost();
			LOG.info("hostname " + ip.getCanonicalHostName());

			String znode_stats = parentZnode + Constants.DEFAULT_ZOOKEEPER_ZNODE_STATS + "/" + ip.getCanonicalHostName() + ":" + instance;
			stat = zkc.exists(znode_stats, false);
			if (stat != null)
				zkc.delete(znode_stats,-1);
			zkc.create(znode_stats,new byte[0],ZooDefs.Ids.OPEN_ACL_UNSAFE,CreateMode.EPHEMERAL);
